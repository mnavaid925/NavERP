...
| 4 | SCM | `Party` (supplier) · `Item` · `Location` · `StockMove` · `SalesOrder` · `WorkOrder` · `accounting.Bill` (3-way match) · `accounting.Currency`/`PaymentTerm` · `core.Document` | **PurchaseRequisition, RFQ, RFQQuote, PurchaseOrder, GoodsReceiptNote** (4.1) · **SupplierProfile, SupplierScorecard, SupplierContract, SupplierCatalog, SupplierRiskAssessment** (4.2, as-built) + Shipment, Carrier, RoutePlan, DemandForecast, ReturnAuthorization, BillOfMaterials |
