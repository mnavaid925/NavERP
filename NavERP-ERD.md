...
| 4 | SCM | `Party` (supplier) · `SalesOrder` · `WorkOrder` · `accounting.Bill` (3-way match) · `accounting.Currency`/`PaymentTerm` · `core.Document` | **PurchaseRequisition, RFQ, RFQQuote, PurchaseOrder, GoodsReceiptNote** (4.1) · **SupplierProfile, SupplierScorecard, SupplierContract, SupplierCatalog, SupplierRiskAssessment** (4.2) · **the INVENTORY SPINE — ItemCategory, UOM, Item, Location, LotSerial, StockMove — plus StockTransfer, StockAdjustment, ReorderRule** (4.3, as-built) + Shipment, Carrier, RoutePlan, DemandForecast, ReturnAuthorization, BillOfMaterials |
