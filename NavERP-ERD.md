...
| 4 | SCM | `Party` (supplier) · `Item` · `Location` · `StockMove` · `SalesOrder` · `WorkOrder` · `accounting.Bill` (3-way match) | **PurchaseRequisition, RFQ, RFQQuote, PurchaseOrder, GoodsReceiptNote** (4.1, as-built) + Shipment, Carrier, RoutePlan, DemandForecast, ReturnAuthorization, BillOfMaterials |
...
| 6 | Procurement | `scm.PurchaseRequisition` · `scm.RFQ` · `scm.PurchaseOrder` · `scm.GoodsReceiptNote` (all **extended by FK**, not re-declared) · `Party` (vendor) · `Item` · `Contract` · `Document` · `JournalEntry` | VendorScorecard, StrategicSourcingEvent, eAuction, SupplierRisk (Module 6 adds its sourcing/contract layer ON TOP of the SCM 4.1 procurement spine) |
