...
| 4 | SCM | `Party` (supplier) · `SalesOrder` · `WorkOrder` · `accounting.Bill` (3-way match) · `accounting.Currency`/`PaymentTerm` · `core.Document` | **PurchaseRequisition, RFQ, RFQQuote, PurchaseOrder, GoodsReceiptNote** (4.1) · **SupplierProfile, SupplierScorecard, SupplierContract, SupplierCatalog, SupplierRiskAssessment** (4.2) · **the INVENTORY SPINE — ItemCategory, UOM, Item, Location, LotSerial, StockMove — plus StockTransfer, StockAdjustment, ReorderRule** (4.3) · **PutawayTask, PickTask, CycleCountTask, YardVisit** (4.4 WMS, as-built — bins are `Location`s extended with capacity/pick_sequence/abc_class, and cycle counts resolve into the existing StockAdjustment) · **SalesOrder, SalesOrderLine, SalesOrderAllocation** (4.5 OMS, as-built — SCM ships the sales order FIRST, so it OWNS it here; Modules 8/9 EXTEND it by FK rather than re-declaring. The allocation is a SOFT reservation and posts no StockMove: on-hand stays a pure aggregate of the ledger and stock moves only via 4.4's pick) · **Carrier, CarrierRateCard, Load, LoadStop, Shipment, TrackingEvent, FreightInvoice, FreightInvoiceLine** (4.6 TMS, as-built — a `Carrier` is a spine-backed profile on `core.Party`, never a duplicate company table; a shipment's status/ETA/POD are PROJECTED from the append-only `TrackingEvent` log the same way on-hand is projected from `StockMove`; and an approved freight audit DRAFTS an `accounting.Bill` rather than posting a journal entry itself, L29) · **DemandForecast, DemandForecastPeriod, SeasonalityProfile, SeasonalityIndex, DemandSignal, ForecastAdjustment** (4.7 Demand Planning, as-built — demand history is DERIVED on demand from `SalesOrderLine`/`StockMove` and is never stored, so no history table exists to drift; the period grid keeps the decomposition in separate columns (baseline → seasonal index → event uplift → signal → consensus → final) so the plan explains itself; and **Safety Stock Calculation EXTENDS 4.3's `ReorderRule` in place** with the policy/computed columns rather than declaring a second policy table at the same `(tenant, item, location)` grain — `calculate()` writes only the `computed_*` columns and a separate reviewed apply promotes them into the live `safety_stock`/`reorder_point` that 4.1 purchasing reorders against) + RoutePlan, ReturnAuthorization, BillOfMaterials |
| 5 | Inventory (IMS) | `scm.Item`/`ItemCategory`/`UOM`/`Location`/`LotSerial`/`StockMove` + `scm.StockTransfer`/`StockAdjustment`/`ReorderRule` — all **extended by FK**, not re-declared (SCM 4.3 shipped the inventory spine first, L29/L36) · **ItemAttribute, ItemPrice, ProductFile** (5.1, as-built — the `PriceList` planned here shipped as `inventory.ItemPrice`, sell-side price rows over `scm.Item`; the cost side stays on `scm.Item.standard_cost`) | GoodsReceipt (or reuse `scm.GoodsReceiptNote`), CycleCount program/scheduling, PutawayRule, PickList, serial-genealogy — i.e. Module 5 adds the inventory *operations* layer ON TOP of the SCM 4.3 spine; on-hand and valuation stay derived from the append-only `scm.StockMove` |
...
| 8 | Sales | `Party` (customer/contact) · `crm.AccountProfile`/`crm.ContactProfile` · `crm.HealthScore`/`crm.HealthScoreHistory` · `crm.Opportunity` · `crm.Lead`/`crm.Territory`/`crm.EmailCampaign` (CRM remains the identity, health, opportunity, and campaign owner) · `scm.SalesOrder` · `accounting.Invoice` · `Activity` · `Document` · `core.PartyRelationship` · `core.ConsentPurpose` · `core.AuditLog` | **8.1 Lead Management:** `LeadScoreEvent` (append-only score ledger), `LeadQualification` (BANT/MEDDIC assessment), `LeadRoutingRule` (deterministic owner resolver configuration), `LeadNurtureEnrollment` (CRM drip enrollment/lifecycle state). Capture remains CRM-owned; handoff calls the CRM conversion service. **8.3 Contact & Account Management:** `PartyEnrichmentEvent` (local, consent-aware proposal/review evidence; no provider transport), `AccountStakeholder` (account-specific buying-center context), `AccountClassification` (one current human-governed segmentation row), and `AccountPlan` (`ACPL-` time-bounded growth plan with links to existing CRM opportunities). 8.3 adds no duplicate account/contact, hierarchy, health, opportunity, order, invoice, product, or financial-ledger table. Opportunity, Quote, Forecast, Territory, and CommissionPlan remain future Sales work. |
