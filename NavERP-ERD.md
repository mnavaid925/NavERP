...
| 8 | Sales | `Party` (customer) · `SalesOrder` · `Invoice` · `Activity` · `PriceList` · `Contract` · `crm.Lead`/`crm.Opportunity`/`crm.Territory`/`crm.EmailCampaign` (8.1 reuses CRM; Sales adds no duplicate lead/opportunity/territory/campaign/conversion tables) | **8.1 Lead Management:** `LeadScoreEvent` (append-only score ledger), `LeadQualification` (BANT/MEDDIC assessment), `LeadRoutingRule` (deterministic owner resolver configuration), `LeadNurtureEnrollment` (CRM drip enrollment/lifecycle state). Capture remains CRM-owned; handoff calls the CRM conversion service. Opportunity, Quote, Forecast, Territory, and CommissionPlan remain future Sales work. |
