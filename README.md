...
**19 of 19 sub-modules — Module 6 complete.** **6.16 supplier performance & evaluation** is the layer that makes an existing table configurable rather than replacing it: `scm.SupplierScorecard` already shipped with four **hard-coded** dimensions and a class-level `WEIGHTS` dict, so 6.16 declares **no second scorecard** — `SupplierKpiScore` FKs it by string and the "New period" button links out to `scm:scorecard_create`. What it adds is a tenant-definable KPI library (`SupplierKpi` [SKP-], whose `weight` field is what replaces that dict), 360 feedback feeding survey-sourced KPIs (`SupplierFeedback` [SFB-], where one `respondent_kind` CharField buys the whole perception-gap board), and PIPs that escalate to the **existing** `VendorSuspension` register rather than inventing a second blocking mechanism. Generating a scorecard is a documented **one-way door** — it writes SCM's four columns, sets `manual_override`, and SCM's signal engine skips that row forever after — so it refuses a published or archived period outright, and the review caught the case that made it dangerous: generating with an **empty** KPI library used to fire the door on a no-op, destroying a derivable A-grade score while reporting success in a green message. Seven columns are frozen at write time so a later KPI retune cannot rewrite history, a resolver with no data returns `None` rather than a phantom zero that would silently tank a supplier, and the benchmarking boards carry a standing honesty note — the cohort is **your own supply base**, there is no external industry feed, and `industry_benchmark_value` is labelled hand-entered wherever it appears. **6.17 risk & compliance management** is the integrity layer, and its defining ruling is one it did not make alone: `procurement.ProcurementPolicy` already belonged to 6.19, which shipped first, so 6.17 declares **no second policy model** and owns only the acknowledgement half — `PolicyAttestation` FKs that table by string, and 6.19's own docstring had already reserved the ledger for it. A build agent that tried the second declaration got `RuntimeError: Conflicting 'procurementpolicy' models` and stopped rather than renaming around it. The sub-module is deliberately honest about what it can prove: `core.AuditLog` has no hash, no sequence and nothing preventing UPDATE, so `AuditSeal` [ASL-] is a SHA-256 chain that makes tampering **evident, not impossible**, and every page says so — ranges are id-keyed rather than time-keyed (a time window has a late-arrival hole) and `verify()` names the first offending log id, because “this seal is broken” without a location is not actionable. `ComplianceScreening` [SCR-] separates what the *lookup* returned from what a *human* decided into two columns, and `clear()` asks the database rather than its own cached counter — a test forces both counters to zero by raw UPDATE and proves the gate still holds. `SupplierRiskSignal` [SRS-] exists because a bare risk score without provider+metric is a lie: `METRIC_SCALES` is what lets RapidRatings FHR 100 read as healthy and D&B SER 9 as dangerous on the same page, with band and trend derived from risk *position* so a falling SER reads as the improvement it is. `FraudAlert` [FRD-] runs six rules that intersect none of 6.14's eight, writes only its own rows, and refuses an over-long window in **zero queries** — a cap that materialises the range is the payload it was meant to prevent. Sign-off is **owner-only against a peer, a tenant admin, a workspace superuser and the tenant-less superuser**, because a signature somebody else can stamp is not evidence. The vendor bank-detail fraud rule is stated on the page as **not buildable** rather than faked. 470 tests across four lanes. **6.19 document & knowledge management** is the procurement repository, and its central ruling is what it does NOT reuse: `core.Document` stays untouched because its `version` is a flat CharField with no revision chain and its GenericForeignKey cannot be tenant-filtered, joined or faceted on a list page — so 6.19 declares `ProcurementDocument` [PDOC-] with four real link FKs (`core.Party`, `scm.SupplierContract`, `scm.PurchaseOrder`, `procurement.SourcingEvent`). Its spine is an **immutable revision chain**: `ProcurementDocumentRevision` rows are allocated under a `select_for_update()` lock on the parent with a unique-constraint backstop, approve refuses `revision_no <= current_revision_no` (re-checked inside the lock against a TOCTOU race), and `current_revision` filters `is_approved=True` so a re-allocated number can never put an unapproved file on the record. **Full-text search is honest about itself** — an `extracted_text` copy populated by a lazy `pdfplumber` import that never raises, searched with `icontains` only from four characters, and the word OCR appears on no surface because there is none. `ProcurementPolicy` [PPOL-] is the policy library with a cycle-guarded supersession chain and **advisory-only** spend thresholds that enforce nothing (6.3 enforces); it carries `requires_acknowledgment` as a bare hook and ships no sign-off ledger, because 6.17 owns attestation and FKs this table. `KnowledgeResource` [PKR-] holds RFP templates, scorecards and playbooks with an atomic `F()` usage counter. Two security rulings are load-bearing: files are served through an authenticated, tenant-scoped `pdocrevision_download` with `Content-Disposition: attachment` — never `file.url`, because `MEDIA_ROOT` sits under the docroot with no tenant partitioning — and `classification` is **enforced in the queryset** on every surface including the stat tiles and the edit form, not left as a decorative label. 1,003 tests across four lanes. **6.1 user dashboard & portal** is the people/workflow layer AROUND the SCM 4.1 procurement spine (L36: it declares no requisition/PO table of its own): a `ProcurementAlert` Task & Alert Center (kind/severity + an open→acknowledged→resolved lifecycle with who/when stamps, internal-path-only links) and a `WidgetPreference` per-user overview layout (absence of a row = visible). The **Personalized Overview** (`/procurement/`) is a computed page over `scm.PurchaseRequisition` / `scm.PurchaseOrder` aggregates (pending approvals, committed spend this vs last month, open PO value, approaching deadlines) whose sections honour the stored widget choices; **Quick Requisition Entry** drafts a single-line requisition INTO `scm.PurchaseRequisition` in one transaction (requester = signed-in user, derived total, hand-off to scm's submit/approve); the **Recent Activity Feed** is `core.AuditLog` filtered to procurement content types (always windowed, my-actions by default, per-entry field-level diff view, no create/edit/delete — it is a record, not an opinion); **Self-Service Reporting** computes personal usage/spend plus a six-month committed-spend trend and exports the user's OWN requisitions as CSV with formula-injection neutralization. **6.2 requisition management** adds the management lens over the same spine: **Requisition Tracking** (`procurement:req_list` register + a detail page whose timeline IS the immutable `core.AuditLog` trail from draft through approval to PO conversion, with linked RFQs/POs); the **Duplicate Requisition Check** is an explainable heuristic (same title or any line item, case/space-insensitive, within 30 days, live statuses only) surfaced as a badge column on the register, an advisory panel with match reasons on each detail page, a warning after template apply, and a `?dupes=1` deep-link filter — never auto-blocking; **Requisition Templates** [RQT-] (+lines) are recurring-order blueprints whose Apply drafts a fresh spine requisition under the signed-in user (individual line creates so derived totals compute; duplicate warning after); **Cancellation/Amendment** [RAM-] is the gated workflow scm lacks for pending/approved requisitions: requester files cancel-or-amend (header fields + proposed add/update/remove line rows, one open amendment per requisition), tenant admin approves (atomic apply → PR cancelled or changes written + totals recalculated) or rejects with reason. Creation itself stays 4.1's full form (the sidebar bullet links there — extend, never re-declare). Idempotent `seed_procurement` seeds alerts, three templates and a pending amendment against an existing seeded PR. **6.3 approval workflow engine** adds the multi-tier governance AROUND 4.1's requisition spine (the document and its status machine stay `scm.PurchaseRequisition`; scm's own single-step approve remains): **Dynamic Routing Rules** (`ApprovalRoutingRule`, config posture — no numbering) resolve how many sequential signatures spend needs from three optional dimensions — `core.OrgUnit` department pin, a `commodity` keyword matched case-insensitively against the lines' L28 free-text sku/description, and a HALF-OPEN amount band (floor inclusive, ceiling exclusive, so adjacent bands never double-fire) — ranked most-specific-wins then narrowest band then oldest; NO matching rule means ONE tier, never zero; **Approval History & Audit Trail** is `RequisitionApproval` [RQA-], the append-only signature register keyed `(tenant, requisition, tier)` — safe because a rejected spine status is terminal — whose rows snapshot `tier_count` at decision time so a later rule change never rewrites what an approver thought they were signing, each decision written under `select_for_update` on the spine row with every step audited, and the FINAL tier (or a rejection at any tier) performing the spine's own transition field-for-field; **Delegation of Authority (DOA)** is `ApprovalDelegation`, dated grants (department-scoped or workspace-wide) whose direction is delegate-ward — `active_for(user)` answers "whose authority does this signer hold", and a signature made under a grant is stamped `via_delegation` so history shows both names; **Escalation Management** is `EscalationPolicy` (per-tenant singleton, tenant-unique via migration 0006; a routing rule may override its idle window per queue — where 0 means "escalate immediately") plus an engine whose Run verb takes the policy row lock as a per-tenant mutex and raises idempotent 6.1 `ProcurementAlert`s for chains idle past their window (evaluating ALL pending chains, not just the board's cap); **Mobile Approval Interface** (`approval_mine`) is the pocket triage surface with one-tap decisions. Separation of duties is enforced inside the lock: nobody signs their own requisition, elevated chains are admin-only at every tier, and the final signature always demands a tenant admin — mirrored on-screen by per-row gate badges. Rule/grant writes are tenant-admin gated; queue/history/mine member-readable. Migration 0005 (+0006); `_seed_approval_engine` seeds the catch-all/department/commodity rule ladder, policy + leave-cover DOA grant per tenant. **6.4 vendor management** adds the pieces scm 4.2 SRM never built, mapping its Onboarding/Classification/Risk bullets onto the existing `scm:supplierprofile_list` (+`?tier=`) and `scm:riskassessment_list` pages (L36 — extend, never re-declare): **Vendor Portal** is `VendorPortalAccess` [VPA-] — the staff-managed login↔supplier binding (`portal_user` OneToOne so one login holds one vendor identity; several logins MAY share a supplier company) behind the gated `vendor-portal/` pages whose refusal ladder (no row → refused; NULL supplier → refused; blocked → read-only) mirrors crm 1.4; suppliers see their own POs, their submission history and "our record of you" from `scm.SupplierProfile`, and file **invoice submissions** [VIS-] that procurement reviews submitted→under_review→accepted/rejected under a row lock — acceptance is a REVIEW decision only, nothing posts to any ledger, the bill stays keyed in Accounting AP; **Vendor Blacklisting/Suspension** is `VendorSuspension` [VSU-], a request→decide→lift register around scm's one-click suspended flag (any member files, admin decides with note under lock, lift demands a mandatory reason, decided rows are immutable and undeletable) whose `blocking_for()` honours `ends_on` expiry and blocks every door at once — scm's `purchaseorder_approve`/`send` verbs consult the same register (read-only local import) so a blocked vendor cannot be handed a new or dispatched PO, and the portal refuses invoice AND bid submissions while in force. The portal home also projects **Invoices & payments** read-only from accounting.Bill for the bound party, and hosts 6.5's deferred gated **bid page** (`vendor_portal_bids*`: own bids only, drafts completed via a server-forced form, submission through `SourcingBid.submit()` under the bid+event double lock). Migration 0007; `_seed_vendor_management` seeds access + requested/active/lifted suspensions + accepted/submitted submissions over existing approved suppliers per tenant. **6.5 sourcing & tendering** adds the competitive-STRATEGIC layer above scm's operational RFQ (which keeps line-by-line pricing): **Event Creation & Scheduling** is `SourcingEvent` [SEV-] — tender/RFP/restricted-RFQ with a `draft→open→closed→awarded/cancelled` machine driven only by POST verbs (open/close/cancel admin-gated, each stamping its `*_at` under a row lock), an optional traceability FK to the triggering spine requisition, a nullable budget estimate (None = unknown; analytics never fakes a zero) and supplier-facing rules text, edited as header + inline `EventCriterionFormSet` whose weights must sum ≤100 (below renders as visible coverage); **Bid Submission Portal** is `SourcingBid` [BID-] — whole-package proposals from `core.Party` supplier/vendor-role rows with compliance self-declaration (+required gap note), `draft→submitted→shortlisted/disqualified→won/lost`, submit/shortlist/disqualify all row-locked so a stale decision can never overwrite a fresh award (the staff console captures bids this pass; the gated supplier page is 6.4's portal-access follow-up and `submitted_by` already carries the trail); **Bid Evaluation Matrix** scores live on bid detail — one server-validated input per criterion (`NaN`-proof), blank clears, weighted score = Σ(score/max × weight) computed by ONE implementation (`weighted_total`) shared verbatim by the model method and every batch path; **Award Recommendation** is the computed board over closed events (~4 queries for 20 scenarios via `evaluate_events_batch`) ranking compliant live bids score-desc then price-asc, with a one-click admin award that marks exactly one bid won and the rest lost atomically; **Sourcing Analytics** derives savings-vs-budget (only where both sides exist), participation and cycle time plus six calendar-month activity buckets in two queries. Migration 0009; `_seed_sourcing` drives one awarded tender through the real lifecycle verbs with scored bids, one open RFP taking bids and one cancelled RFQ per tenant. **6.6 RFx Management (RFI, RFP, RFQ)** adds the questionnaire layer: `RfxEvent` [RFX-] (rfi/rfp/rfq, draft→issued→closed/cancelled; issued events freeze so responses stay comparable) carries the **Questionnaire Builder** — typed questions (text/long/number/date/choice) with per-question weights, appended in order by the inline formset and fine-reordered via guarded ▲/▼ swaps; `RfxResponse` [RXR-] (one per event×supplier) is the **Response Collection** repository — answers pre-created one-per-question plus a cover attachment — evaluated through a STATUS_FLOW-gated lifecycle (submit → under review → scored / disqualify+reinstate); the **Side-by-Side Comparison** matrix ranks admissible submissions best-first (`?compare=1` deep-links the register to events with 2+ submissions); the **Scoring & Weighting System** derives everything on read — earned = Σ(score×weight) against 10×Σ(scored weights), computed page-wide via two grouped queries so no list ever N+1s — and drives a tenant-wide leaderboard; `is_template=True` events ARE the **RFx Template Library**, cloned into real drafts (copies, never links) via Use. Suppliers are `core.Party` supplier-role rows; the sourcing link is an optional FK to the scm PR spine (L36). Migration 0008; `_seed_rfx` seeds a library RFI blueprint plus an issued RFP with one fully-scored and one partial response per tenant.. **6.8 contract management** adds the CLM surface around scm 4.2's SupplierContract spine (L36 - never re-declared): `ContractClause` is the pre-approved authoring library selected onto agreements via `ContractClauseLink` (per-contract negotiated overrides), `ContractSigner` carries per-signer bearer tokens (`secrets.token_urlsafe(32)`) gating the PUBLIC sign page `contract-sign/<token>/` exactly like crm 1.9 - GET stamps viewed_at, POST signs/declines under a signer row lock, responses are immutable evidence and completion stays DERIVED; `ContractAmendment` [CAM-] files header-term proposals (blank = term stands) that an admin approve writes onto the spine under the CONTRACT row lock via apply(), reject is terminal, one open amendment per agreement; `ContractMilestone` [CMI-] tracks one-shot deliverables/payment milestones/penalties with complete/waive verbs (distinct from 4.12's recurring ComplianceRequirement register); the **Renewal & Expiration** board is computed over each contract's own notice window (`end_date - renewal_notice_days`) with an idempotent Run raising 6.1 ProcurementAlerts (kind=contract) deep-linked to the spine record. Clause/legal writes are tenant-admin gated; migration 0011; `_seed_contracts` seeds the clause library, one authored agreement with links/signers/milestones and a pending amendment per tenant.. **6.8 contract management** adds the CLM surface around scm 4.2's SupplierContract spine (L36 - never re-declared): `ContractClause` = pre-approved authoring library, selected onto agreements via `ContractClauseLink` with per-contract negotiated overrides; `ContractSigner` carries per-signer bearer tokens gating the PUBLIC sign page `contract-sign/<token>/` exactly like crm 1.9 (GET stamps viewed_at; POST signs/declines under a signer row lock; responses are immutable evidence; completion stays DERIVED); `ContractAmendment` [CAM-] files header-term proposals (blank = term stands) that an admin approve writes onto the spine under the CONTRACT row lock via apply(), reject terminal, one open amendment per agreement; `ContractMilestone` [CMI-] tracks one-shot deliverables/payments/penalties with complete/waive verbs (distinct from 4.12's recurring ComplianceRequirement register); **Renewal & Expiration** board computed per contract notice window (`end_date - renewal_notice_days`) with an idempotent Run raising 6.1 ProcurementAlerts deep-linked to the spine record; legal writes tenant-admin gated; migration 0011; `_seed_contracts` seeds clause library + authored agreement + signers + milestones + a pending amendment per tenant. **6.7 E-Auction Management** adds the reverse-auction engine: \Eauction\ [EAUC-] owns the **Auction Setup & Configuration** bullet (start/reserve price, per-supplier \min_decrement\, anti-snipe extension trio: trigger seconds, extension length, hard cap); \EaucBid\ [EBID-] is the APPEND-ONLY lowering-bid log whose write-time rule engine executes **Bid Extension & Rule Enforcement** - invitee-only, first bid under the start ceiling, own-pace undercut via a Min-aggregate best, strict improvement of the global best, automatic close-extension inside the trigger zone, all under the auction row lock with the cap enforced; "live" is DERIVED from the window so every status keeps exactly one writer; the **Live Bidding Interface** is the live floor + HTMX-polled board + lowering-bid screen (vendor-portal logins are pinned to their bound supplier Party via 6.4's VendorPortalAccess; staff record on behalf of invitees); the **Auction Monitoring Console** shows live rankings/participation/extension counters from ?state=live deep-links; **Post-Auction Results** derives final ranking and savings vs start/reserve from the log and records the award ONCE - leader-only, row-locked, admin read-only afterwards. Suppliers are core.Party roles; sourcing link is an optional scm PR FK (L36). Migration 0010 (e-auction ops only); \_seed_eauction\ seeds an awarded auction with full bid history plus one live auction with an extension already used. **6.9 catalog management** adds the governed BUY-side layer over scm 4.2's simple `SupplierCatalog` price lists (never re-declared, L36): **Catalog Item Creation** is `CatalogItem` [PCI-] — internal lines hard-FK the verified `scm.Item`/`scm.UOM`/`accounting.Currency` masters while supplier-product identity stays free-text per L28, with preferred/active/category flags and a draft→pending_approval→approved/rejected→blocked/archived machine whose verbs stamp who/when exactly once; **Pricing & Tier Management** is `CatalogPriceTier` — effective-dated volume breaks (min_quantity/unit_price/discount_pct) with an optional `scm.SupplierContract` FK for contract pricing and derived `effective_price()`; **Catalog Approval Workflow** gates BOTH new items and price changes — tiers carry their own propose→approve lifecycle so a changed price is just another gated row, the single-occupancy invariant (one ACTIVE tier per break) is enforced in clean() AND re-checked inside approve() under lock, and decision verbs are tenant-admin gated while members keep viewing/proposing rights; **Punch-out Catalog Integration** is `PunchOutEndpoint` [POE-] — per-supplier cXML/SAP-OCI/manual-link configuration with a write-only secret (popped from edit forms, password input, admin-excluded, redacted in audit AND added to core's sensitive-fields frozenset), live handshake deferred behind the CRM-webhooks SSRF-guard precedent with the test verb stamping last_session_at only; **Supplier Catalog Hosting** is `CatalogUploadBatch` [CUB-] — `.csv` intake (2 MB cap, 10k-row refusal) whose validate pass runs under one atomic block with a select_for_update status re-check, stages valid rows as pending_approval items (the approval gate holds even for malicious files), prefix-escapes formula-injection cells and records line-numbered errors. Migration 0013 (+0014 related-name prefixes); `_seed_catalog` seeds an approved line w/ two active tiers, a pending product, a blocked line, two punch-out configs and one validated batch per tenant. **6.11 order fulfillment & tracking** adds the inbound-visibility layer over the scm 4.1 order spine, read-only against it throughout (6.10's `PurchaseOrderChange` stays the only spine mutator and receipts/QC remain 6.12): **Advanced Shipping Notice (ASN)** is `AdvancedShipmentNotice` [ASN-] + `AsnLine` - the supplier's declaration of what is in the box against one `scm.PurchaseOrder`, with a flat packing cube (packages/pallets/weight/volume, deliberately not a recursive pallet-carton tree), BOL/container/freight terms, and lines FK'd to the spine PO line that MIRROR its free text because `core.Item` does not exist (L28) - lot/serial/expiry/origin are the supplier's PRE-ARRIVAL declaration as plain text, the real `scm.LotSerial` row still being created at receipt; **Real-time Freight Tracking** deliberately adds NO tracking table - scm 4.6 TMS already owns inbound freight, so the ASN holds a nullable `scm.Shipment` FK and the computed `inbound_tracking` board READS 4.6's `current_status_text`/`last_known_location`/`eta` projections with the supplier-declared carrier/tracking number as fallback (L36); **Delivery Confirmation** is the `delivery_confirmation` arrivals queue - today/overdue/awaiting/confirmed-7d buckets whose inline confirm posts to the ASN's own `confirm_delivery` verb rather than a second path, every verb re-checking its guard inside the method under a row lock so a double-submit cannot re-stamp `delivered_at` or the POD block; **Split Delivery Management** is `DeliverySchedule` [DSC-] - Coupa's four columns on one row (buyer scheduled quantity/need-by vs supplier promised quantity/date) plus a split console dividing the UNCOMMITTED remainder into K evenly spaced instalments with the last absorbing the rounding, the whole split inside one atomic block holding `select_for_update` over the line's existing rows so two buyers cannot both over-commit (over-commitment past the ordered quantity is a hard block; under-coverage only a derived warning); **Backorder Management** is `Backorder` [BKO-] - the recorded shortfall with a reason, original vs revised promise date and a reschedule counter only the verb may move, escalating idempotently into a 6.1 `ProcurementAlert` whose link is built via `reverse()` because the alert's own `clean()` refuses anything but an internal path, and past_due/at_risk/no_commitment risk buckets that are DERIVED yet filtered as ORM date arithmetic before pagination so the page counts cannot lie. Migration 0016; `_seed_order_fulfillment` seeds an in-flight ASN whose first line is deliberately short against a line that still has an outstanding balance, one ASN taken to delivered through the real confirm verb, a three-instalment ladder and two backorders per tenant. **6.12 goods receipt & inspection** is mostly a MAPPING exercise, and that is the point: six of its ten NavERP bullets were already built elsewhere and are wired to those pages rather than rebuilt (L36) - inventory posting and receipt reversal are scm's `_post_grn_receipt`/`_reverse_grn_receipt`, which already post and compensate real row-locked StockMove legs; QC checklists are inventory's `QcChecklist`/`QcRoutingRule` plus `scm.QualityInspection`, which already carries a `goods_receipt` FK; quarantine is `inventory.QuarantineOrder`; lot capture is `scm.LotSerial`; barcoding is inventory's label/scan/putaway trio. With `apps/quality` still unbuilt and two quality registers already in play, 6.12 deliberately owns only the COMMERCIAL consequence and points at the rest through nullable FKs. The three real gaps: **Receipt Tolerances** is `ReceiptTolerancePolicy` - over/under percentage or absolute quantity, an unlimited-over flag and early/late day bands, scoped by item XOR category with an optional supplier pin, with NO unique constraint because overlapping rules are legal by design and a most-specific-wins resolver (item over category over catch-all, supplier-pinned beating supplier-agnostic) decides instead; judgement is a separate function so selection and verdict are independently testable, quantity breaches outrank date breaches, and where both a percentage and an absolute quantity are set the more restrictive wins; **Discrepancy Reporting** is `ReceiptDiscrepancy` [RDS-] - seven typed kinds with severity, photo and document evidence under the core upload allowlist, a mandatory replace-or-credit remedy, an `editable=False` status moved only by the notify/resolve/cancel verbs (each re-checking its guard inside, so a double-submit changes nothing), and typed escalation pointers to `scm.NonConformance` and `inventory.QuarantineOrder` that its free-text reference could never give; **Return to Vendor (RTV)** is `ReturnToVendor` [RTV-] + lines, genuinely absent before now because `scm.ReturnAuthorization` is the CUSTOMER RMA - draft/authorized/shipped/closed with a derived expected-credit value and an advisory duplicate-RMA badge that never blocks. An RTV posts NO StockMove and NO JournalEntry, and that is correct rather than an omission: `_post_grn_receipt` only ever posted `quantity_received`, so dock-rejected quantity never entered the ledger and there is nothing to remove - the security test lane asserts zero new rows after an authorize. Plus three computed boards: a receiving console that finally closes the ASN->GRN hand-off 6.11 deferred (keyed supplier_reference -> delivery_note_ref, normalized so a case- or padding-differing reference books once instead of minting duplicates), a tolerance-exceptions board and a receipt audit trail. Migrations 0017-0019; `_seed_goods_receipt` seeds the policy ladder, discrepancies across their lifecycle and a return with lines per tenant. **6.13 invoice & voucher management** is the AP lifecycle over the 4.1 order spine: **`SupplierInvoice`** [SIV-] carries an 11-state machine (draft/parked/captured/blocked/disputed/pending_approval/approved/scheduled/paid/void/reversed) whose derived `subtotal`/`tax`/`total` are `editable=False` and owned by `recalc_totals()`, and whose `approve()` posts **exactly one** `accounting.Bill` plus a balanced `JournalEntry` atomically behind a double-submit guard - the only place the sub-module writes to the ledger, which is why every write path consults one `_editable()` gate: mutating a posted header desyncs the GL from the AP subledger undetectably. **Three-Way Matching** is `run_match()` (first-breach-wins, cumulative checks, duplicate scoring, vendor-agreement guard) recording **`InvoiceMatchVariance`** rows across 11 variance types with auto_accept/warn/block outcomes; tolerance bands are workspace constants here because 6.12's `ReceiptTolerancePolicy` already owns the per-vendor/SKU overrides. **`SupplierInvoiceLine`** hangs optional po_line/receipt_line/item/gl_account FKs off the header and signs credit-memo lines **negative**, which is what lets 6.14's spend cube `Sum` them with no special-casing. **`InvoiceDispute`** [DSP-] runs 10 reason codes through a 6-state workflow and tracks `disputed_amount` separately from the invoice total on purpose, so the undisputed balance stays payable while a dispute is open. Plus computed boards for the match cockpit, duplicate suspects (grouped on a normalised `invoice_number_norm` so a case- or padding-differing reference books once instead of minting a duplicate, and never auto-rejecting), the payment schedule bucketed by due horizon, dispute aging, and a dashboard whose `#discount` panel is the Early Payment Discount Tracking bullet. **Naming honesty:** NavERP.md says "Invoice Capture (OCR)" but **no OCR engine is implemented** - the page is **Assisted Capture**, a `pdfplumber` text-layer extraction (lazy optional import; degrades to manual keying when absent) with per-field confidence badges over a pre-filled review form, and the `ocr` SOURCE_CHOICES value records provenance of externally-captured data rather than claiming a capability. Migration 0020 (+0022 indexes); `_seed_invoice_vouchers` seeds 16 invoices across every status including credit/debit memos, a PO-less service invoice and a deliberate duplicate pair, 16 variances, 6 disputes and 5 payment terms, idempotent on (vendor, invoice_number_norm). **This sub-module shipped before its review wave ran, and the review that finally ran found 55 findings including 8 Criticals** - a detail page that 500'd on any invoice with a duplicate candidate (the seeder creates one), a Capture flow that could never save, a line delete with no status guard, and a 1+200-query board. All 55 are fixed and recorded in `.claude/tasks/review-procurement-6.13.md`. **6.14 spend analytics & reporting** is deliberately the INVOICED twin of an existing cube rather than a second one: SCM 4.11 `scm:spend_analytics` already ships the **committed/PO** spend cube, and its own header caveat records why that cube is limited - `scm.PurchaseOrderLine` carries free-text `item_description`/`sku_hint` and **no `Item` FK**, so its category axis is GL-account-based and any per-SKU figure is a best-effort text match. 6.13's `SupplierInvoiceLine` does carry real `item` and `gl_account` FKs and sees the PO-less / service spend 4.11 structurally cannot, so 6.14 aggregates `SupplierInvoiceLine` filtered to `approved`/`scheduled`/`paid` as its default basis, offers committed PO spend as a second selectable basis, and **links to** 4.11 from its dashboard instead of duplicating it. Credit-memo lines are already signed negative so a plain `Sum` nets. Four models across three entity files: **`SpendClassificationRule`** (TenantOwned config master, no number) is the honest non-ML answer to spend classification - a readable, auditable, priority-ordered vendor/GL/keyword/invoice-type/department rule ladder resolving to `scm.ItemCategory` (the only taxonomy in the tree), whose `line_filter(basis)` returns a `Q` **or `None`** where `None` means *can match nothing on this basis* and must never be read as *no filter*; **`MaverickSpendFinding`** [MSF-] is the off-policy register - eight reason codes (no contract, PO-less invoice, no requisition, off-catalogue, non-preferred supplier, price above contract, suspended supplier, split purchase), a five-state disposition workflow whose stamps are all `editable=False` and move only through guarded verbs, `leakage_amount` derived as `max(0, amount - benchmark)`, and a derived `dedupe_key` under `unique_together` that makes `scan()` idempotent so a re-scan of an unchanged window raises nothing and preserves every existing decision; **`SpendReport`** [SPR-] **+ `SpendReportSnapshot`** mirror `crm.AnalyticsReport`/`ReportSnapshot` field-for-field, the snapshot deliberately having no form and no create/edit view because it is minted only by the snapshot POST (recorded in the view docstring so a CRUD-completeness reviewer reads a decision rather than a gap). Five computed pages over a new single-writer `apps/procurement/analytics.py` and no materialized cube: spend dashboard, category spend (Pareto + HHI + price spread), classification workbench, maverick dashboard and CSV export. Three honesty constraints are contractual rather than cosmetic: the **Custom Report Builder is GUIDED, not drag-and-drop** (measure, dimensions, window and Top-N are dropdowns - only one surveyed product actually ships drag-and-drop, and the sidebar comment that denies it guards the label against a future session 'correcting' it to match the aspirational bullet); **Data Export ships CSV/XLSX download only** with no BI/PowerBI connector implemented or implied; and because there is **no FX-rate table anywhere in this repo**, money is summed at face value per currency and a mixed-currency window renders per-currency rows instead of one meaningless total. The department axis is honestly weak - a three-hop nullable `Coalesce(requisition__org_unit, ship_to)` chain that is NULL for every PO-less invoice - so every department breakdown renders an explicit `(unassigned)` bucket rather than dropping rows and making the totals disagree with the KPI strip; likewise the committed basis has no item taxonomy at all, so its category axis resolves through the rules or falls to `(Unclassified)`. 6.14 writes NOTHING to `accounting.*` (L29) - it is a read-only analytics pass. Migration 0021; `_seed_spend_baseline` seeds nine recognised-spend invoices so the cube has something to aggregate and `_seed_spend_analytics` seeds the rule ladder, findings across their lifecycle, saved reports and a snapshot, both idempotent. **6.15 budget & cost management** is built by subtraction under the L29/L36 rule: `accounting.Budget`/`BudgetLine` (2.13) already IS the budget table and 4.1's `budget_check()` already performs the view-time availability check with no stored encumbrance — so 6.15 adds no budget table and no stored balances, only two procurement-owned models and three derived pages. **`BudgetMapping`** is config glue (no numbering — the SpendClassificationRule config posture): a `PROTECT` FK onto `accounting.Budget` plus optional `core.OrgUnit` / `accounting.Project` scope and a default GL account, ranked by lower-wins `priority` with ties broken on id — NO unique constraint on purpose (nullable-column uniques are not portable and priority overrides are legitimate) — with a cross-tenant guard on all four FKs in `clean()` and a most-specific-wins `resolve(tenant, org_unit, project)`. Its module file is the sub-module's single source of the commitment vocabulary: `OPEN_COMMITMENT_PO_STATUSES` copied verbatim from scm 4.18's report, `COMMITTED_PR_STATUSES = ("approved",)` deliberately EXCLUDING converted requisitions (a converted PR already IS its PO — counting both double-counts), and `REQUESTED_PR_STATUSES = ("pending_approval",)`. **`CostForecast`** [FCST-] is a frozen projection under the SpendReportSnapshot exemption — no edit route by design; the three `editable=False` Decimal(18,2) amounts are written once, from `compute_forecast_amounts()` (committed = open PO lines; historical = recognised 6.13 invoices over the horizon window; forecast = committed or run rate or their mean per method), stamped with `created_by` in the hand-rolled create path, and a wrong forecast is deleted and re-frozen rather than amended. The honesty rule is contractual rather than cosmetic: arithmetic projection only, one shared FORECAST_NOTE printed on every page saying it is not a prediction model and nothing learns. Three derived pages compute at view time and lock nothing: the **Budget Availability Check** mirrors `budget_check()` semantics for a chosen budget × department × GL account (committed = open PO lines via `purchase_order__requisition__budget` + approved-not-converted requisition lines; requested = pending lines; remaining; red over-budget badge), explicitly advisory — nothing is reserved; **Commitment Accounting** is a read-only register unioning open PO line totals with approved-not-converted requisition estimates (source/budget/vendor filters, ROW_CAP truncation); and **Variance Analysis** is per-BudgetLine rows — budgeted vs committed vs recognised invoices (actuals basis = 6.13's recognised statuses, not scm landed-cost vouchers) vs remaining and a variance percent (`None` on a zero base, never a fake 0%) with non-overlapping org attribution (company-wide lines absorb only un-attributed spend, with a caveat) and CSV export. Money goes through 6.14's `money()` (never `q2`'s 14,2 ceiling), grouped aggregates call `.order_by()` before GROUP BY, and every computed page prints the currency caveat because no FX-rate table exists in the tree. Migration 0024; `_seed_budget_cost` seeds four mappings (workspace default, department, inactive second-department row, project-pinned) and two forecasts computed through the real function per tenant, idempotently. **6.18 inventory & warehouse integration** is the procurement side of the stock boundary, and what it does NOT own is the design: three tables plus three computed pages with no table behind them, and `apps/procurement` writes **zero** `scm.StockMove` rows anywhere. `ReplenishmentPolicy` is the procurement overlay on `scm.ReorderRule` — preferred vendor (`scm.Item` carries none), order rounding whose cap is applied LAST so it beats the multiple, and the org-unit/budget/GL defaults the generated requisition inherits so 6.15's budget check and 6.3's routing both still run; `include_on_order` defaults True because `ReorderRule.is_below_point()` tests on-hand only and an item already covered by an open PO otherwise re-triggers every run. `ReplenishmentRun` [RPL-] is the **persisted** proposal nothing in the tree had (both existing reorder pages compute and forget): nine grouped queries then pure Python, eleven `editable=False` snapshot columns so a line still explains itself after the stock it measured moved on, per-line accept/snooze/dismiss, and a release that raises one **draft** `scm.PurchaseRequisition` per vendor rather than a PO, so approval routing and the budget check are not bypassed. `MaterialIssue` [MIS-] carries issue AND return-to-stock in one document by `movement_type`, and posting mints a **draft** `scm.StockAdjustment` — the `CountProgram.generate_tasks()` bridge — with per-item demand summed across the document so two lines of one item cannot pass individually and overdraw together. The three derived pages (buyer stock position, received-goods location map, count accuracy) each carry an unsoftened note naming what they cannot tell you: the SKU-text join, the bin trail being a convention over `PutawayTask` rather than a foreign key, and that root-cause attribution is not recorded anywhere. Migration 0027; `_seed_inventory_warehouse` seeds three policies, one **genuinely generated** run (proposing nothing where the workspace is well stocked rather than inventing lines) and two issue documents per tenant, never calling `post()` so a re-seed cannot write into another app's ledger. 143 tests. |
| 7 | Project Management | `projects` | 🟦 7.1–7.14 built — 14 of 19 sub-modules. **7.1 project initiation & charter** is the demand-to-charter front door: `ProjectRequest` [PRQ-] carries the intake **and** the business case (cost/benefit with a risk-adjusted ROI on documented flat discount factors, deliberately not 7.5's Monte Carlo) through the go/no-go verbs, then converts — under a row-level compare-and-swap, so two concurrent approvals cannot mint two projects — into `Project` [PRJ-], whose charter has its own submit/approve gate. `ProjectStakeholder` [PST-] is the RACI register, deriving an engagement strategy from the influence×interest grid; `ProjectKickoff` [PKO-] is the one-per-project ceremony with baseline acknowledgement. The module's spine is an evidence model: once a request is decided, a charter approved or a ceremony attested, the row stops being editable — you cannot forge the signature, so you must not be able to change what it signs. Note `projects.Project` is distinct from the pre-spine `accounting.Project` and `crm.CrmProject` stand-ins,  **7.2 project planning & scheduling** turns a chartered project into a managed plan: `ProjectTask` [TSK-] is the WBS node and the schedulable activity in one row (`deliverable` nodes roll their subtree up on read; `work_package` leaves carry the planned window, effort, estimation method and confidence — the WBS `1.2.3` code and the deliverable rollups are computed in the tree view, never stored); `TaskDependency` [DEP-] is the sequencing network (four link kinds, signed lag/lead, both endpoints pinned to one project), with the critical chain computed on read as the longest dependency walk; `ProjectMilestone` [MST-] pins decision points, phase gates carrying entry/exit criteria and an `achieve` verb that stamps `actual_date` exactly once; `ScheduleBaseline` [BSL-] freezes the plan (snapshot columns written at freeze time) — frozen rows refuse edit/delete, what-if scenarios carry the fast-tracking/crashing experiments, and activate/promote keep exactly one active baseline per project. 7.2 adds no money columns (7.4's) and no actuals (7.8's). **7.3 resource management** turns the plan into staffed, time-logged work: `ResourceProfile` [RSP-] is the pool register — an internal person via `hrm.EmployeeProfile` or an external via `core.Party`, exactly one of the two, carrying `weekly_capacity_hours` (the denominator of every capacity computation) while skills/certifications stay HRM 3.40's register (deep-linked as a lens from the resource detail page); `ResourceAllocation` [RAL-] books a role (+ optionally a named resource — a NULL resource is a *placeholder*, a state of the booking, never a fake person) against a project or an approved-unconverted request for a magnitude in one of three units over a window, with `booking_status` moved only by audited POST-only verbs (assign, substitute — which releases the current row inside a `select_for_update` and mints a successor — commit, complete, cancel) and a placeholder refused at firm until staffed; the computed **Capacity & Demand** board prorates soft/firm bookings per resource-week against capacity (`?weeks=` clamped 4..13) with over-allocation alerts and a `#demand` section whose unfilled placeholders are the hiring trigger; `ResourceTimeEntry` [RTE-] is the thinnest project-facing time log (the payroll-grade weekly timesheet stays in HRM — its entries cannot join `projects.Project`) with a draft→submitted→approved/rejected verb chain whose approval stamps are written exactly once, a per-person-week bulk-approve, and an actuals-to-plan section comparing approved hours against live bookings' `planned_hours`. No money columns anywhere (7.4/7.15's). **7.4 cost & budget management** gives the plan its money layer: `BudgetRevision` [BVR-] is both the budget document and its change control — the **approved-and-activated revision IS the cost baseline** (no `is_active` boolean, no second baseline table; `bvr_activate` supersedes every other approved revision of the project inside one atomic block, and approve deliberately does not activate so two approved rows can coexist while the re-baseline decision is pending), with a submit→approve/reject→activate verb chain whose rejection requires a stated reason and whose evidence stamps (`decision_notes`, `decided_by/_at`) are verb-written and form-excluded; `CostControlAccount` [CCA-] is the earned-value structure — one account per WBS deliverable with a GL lens (PROTECT, never posts) and a separable contingency, computing BAC/EV/PV/AC/committed/available/CV/SV/CPI/SPI/EAC/ETC/TCPI/VAC as guarded derived properties (never stored columns; `cpi` is None with no actuals rather than a fabricated 1.0, and PV is an honestly-labelled planning-grade linear window), with `cached_property` memoization so the register stays at sibling-level query counts; `ProjectBudgetLine` [PBL-] is the row everything rolls up from (seven categories anchored to WBS work packages, `clean()` pinning every anchor to the revision's project) — and the active baseline is frozen through BOTH surfaces: its lines refuse edit/delete exactly like the revision, so change control cannot be bypassed by rewriting what the baseline sums; `ProjectExpense` [PEX-] is the cost evidence (commitments, actuals, accruals with a soft `source_number` reference into the procurement engines — never an FK) where only POSTED rows burn budget, void stays visible but stops counting, amounts are non-negative, and posted/void rows refuse edit/delete because they are what the CPI/EAC forecast stands on. **7.5 risk & issue management** gives the plan its uncertainty layer: `ProjectRisk` [RSK-] is the register — taxonomy, 1–5 probability/impact ordinals whose product bands into low/medium/high/critical, EMV and the residual pair as derived properties (never stored), the realize verb atomically minting the linked issue so the two registers cannot drift, and close/reopen (admin) freezing or unfreezing evidence; `RiskResponseAction` [RRA-] is the committed work executing a strategy (own owner/due/cost, `rra_complete` terminal); `ProjectIssue` [ISS-] is what already happened (severity triage, verb-written resolution evidence, denormalized escalation state feeding the `?escalated=1` queue); `IssueEscalation` [ESC-] records each step of the escalation path (levels 1–4, who was told, why, outcome) with its own register — admin-gated like `iss_escalate` itself, so a member cannot forge the trail; and the two computed boards — analysis (P×I matrix, EMV table, seeded Monte Carlo with clamped, per-field-resolved parameters and an id-ascending draw order that makes every seed reproducible) and monitoring (top risks, exposure burn-down, review queue, tolerance flag, project-scoped lessons lens) — derive everything on read, capped at 2000 register rows. **7.6 quality management** is the plan's conformance layer, and its first ruling was a *negative* one: the enterprise QMS (NCR/CAPA/audit programme/`QualityInspection`) already lives in scm 4.9, so 7.6 is strictly deliverable-scoped and its inspection class is deliberately named `DeliverableInspection` because the other name is taken. `QualityPlan` [QPL-] states what one deliverable must satisfy — required measurable `acceptance_criteria`, a verification method, free-text standard/regulatory references (a standards master is 7.19's) — with an approve (login) → supersede (admin) lifecycle whose stamps are verb-written; `QualityReview` [QRV-] is ONE register covering both the QA half (methodology/compliance/gate reviews) and the continuous-improvement half (kaizen/retrospectives with an owned, dated improvement action and an optional 1–5 maturity score), discriminated only by `review_type`; `DeliverableInspection` [QCI-] puts QC execution and the acceptance decision on one row — `qci_record` stamps the result while keeping the row live (a terminal status written early would let `is_locked` lock the row out of the decision it is owed), then `qci_accept`/`qci_reject` move the SAP-style usage decision, the acceptor stamps (including an external `core.Party` signatory) and the terminal status together, which is the moment the row freezes; `QualityDefect` [QDF-] is the punch list (deliverable-quality categories, scm's severity vocabulary, a punch-list disposition) that LINKS to 7.5's `ProjectIssue` through the atomic `qdf_raise_issue` bridge — never a second issue log — and whose resolved/closed/cancelled rows are frozen evidence. The two computed boards derive everything on read: continuous improvement (a CMMI-banded maturity score computed from the reviews' own scores and the defect closure rate, a six-month opened-vs-dispositioned defect trend as CSS bars, and the lessons lens) and deliverable acceptance (one row per WBS deliverable — its plan, latest inspection, open punch-list count and derived acceptance state — plus the acceptance queue). Migration 0009; `_quality` seeder block; 202 tests across the four lanes. **7.7 scope & requirements management** closes the module's front-to-back loop by owning the REQUIREMENTS BASELINE, and its first ruling is again a boundary: the WBS is 7.2's, the money the creep board costs against is 7.4's, the risks are 7.5's, and 7.6's `qci_accept` is the *deliverable* QC decision — so 7.7's `ScopeVerification` is the **requirement-level** acceptance that closes the traceability loop, a different question that was still unanswered. `Requirement` [REQ-] is what the stakeholder actually asked for: six requirement types, the eight documented elicitation techniques (a closed list so the register can be sliced by how it was captured — interview/workshop/survey/user-story/observation/document-analysis/prototype/brainstorm), **MoSCoW** as the one priority vocabulary, IEEE 1012's four verification methods, a self-FK for the epic/story tree, a `wbs_node` FK to 7.2's work package as the traceability link, a `core.Party` source (never a second stakeholder table), and a documented `version` marker whose real audit trail is the change register. `ScopeItem` [SCI-] is the boundary register and its defining choice is ONE model for all five kinds — in-scope, out-of-scope, assumption, constraint, dependency are all *a claim about the boundary* and the type is a lens, the 7.6 one-object-with-a-kind ruling again; its five states include a **`violated`** one, which the build had omitted from the choices entirely — the single status that means the boundary was breached was unwritable until migration 0010 added it, and the same fix froze it like its terminal siblings because a violated boundary is evidence. `ScopeChangeRequest` [SCR-] is what changed the agreement plus the CCB decision: its own triage priority (deliberately NOT the requirement's MoSCoW — a change is triaged, a requirement is prioritised), three impact dimensions (a Decimal cost, a **positive** schedule impact in days, and a four-level quality impact) that are the creep board's three axes, an approve/reject chain that stamps `decided_at` because the board groups by decision month, and a module-level `HIGH_COST` that `is_high_impact` and the board's `high_impact_count` both read. `ScopeVerification` [SVR-] separates `result` (pass/conditional/fail) from the verb-written `acceptance_status` (pending/accepted/rejected/waived) so the engineering outcome and the business decision are never conflated; a rejection demands a written reason and a waiver an authority note. There is **no stored score column anywhere** — coverage, `coverage_pct`, `bar_pct`, every creep rollup and the whole matrix are computed on read, the module's standing ruling. The review wave found three Criticals and the fixer closed all 20 actionable findings, including a correctness bug the review itself missed (a second wrongly-signed seed literal). One is worth naming because it is subtle: eight admin-gated verbs stacked `@tenant_admin_required` ABOVE `@require_POST`, and since decorators apply bottom-up that answered a **member's GET with 403 instead of the house 405** — the role check ran before the method check, a bug invisible to any admin-actor test; the security lane now asserts both actors get 405. **Two items are recorded for the product owner rather than silently decided:** whether `sci_realize`, `sci_retire` and `svr_accept` should be admin-gated (they are login-only today, matching the `qpl_approve` precedent), and the same 403-vs-405 decorator ordering that exists in 7.1–7.5 and was deliberately not swept in this run. Migrations 0007 + 0010; `_scope` seeder block; 543 tests across the four lanes. **7.8 task & work management** is the EXECUTION layer, and its defining ruling is that it extends `ProjectTask` IN PLACE rather than declaring a second task table: the six execution fields (assignee, priority, MoSCoW, the Eisenhower pair, percent_complete) and the two verb-written stamps (actual_start/actual_end) are new columns on 7.2's row, so sub-tasks keep working through its existing self-FK and no `WorkTask`/`TaskExecution` table exists. Two new registers sit on top: `TaskChecklistItem` [TCL-], where the POST-only `tcl_check` toggle is the ONLY writer of `is_done`/`done_by`/`done_at` (one verb per direction, clearing the stamps on the way back), and `TaskBlock` [TBK-], the manual-blocker evidence row whose `reason` and `unblock_criteria` are both required because a block with no stated cause or no exit is not evidence — minted only by `tsk_block` and closed exactly once by `tsk_unblock`, with the admin refusing add and delete for the same reason. **Blocked is DERIVED, never stored**: `is_dependency_blocked` walks the 7.2 link network and `is_manually_blocked` reads the open row, and `is_blocked` is their OR. `moscow` is deliberately NULLABLE — null is the board's Unclassified bucket, so the nullability is load-bearing rather than sloppy. Three computed pages derive everything on read (no snapshot table): the board buckets live rows into the four status columns with a ready queue, the priority page groups by MoSCoW and buckets the Eisenhower quadrants and both bucket **live work only**, and the Gantt resolves its window from the union of planned dates and draws a today line only when today actually falls inside it. `tsk_block` refuses terminal work and re-tests that inside its row lock, and the bulk verb mirrors the rule so a block can never outlive the work it was holding; the same bulk verb is capped at 500 ids with a truthful truncation message, resolves every id through one tenant-scoped fetch, and loads the old assignee only in the audit branch that needs it. `TaskExecutionForm` guards attested history — a cancelled row refuses execution writes and a done row keeps its verb-attested 100% — and it is the only execution write surface, which keeps the six fields off 7.2's frozen `TaskForm`. The review found 2 Criticals and 10 Importants; the fix pass closed all 17 with 16 Minors, and the headline performance win was measurable rather than theoretical: the task-detail page's three 7.8 panels were reading the model's re-querying properties instead of the prefetch cache, so calling `.filter()` and two COUNTs bypassed it — moving the figures into the view took the page from **31 to 13 queries**. Migration 0011; `_taskwork` seeder block; 137 tests across the four lanes. **7.9 collaboration & communication** is the human-coordination layer that hangs off the existing spine rather than beside it: seven tables in one sub-module — `Channel` [CHN-] (three kinds, including an archived state the `chn_archive` verb flips), `ChannelMessage` [CHM-] (self-FK threading, `@`-mentions as an M2M), `DocumentShare` [DSH-] (who may do what with an **already-stored** document, with `dsh_{revoke,claim,release}` writing a single access row), `Meeting` [MTG-] with its `MeetingAgendaItem` [AGI-] and `MeetingActionItem` [MAIT-] children, and per-recipient `ProjectNotification` [NTF-] — plus the merged **`activity_feed`** computed page, which has no table of its own. The prefixes are chosen to be collision-proof rather than obvious: `CHM` because `MSG` is `scm.IntegrationMessage`'s, `MAIT` because `MAI` is `hrm.MeetingActionItem`'s, and the model tests pin both. `ChannelMessage.clean()` enforces three invariants, and the one that mattered is **I1**: a root that already carries replies cannot be repointed to another channel, because the replies' own channel FK is not rewritten and the orphaned thread would then render on *neither* channel — the guard is tested both ways, so a reply-less root may still move and a reply's body stays editable. The same review found **I8**, the mirror image of 7.8's member bug: a project member could clear or delete a *teammate's* row through the ordinary edit/delete routes, so every write surface was re-scoped to the actor (L27). Seven entities, 41 routes and 41 templates ship, with the repo's first **three-level template nesting** (`collaboration/meeting/agendaitem/form.html`) — the child forms live under their parent's folder because they have no register of their own. Migration 0012 lands the tables and 0013 adds the four `(tenant, -created_at)` indexes the review flagged; the `_collab` seeder block runs under its own guard after `_taskwork` and creates **3 channels / 18 messages / 4 shares / 4 meetings / 8 agenda items / 7 action items / 18 notifications** per tenant, deliberately crossing the register's 15-row page so the paginator has a genuine page 2, while creating at most two `core.Document` rows — the file repository is **7.10's**, not this sub-module's. **Four Criticals** surfaced in Step-3 verification rather than in a review lane: a Django template filter written inside a Python f-string made every meeting refusal path 500 (C-A), `msg_edit` called `save_m2m()` after a committing `save()` and raised (C-B), all three new registers paginated an unordered annotated queryset — a warning class that was clean before 7.9 (C-C) — and the seeder's 13 messages fell short of the contract's page-2 minimum (C-D). The six serial reviewers filed no new Criticals, 8 Importants and 21 Minors; the fixer closed them across 21 commits, and the security lane proved the I8 tests genuinely detect the hole by reintroducing it as a negative control. 382 tests across the four lanes. **7.10 document & knowledge management** is the record layer, and its first ruling is again what it does NOT reuse: `core.Document` stays untouched — its `version` is a flat CharField with no chain and its GenericForeignKey cannot be tenant-filtered, joined or faceted on a list page — so 7.10 declares its own `ProjectDocument` [PDM-] with **real link FKs** to `ProjectMilestone`/`ProjectTask` (never a GFK), exactly as procurement 6.19 ruled before it. Five models: `ProjectFolder` [PFD-] is the per-project tree (parent self-FK, with the materialized path and the per-folder document count computed in the tree view and never stored; `ProjectDocument.folder` is PROTECT, so a folder that still holds documents or children cannot be deleted), `ProjectDocument`, `ProjectDocumentRevision` [PDV-] — the immutable chain whose invariants are copied verbatim from 6.19: `revision_no` allocated inside a `transaction.atomic()` behind a `select_for_update()` on the parent, an upload NEVER moving the pointer, approve refusing `revision_no <= current_revision_no` and re-checking that refusal INSIDE the lock, `current_revision_no` an integer pointer (0 = none) rather than a circular FK, `current_revision` filtering `is_approved=True` so a re-allocated number can never put an unapproved file on the record, immutability STRUCTURAL (no edit url, view or template exists — everything but `change_note` is `editable=False`), and a check-out lock that is cooperative and verb-written by `pdm_checkout`/`pdm_checkin` alone — `DocumentTemplate` [DTM-], the standards library that is deliberately **tenant-wide with no project FK** because a standard belongs to the PMO and not to one project, and `KnowledgeEntry` [KNE-], the insight library whose `usage_count` is an atomic `F()+1` **click counter** (it does not know who pressed it) and whose `is_featured` is a **shelf, never a permission**, and which carries **no FileField at all** — the artifact is picked from the repository, so a playbook keeps one checksum, one text read and one history. Three computed pages hold no table: the repository overview, the retention board (where `retain_until` is derived from `created_at + 30 * retention_months` so nothing goes stale at midnight, and the reminder Run raises idempotent in-app rows whose dedupe key is the reminder **title** — because 7.9's inbox has no link column — as a button, not a cron, since the repo has no scheduler and no mail worker), and the knowledge search. Search is a denormalized copy with a stated caveat: `extracted_text` has exactly two writers (the approve verb and the re-index Run), the `icontains` sweep joins only from four characters, extraction is a lazy `pdfplumber` import bounded by a page cap AND a character cap, and every page says a scan has no text layer. Migration 0014; `_docmgt` seeder block; 22 templates. **Finishing this pass mattered more than building it.** The backend had been committed by an interrupted session whose templates, migration and seeder were never written — 21 of 22 templates absent, `projects.0014` unapplied so `nav_erp.projects_projectfolder` did not exist, no seeder rows, and the sidebar advertising 7.10 as **Live** over pages that would 500. Recovering it surfaced four defects in the already-committed code, and each was fixed rather than templated around: `pdm_list` rendered a filter bar that **filtered nothing** but `q` (the view now hands the whole `filters` spec to `crud_list`, which is where the L11 guards live), `kne_search` raised `NameError` on **every non-empty `?q=`** because `Q` was never imported, `ProjectDocumentForm` left `project` editable on an instance whose `folder`/`milestone`/`task` querysets were narrowed from the OLD project — so changing it could never validate, which is exactly what the form's own docstring claimed was impossible (the field is now disabled on edit), and the run's smoke sweep is what caught the first three. One **pre-existing 7.9 defect** was found while smoking the module overview and fixed: `_cc_activityfeed` was imported into `apps/projects/urls/__init__.py` but never concatenated into `urlpatterns`, so `projects:activity_feed` raised `NoReverseMatch` and took `/projects/` down with it — three templates reverse that name, and 7.9's own `test_collab_security.py` was **red on `main`** because of it (verified by reverting the fix: 1 failure before, 0 after). **Then the close-out ran**: six serial reviewers produced **53 deduped findings — 5 Critical, 22 Important, 26 Minor** — and `code-fixer` pass 1 closed all five Criticals and all 22 Importants (26 fixed, 1 skipped, 0 refuted) across 71 commits, with every Critical re-verified by an independent probe. The five are worth naming because four are silent: the upload verb read the raw `UploadedFile` instead of the saved `FieldFile`, so every UI upload stored an empty search copy **and `pdv_approve` then overwrote the parent's copy with it** (measured 103 chars → 0); `ProjectFolder._is_descendant_of()` seeded its `seen` set with `self.pk`, making the cycle guard **structurally unable to return `True`** — a folder cycle was creatable through the ordinary edit form and the branch then silently vanished from the tree; four `onsubmit="return confirm('… {{ name }} …')"` handlers interpolated a member-authored folder name into a single-quoted JS literal, and because an inline handler's attribute is character-reference-decoded before the browser compiles it, HTML escaping does not protect it — **`x',alert(document.cookie),'y` executes** (lesson L42's third recurrence); `.svg` was on 7.10's upload allow-list, a fork of the house list that added the one scriptable extension `core.forms.ALLOWED_DOC_EXTENSIONS` omits, while `MEDIA_ROOT` is served unauthenticated with `Content-Disposition: inline`; and `doc_retention_run` wrote a 23-character action into `AuditLog.action` (`varchar(10)`), silently truncated on this non-strict checkout and a `DataError`/500 on a strict-mode server. **Two items are carried rather than folded into a 7.10 fix** — the anonymous `/media/` handler (`config/urls.py`, the enabling half of the SVG chain, and one authenticated media view would fix 14 modules at once) and the repo-wide over-length audit-action sweep (**25** other sites, overwhelmingly procurement 6.19's). **One real gap remains: 7.10 has no pytest module of its own** — the `docmgt_*` conftest block and the four `test_docmgt_*.py` lanes are the next pass, so 7.10's gate today is `temp/smoke_710.py` (**169 checks, 0 failures**) plus the four existing lanes continuing to pass. The full `apps/projects` suite is **3270 passed, 0 failed, 2 skipped**. **7.11 time & attendance tracking** bridges operational project delivery with labor accounting and utilization intelligence, implementing all five NavERP 7.11 capability bullets: (1) *Timesheet Entry & Submission* extends `ResourceTimeEntry` [RTE-] with `is_billable` (BooleanField) and `activity_code` (CharField) tied to `TimeActivityCode` [TAC-] (6 standard overhead categories: direct project, client consulting, internal overhead, general admin, training, R&D); (2) *Approval Workflows* adds a frozen re-logging loop (`rte_relog`) creating a corrected draft without mutating audited rejected evidence; (3) *Billable vs. Non-Billable Hours* implements chargeability analytics, overhead distribution, and client billing splits; (4) *Overtime & Leave Integration* provides `OvertimeRule` [OTR-] (daily, weekly, weekend, holiday threshold multipliers) and `ProjectOvertimeRecord` [POT-] (draft→submitted→approved/rejected maker-checker workflow, pay and billable equivalent calculations) synchronizing on a unified `time_calendar` grid with `hrm.LeaveRequest` and `hrm.PublicHoliday`; (5) *Time Reporting & Utilization* delivers a full KPI utilization dashboard (`utilization_dashboard`) with individual resource rates vs targets, client billing splits, category allocations, and capacity vs demand variance. Migrations 0017 and 0018; idempotent `_time_attendance` seeder block; 11 templates; 53 unit tests. **7.12 portfolio & program management** delivers strategic multi-project governance and capital allocation across all five NavERP 7.12 capability bullets: (1) *Portfolio Dashboard & Heat Maps* computes multi-project health indicators, investment distribution, and a 4-quadrant strategic alignment heat map matrix on `pfm_dashboard`; (2) *Program Dependency Mapping* manages cross-project dependencies via `ProgramDependency` [PDEP-] with lead/lag days, criticality, and `pdep_clear`/`pdep_reopen` lifecycle verbs; (3) *Strategic Alignment & Scoring* provides `PortfolioInvestment` [PIN-] evaluating projects against a 4-criterion weighted model (strategic fit, financial return, delivery risk, capacity fit) with derived composite `weighted_score`; (4) *Capacity & Pipeline Planning* visualizes intake pipeline progression over 7.1 `ProjectRequest`s and allocation demand; (5) *Portfolio Reporting & Governance* provides top-level investment envelopes via `Portfolio` [PRT-], coordinated delivery clusters via `Program` [PGM-], and executive decision verbs (`pin_fund`, `pin_reject`, `pin_defer`). Migration 0019; idempotent `_portfolio_management` seeder block; 13 templates; 43 unit tests across models, forms, views, and security. **7.13 agile & scrum management** delivers timeboxed iterative delivery and agile tracking across all five NavERP 7.13 capability bullets: (1) *Sprint Planning & Backlog Grooming* on `sprint_backlog` workbench with epic/points assignment and project filtering; (2) *Sprint Execution & Daily Standups* on `sprint_execution` active sprint workbench with day-by-day burndown chart (ideal vs actual points), standup notes logger, and open blockers panel; (3) *Release & Version Planning* via `ProjectRelease` [REL-] with version tags, feature flags, and `rel_publish` verb; (4) *Epic & Feature Management* via `ProjectEpic` [EPC-] with color codes and derived progress rollups; (5) *Retrospectives & Team Health* via `SprintRetrospective` [RET-] with sentiment score (1.0–5.0) and action items, plus `SprintImpediment` [IMP-] blocker tracking and team velocity analysis on `velocity_report`. Migration 0020; idempotent `_agile_scrum` seeder block; 19 templates; 55 unit tests. **7.14 client & external collaboration** delivers stakeholder transparency, contract governance, and client billing integration across all five NavERP 7.14 capability bullets: (1) *Client Portal & Visibility* manages external client access tokens, permission toggles (progress, milestones, deliverables, financials, feedback), and expiration checks via `ClientPortalAccess` [CPA-]; (2) *Client Feedback & Approvals* captures deliverable reviews, annotations, and formal client sign-off/rejection maker-checker workflows via `ClientApprovalRequest` [CFB-] (`cfb_approve`/`cfb_reject`); (3) *Contract & SOW Management* authors master service agreements and statements of work via `StatementOfWork` [SOW-] (`sow_activate`), tracking contract value adjustments across an approved change order chain via `SOWAmendment` [SWA-]; (4) *External Vendor Coordination* tracks third-party/subcontractor task handoffs, deliverable links, acceptance workflows, and 1–5 vendor scorecard ratings via `VendorHandoff` [VHD-] (`vhd_accept`/`vhd_reject`); (5) *Billing & Invoicing to Clients* maintains project delivery billing schedules (fixed fee, T&M, milestone, retainer) via `ProjectClientInvoice` [PCI-] with 1-click atomic customer invoice and line-item creation directly into the canonical `accounting.Invoice` AR ledger (`pci_generate_invoice`). Migration 0021; idempotent `_client_collaboration` seeder block; 16 templates; 37 unit tests across models, forms, views, and security. |
