...
10. **`NavERP.md`** is the master module catalog (modules 0–23) for planning; **`NavERP-ERD.md`** is the unified core data model (the `Party` + two-ledger spine every module reuses).
...
- **Module 0 — System Admin & Security** is the cross-cutting foundation: build it first as the apps `core` (Tenant, TenantMiddleware, navigation `MODULE_CATALOG` 0–23, AuditLog, decorators, the unified-core masters + ledgers), `accounts` (User/Role/Permission/UserInvite + email-or-username auth + IAM/RBAC), `tenants` (subscription/billing/branding/encryption keys/health), and `dashboard` (KPI aggregation). Modules 1–23 are domain apps built on top via the `/next-module` skill.
...
After Module 0, build modules 1–23 (CRM, Accounting & Finance, HRM, SCM, Inventory, Procurement, Project Management, Sales, eCommerce, BI, Asset Management, Quality, Document Management, Manufacturing Execution (MES), Product Lifecycle (PLM), Maintenance & Reliability (CMMS/EAM), Field Service (FSM), IT Service Management (ITSM), Retail & POS, Facilities & Workplace, Treasury & Financial Operations, Sustainability/EHS/ESG, AI & Intelligent Automation) one at a time with the `/next-module` skill — each as a Django app under `apps/<slug>` reusing the unified core.
