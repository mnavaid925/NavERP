  ...
  LANES.map(L => () =>
    agent(
      [
        ...
        '  - Do NOT run git.',
        '',
        'NAME COLLISIONS ARE THE FAILURE MODE OF THIS FAN-OUT (L47). pytest imports all four files into one session,',
        'so a duplicate module-level name silently shadows a sibling lane\'s and breaks tests OUTSIDE your file —',
        'where your own scoped run cannot see it. Therefore:',
...
const green = await agent(
  [
    ...
    '     Never use -k to scope this run. A -k filter cannot detect a name collision, because the damage lands',
    '     outside the filter — that is exactly the failure this phase exists to catch (L47).',
    '  2. Fix every failure:',
    '     - A collision between the four new files, or with an existing sibling suite -> rename in the NEW file',
    '       (prefix _' + subslug + '_ / test_' + subslug + '_), never by editing the older suite.',
