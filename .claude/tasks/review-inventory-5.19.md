...
### Critical
- [ ] **C1** `management/commands/seed_inventory.py` — contract §7 `_seed_integrations` hook absent
...
### Important
- [ ] **I1** views StockSyncRuns.py:131 — `stocksyncrun_retry` has NO status guard: any run
...
- [ ] **I2** models ChannelListingMaps.py:93 — `external_variant_id` CharField(blank=True, NOT NULL)
...
- [ ] **I3** models StockSyncRuns.py:171-174 + _choices.py:115 — as-built indexes `attempt_no - 1`
...
- [ ] **I4** views IntegrationChannels.py:100-105 + channel/detail.html:149,164 — listings panel
  ...
  design).
- [ ] **I5** channel/detail.html:145-217 — zero links to any `listingmap_*` route anywhere in the
...
- [ ] **I6** apiclient/detail.html:19 — Edit link rendered unconditionally though edit is
  @tenant_admin_required → dead-end for staff. Wrap in existing `{% if is_admin %}`.
- [ ] **I7** views ApiClients.py:118-135 — issue_token mints fresh credentials for REVOKED clients
  (revocation-bypass primitive once a gateway honors hashes). Gate: refuse unless status=="active".
- [ ] **I8** apps/inventory/forms/__init__.py — three 5.19 forms imported but missing from
  `__all__`; star-imports omit them. Append the three names.

### Minor
- [ ] **M1** views StockSyncRuns.py:86 — echoes raw `?channel=` param; mirror sibling's validated echo.
- [ ] **M2** migration — runs default landing orders `-started_at` with no `(tenant, started_at)`
  leading index (file-sort over unbounded register). Add index alongside I2's migration.
- [ ] **M3** views IntegrationChannels.py:200-203 — `_run_stats` two COUNT round-trips; one
  grouped aggregate matches house style.
- [ ] **M4** views IntegrationChannels.py:110 — `_run_stats` extra_context evaluates before crud_detail
  404s; make lazy/callable if trivially possible, else skip with reason.
- [ ] **M5** apiclient/detail.html:83+ — bare `<dt>/<dd>` without `<dl>` ancestor; wrap groups.
- [ ] **M6** apiclient/detail.html:127 — Issue/Rotate Token POST lacks confirm() (rotate invalidates
  previous token silently); add like rotate-key elsewhere.
- [ ] **M7** apiclient/form.html:23 + listingmap/form.html:28 — raw `<ul>` non_field_errors vs
  channel/form.html styled loop; unify on the styled pattern.
- [ ] **M8** ChannelListingMap `?sync_enabled=` lens unindexed — boolean cardinality + usually
...
- [ ] **M11** views IntegrationChannels.py:101-109 — panels re-keyed `filter(channel_id=pk)` instead
