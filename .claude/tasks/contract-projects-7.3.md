...
| `resourceallocation/detail.html` | Page actions (each a POST form with `onsubmit="return confirm(...)"` + `{% csrf_token %}`): **Assign** (admin-gated + status gate `{% if obj.booking_status == 'requested' or obj.booking_status == 'soft' %}` with nested `{% if not obj.resource_id %}` — Django `{% if %}` has no parentheses, see divergence 8; renders `<select name="resource">` over `resources`) · **Substitute** (admin-gated + status gate `{% if obj.booking_status == 'soft' or obj.booking_status == 'firm' %}` with nested `{% if obj.resource_id %}`, also `<select name="resource">`) · **Commit** (`{% if obj.booking_status == 'requested' or obj.booking_status == 'soft' %}`) · **Complete** (`soft`/`firm`) · **Cancel** (`requested`/`soft`/`firm`). detail-grid: Number, Status badge, Project link or Request link, Task link (if set), Resource or Placeholder badge, Role, Skills required, Unit + magnitude, Window, Substitute of (link if set), Substituted by (`{{ successor.number }}` link if set), Requested by (`obj.requested_by.username` if set), Notes. |
...
12. **Assign/Substitute gates are TWO nested `{% if %}` tags, not one compound line** (as-built
    amendment, review 7.3 I2). Django `{% if %}` has no parentheses, so the originally pinned
    one-liners (`not obj.resource_id and obj.booking_status == 'requested' or … == 'soft'`)
    parsed as `(not resource_id and requested) or soft` — Assign rendered on named soft
    bookings and Substitute on firm placeholders, both always refused by their views. The
    status gate is the OUTER tag and the resource test the INNER one, matching the views'
    refusal order. The Template section above carries the corrected strings.
