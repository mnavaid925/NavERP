...
| `total_kinds` | `int` — **always the full kind count**, never the narrowed length | `len(_library_rows(request))` | the "N standard reports" caption — the honest answer to "50+", which is `16 kinds × 4 scope axes` |
| `sibling_boards` | as `rbi_home` | same constant | the "engine you are not reusing" panel |
| `type_filter` | `str` — **non-empty only when it matched at least one row** | `wanted if matched else ""` | the `{% if type_filter %}` banner. The list arrives already narrowed; the template filters nothing (D65) |

`?type=` here is a **display filter only**, and the narrow lives in the view: an unknown key leaves the full list (never an empty page, L11), and the banner is then blank, because a banner naming a filter that did not happen is the same lie in the other direction (D65).

#### `report_standard` — `render(request, "projects/reporting/standard.html", ctx)` (the page that renders all 16 kinds)

GET params read: `type`, `range` (→ `date_range`, only when the value is a `PRESET_RANGES` key), `project`, `portfolio`, `client`, `org_unit`, `from`, `to`, `as_of`.
...
| `range_choices` | `list[tuple]` | `analytics.PRESET_RANGES` (`RANGE_CHOICES` minus `custom`) | the window `<select name="range">` **inside the same GET form** as the four scope picks, not a row of standalone links (D66). Its active option is the one value the template reads straight from `request.GET.range` — the window is not echoed into context, and `RANGE_KEYS` already ignores anything else |
...
| `runs` | queryset ≤ 5 | **`analytics.visible_runs(request).filter(report=report).exclude(pk=obj.pk)[:5]`** (D56 — this row's literal `ProjectReportRun.objects.filter(tenant=request.tenant, …)` was the one fetch in the sub-module that bypassed the ACL helper) | the "earlier runs of this question" strip + the trend link |
...
`obj.save()`; audit `create`; → `redirect("projects:pdb_detail", pk=dashboard.pk)`. The cap of B3.1's
`MAX_TILES_PER_DASHBOARD` is refused **before the form is built** (D61), so the append above is what a
valid POST does on a board that still has room.
...
reachable only through the tie lookup D62 describes).
...
* Form pages: `<form method="post">{% csrf_token %}` + `{% include "partials/form_field.html" %}` per bound
  ...
  **`request.get_full_path` in practice, so a `?range=` or a filter query survives the round trip (D64)**.
...
| **D41** | A2.1: `ProjectReportForm.clean_measures()` "coerces the selected list back into the JSON list and enforces `1 <= len <= 3`" | **no `clean_measures` exists.** `measures` is a declared `MultipleChoiceField`, whose `clean()` already returns a `list`, and `ProjectReport.clean()` raises the field-keyed `"Pick between 1 and 3 measures."` | the A2.4 rule-home ledger itself assigns "measures 1–3 + known keys" to **`ProjectReport.clean()`**. A form copy would raise the same sentence twice on the same field, and `test_reporting_forms.py`'s assertion is about `form.instance.measures` **being a list** — which the field type guarantees, not a hand-written coercion |
| **D42** | A2.1/A2.4: a `Meta.widgets` entry for every choice, date and number field (`form-select`, `form-input`, `type: date`, `step: 0.01`, `min: 1`) | the dicts carry **only what Django does not supply**: the two textarea `rows`, `top_n`'s `max`, and `measures`' `CheckboxSelectMultiple(attrs={"class": "form-check"})`. Verified against the built forms: every Select/Textarea/Checkbox class comes from `TenantModelForm`, `DateInput` reports `input_type` and **drops** a declared `type`, `IntegerField` adds `min` itself (as `min_value - step`, i.e. 0), `DecimalField(decimal_places=2)` adds `step="0.01"` itself | a declared date widget is not merely redundant, it is **discarded** — `TenantModelForm.__init__` reassigns `field.widget` for every `DateField`, so writing one in `Meta.widgets` claims control the form does not have. 7.15's files spell out all of it; 7.16 states only the deltas, so the next reader can tell which attribute came from where |
| **D43** | the A2.4 ledger's two tenancy homes (model `clean()` = invariant, form `_reject_foreign` = field error) | **both fire on a crafted cross-tenant POST**, so that field can carry two messages ("That project belongs to another workspace." + "That record belongs to another workspace.") | deliberate, and the reason each home exists is different: the model's check is what stops a **non-form writer** (the seeder, `rep_freeze`) and is skipped when `tenant_id` is unset; `_reject_foreign` compares against `form.tenant` and so still fires when the instance was never stamped. Dropping either would open a real hole; the doubled sentence is on a hostile-input path only |
| **D44** | A2.3/A2.4: `default_range` / `date_range` are `forms.Select` fed by the model's `RANGE_CHOICES` | both forms override the choices with **`analytics.PRESET_RANGES`** (6 rows, no `custom`) in `__init__` | B3.1 pins PRESET_RANGES as "the only window list a dashboard/tile page may offer", and A1.2 rule 4 / A1.3 rule 4 make `custom` un-saveable on both models — so the full `RANGE_CHOICES` dropdown was one selectable option that could only ever come back as a validation error. `ProjectReport.date_range` keeps all 7 rows: that model *does* carry a date pair |
| **D45** | B3.3: every `STANDARD_REPORTS` row carries `"computer": _s_cost_variance` | **no `computer` key.** A row's `axes` dict is the whole compute spec, and `standard_report()` hands it to the same `_compute()` that serves a saved `ProjectReport` | a per-kind callable is a second arithmetic per report — exactly how two surfaces end up disagreeing about one number. `standard_report()` also keeps the row's metadata (`label`/`description`/`sections`/`drill_url_name`) out of the spec, which the earlier flat row leaked into the engine |
| **D46** | B3.3's example row: two `sections` | **exactly one `sections` row per kind**, with all eight areas used across the 15 kinds | B4 pins each `_standard_section_<area>.html` as reading the *same* flat `columns`/`rows`/`chart_*` from the parent context with "no `with` hand-off, no arguments" — a second section would print the identical table under a second heading. One row keeps the area switch meaningful and the page single-pass |
| **D47** | B3.1 `SIBLING_BOARDS`: the 7.6 row is `projects:qrv_report` | **`projects:quality_improvement`** | `qrv_report` is `quality-reviews/<int:pk>/report/` — a per-object page. `rbi_home` has no pk to hand it, `_resolve_url` swallows the `NoReverseMatch`, and the panel quietly shows 11 of 12. Every row in the constant now resolves bare |
| **D48** | B3.4's function list | **one addition**: `canned_axes()` → `{kind: {"subject", "measures", "dimension_1", "chart_type"}}` | B2.2's `rep_create` key `canned_axes` reads `STANDARD_REPORTS[k]["axes"]`, and the builder's step 1 needs `subject` with it. A function keeps the axes vocabulary owned by analytics instead of re-spelling the row shape in a view |
| **D49** | B2.1: `report_standard`'s junk fallback is the view's own `FALLBACK_KIND = "status_report"` | **`analytics.CANNED_FALLBACK`**, which `standard_report()` uses for the same purpose | two literals for one rule is how the view ends up showing a different report than the engine computed. The view now reads the constant it falls back through |
| **D50** | `views/_common.py`'s docstring: "Every entity module does `from … import *`" | `ReportingHome.py` uses **explicit named imports** (`login_required, render, timezone`) | the precedent B1.0 names for this module is 7.15's `FinancialBoards.py`, which does exactly this, as do the other four 7.15 computed-page modules. The four CRUD-bearing 7.16 modules pull the `crud_*` helpers and use the star import — both forms are in the app, and the one matching the module's job wins |
| **D51** | B4.6/B5.0: "`_csv_response(filename, columns, rows)` added to `views/_helpers.py`" | shipped as the public **`csv_response`** | B4.6's own name is private, but its two consumers are two different entity modules, and D38's ruling is that a `_`-prefixed name imported across a module boundary is not an API. Procurement's twin (`_csv_response` in `BudgetCostManagement/VarianceReport.py`) stays private because it has exactly one |
| **D52** | B2.1 gives `ReportingHome` its own `_chart_config`/`_chart_rows` and owns `SINGLE_CHART_ID` + `SECTION_AREAS`; B2.2 says `rep_detail` unpacks "the same twelve keys" | **all four live in `views/_helpers.py`** (`chart_config`, `chart_rows`, `SINGLE_CHART_ID`, `SECTION_AREAS`), imported by both pages | `standard.html`, `report/detail.html` and a run's replay include the SAME `_result_chart.html` and `_standard_section_<area>.html`. Two byte-identical builders plus a re-spelled `"0"` is precisely the drift B4.7 warns about, and Backend rule 5 puts a helper read by more than one module in `_helpers.py`. A view→view import was available (7.3's `HRReports` does it) and rejected: it would have left the duplication in place |
| **D53** | B2.2: `rag_css` from a literal three-entry map in the view | reads **`analytics.RAG_CSS`**, which is `_RAG_CSS` made public for exactly this | the map already existed twice (analytics' heat bands, `ProjectReportRun.rag_css`); a third spelling in a view is how a badge class stops matching the letter the compute chose. The model keeps its own copy — a model must not import analytics (B3.0), the same accepted price as `summary_pairs` |
| **D54** | B2.2 `rep_create` passes plain `ProjectReportForm`; A2.4 keeps `owner` off the field list; nobody said who writes it | `ProjectReportForm` takes **`user=`** (the view injects it with `partial`, as 6.x's `TradeLicenses` injects `may_edit_controls`) and stamps `instance.owner` in `save()` **only when unset** | `crud_create` builds and saves the form without a user, so a private save landed `owner=None` and `visible_reports` hid the row from the person who had just created it — a lost save, not an empty register. The stamp is form behaviour so it lives in the forms layer, not in a subclass inside a view module (Backend rule 2: a file owns its layer's code); the "only when unset" guard is what stops somebody else's Edit transferring ownership. Asserted both ways: create stamps, second user's edit does not move it |
...
| **D61** | B3.1's constant row says "`wdg_create` refuses beyond it with a message" without saying **where**; B2.5's `wdg_create` paragraph reads as though every POST reaches `form.is_valid()` | The refusal is evaluated **before the form is constructed**, on `dashboard.widgets.filter(tenant=…).count() >= MAX_TILES_PER_DASHBOARD`, and answers `messages.error` + a redirect to the board | Two consequences a builder must not discover by accident. (a) An invalid POST to a *full* board shows the cap message, not its own field errors — correct, because no edit to the tile can make room, and re-rendering a form that cannot save is the dead-button shape B4.7 warns about. (b) The count is on the board's own tiles re-narrowed by `tenant`, which is the same read `_widget_grid` slices, so the cap the view enforces and the cap the page can display cannot drift apart. Proven both ways: 24 tiles → refused, nothing written, nothing audited; delete slot 12 → the next POST is accepted **at 25**, because `max + 1` counts the high-water mark and not the gap (the gap is display-order, and `Meta.ordering` closes it) |
| **D62** | B2.5's `wdg_move`: "renumber fallback … when the neighbour's position equals the moved tile's", with the neighbour fetched by a strict `<`/`>` comparator | The literal pair is unreachable — a row found through `position__lt`/`position__gt` can never have an equal `position`. Implemented as: the pinned strict lookup first; if it returns nothing, a second lookup for a row **at the same `position`, ordered either side by `id`**; equality then holds by construction and takes `renumber_tiles` | A tie is exactly the case the strict comparator cannot see, so taken literally the fallback is dead code and a corrupt board stays corrupt. The tie partner is also the row the reader sees adjacent to this one, because `Meta.ordering` is `["position","id"]` (verified `apps/projects/models/ReportingBusinessIntelligence/DashboardWidgets.py:75`) — so this is the one neighbour worth looking for. Swapping it would write the position value it already holds: a no-op that reports success, which is why the branch repairs instead. Verified: two tiles at `1`, `up` on the second → the board comes back `1,2` with a "move the tile again" message and one `toggle` audit row, and the *same* request again is now a real swap |
| **D63** | R10 pins `redirect_back_or(request, name, **kwargs)` as the shared "come back where you came from" helper for the 11 POST verbs; its one gate is `url_has_allowed_host_and_scheme` | A second, cheaper gate in front of it: **`looks_like_target(candidate)`** — an absolute same-host URL or a root-relative path, else the pinned fallback | `url_has_allowed_host_and_scheme` answers `True` for **any** relative reference (verified `venv/lib/site-packages/django/utils/http.py:301`, the `not url_info.netloc` branch — refusing foreign hosts is its job in `LoginView`, not proving the value is a location). So `?next=home` passed the gate and reached `redirect()`, which tries it as a **view name** and re-raises `NoReverseMatch` for a string containing neither `/` nor `.` ("If this doesn't 'feel' like a URL, re-raise" — verified `django/shortcuts.py:177-186`). Net effect: a hand-typed junk `next` was a **500 on a verb whose write had already committed** — the tile had moved, the run had been archived, and the operator saw an error page instead of the board. `HttpResponseRedirect(candidate)` was the other candidate fix and is rejected: it drops the view-name lookup but turns junk into a *silent* redirect to the wrong page (a browser resolves `Location: home` against the current directory, so `…/widgets/7/move/` would land on `…/widgets/home`), which is worse than loud and defeats the purpose of the fallback. This is a robustness fix, not an openness fix: `//evil.example`, `http://evil.example/…`, `javascript:` and a control-char-prefixed host were already refused and are re-asserted as still refused |
...
`resolve_window`, `compute_widget`, `compute_report`, `standard_report`, `CANNED_FALLBACK`, `canned_axes`,
`RAG_CSS`,
`exec_pack`, `rag_streak`, `rag_history`, `narrative_seed`, `summary_pairs`, `home_dashboard`,
`renumber_tiles`, `json_safe`) and the shared view layer
(`csv_safe`, `redirect_back_or`, `csv_response`, `chart_config`, `chart_rows`, `SINGLE_CHART_ID`,
`SECTION_AREAS`).
