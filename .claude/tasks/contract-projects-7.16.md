...
new names it introduces are analytics symbols (`PRESET_RANGES`, `SIBLING_BOARDS`, `UNASSIGNED`,
`MAX_REGISTER_ROWS`, `MAX_EXPORT_ROWS`, `MAX_TILES_PER_DASHBOARD`, `summary_pairs`, `chart_rules`,
`home_dashboard`, `renumber_tiles`, `resolve_window`) and two view helpers (`csv_safe`, `redirect_back_or`,
`_csv_response`).

**Part B's promise back to Part A (three things A must not break):**
1. `ProjectReportRun.summary` is a **dict** and `.data` carries exactly the other eight payload keys — if A
   changes either shape, B2.3's property list and `rep_freeze`'s write both move with it.
