...
- **Status:** [x] fixed — perf(scm): join carrier__party on the landed-cost charge freight-invoice dropdown
...
- **Status:** [x] fixed — fix(scm): keep a deactivated GL account, tax code or ex-carrier party on the landed-cost charge edit form
...
- **Status:** [x] fixed — feat(scm): default a customs charge rate from DutyTariff.rate_for() in LandedCostChargeForm.clean()
...
- **Status:** [x] fixed — fix(scm): clamp Item.average_cost at both ends of its column width (the Notes' `apply_receipt()` sibling clamped in the same commit, as the note asked)
...
- **Status:** [x] fixed — refactor(scm): promote _keep_current into the shared forms toolkit, then re-pointed DutyTariffs.py and LandedCostVouchers.py at it (3 commits). The 4.13/4.14 `(queryset, tenant, current_id)` variant and 4.17's two others are a different signature with 6 call sites in other sub-modules — deferred to the app-wide pass recorded under Notes (L18)
...
- **Status:** [x] fixed — fix(scm): stop the customs-only duty rate blocking every other landed-cost charge type
...
- **Status:** [x] fixed — **applied 2026-08-17 in the migration window it was deferred to.** `models.Index(fields=["tenant", "party"], name="scm_lcv_tnt_party_idx")` added to `LandedCostVoucher.Meta.indexes` (`984d527a`) and migration **`0032_landedcostvoucher_scm_lcv_tnt_party_idx`** (`08d0a3c3`) — a single `AddIndex`, no field changes, no destructive operations. Applied to `nav_erp`; `makemigrations --check` reports no drift and `manage.py check` is clean. Note for the record: this was the **only** index outstanding — the "app-wide sweep" this was deferred into turned out to be a single index, because 4.17's `(tenant, owner_client)` finding was **not** deferred with it; that pair landed on both `Item` and `Location` in migration `0030` at the time
...
- **Status:** [x] fixed — fix(scm): let a purely-recoverable or purely-expensed voucher reach Accounts Payable, plus fix(scm): show Draft the bill on a voucher that can never be allocated (the detail-page gate had to mirror the relaxed model guard or the fix was unreachable from the UI)
...
```
- **Status:** [x] fixed — fix(scm): clamp the two derived allocation columns to their own widths
...
- **Status:** [x] fixed — fix(scm): clear accrued_at when re-allocating demotes an accrued voucher
...
- **Status:** [x] fixed — perf(scm): drop the duplicate row fetch in dutytariff_detail, plus refactor(scm): read obj.is_current directly on the duty tariff detail page
...
- **Status:** [x] fixed — perf(scm): fold the landed cost voucher list money aggregate into the counts
...
- **Status:** [x] fixed — fix(scm): emit None for an unallocatable per-unit uplift in _allocation_groups
...
- **Status:** [x] fixed — refactor(scm): resolve the allocation row Basis used label through get_basis_used_display
...
- **Status:** [x] fixed — fix(scm): stop landed cost rows asserting a three-way match they never had

## Notes — app-wide / pre-existing (NOT in the fix queue)

### Added by code-fixer while burning down 4.18

- **App-wide pass wanted: `_keep_current` has FIVE implementations under THREE signatures** (M1 named
  three; the grep finds five). The canonical one now lives at `apps/scm/forms/_common.py` with the
  `(form, name, base_queryset)` signature and 4.18's two call sites use it. Still private and still
  able to drift: `forms/AssetManagement/Assets.py:77` `(queryset, tenant, current_id)` — returns a
  queryset and hard-codes `core.Party`, imported by `AssetManagement/MaintenancePlans.py`,
  `AssetManagement/MaintenanceWorkOrders.py` and `LaborManagement/LaborSessions.py` (6 call sites);
  `forms/ThirdPartyLogistics/LogisticsClients.py:51` `(queryset, current_id)` — returns a queryset
  rebuilt off `model._default_manager`; `forms/ThirdPartyLogistics/ClientRateCards.py:83` — BYTE
  IDENTICAL to the promoted one and a two-line delete-and-import away;
  `forms/ThirdPartyLogistics/ClientSlas.py:171` — the same rule again as a bound method. Consolidating
  4.13/4.14/4.17 onto the shared helper is a cross-sub-module sweep with a signature change at every
  call site, so it was deliberately not forked out of step here (L18). `ClientRateCards.py` is the
  free one — identical body, no signature reconciliation needed.
- **Deferred to the next migration window: M3's `(tenant, party)` index on `LandedCostVoucher`.** The
  finding is real (a declared list filter plus `_party_qs`'s reverse join, both on every list page
  load) but it is an `AddIndex`, i.e. migration **0032** — and 0031 is 4.18's claimed number in a
  checkout shared with a concurrent session. The whole change is one line in
  `LandedCostVoucher.Meta.indexes`; the index name `scm_lcv_tnt_party_idx` is reserved for it. Worth
  sweeping the same family in that pass — 4.17's `scm_cbr_tnt_cli_end_idx` and 4.10's
  `scm_rma_tnt_cust_idx` are the reference pattern this one is missing from.
- **Not raised as a new finding, observed while fixing M4:** the detail page's `can_draft_bill` gate
  had to be widened alongside the model guard, because every gate on that page mirrors exactly what
  its verb refuses. Any future change to a `LandedCostVoucher` verb's preconditions has to move both
  halves or the button and the model disagree — the model change alone would have been unreachable
  from the UI and would have looked like a no-op fix.
