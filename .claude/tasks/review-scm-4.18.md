...
- **Status:** [~] skipped — needs a schema change: the AddIndex requires migration 0032, and 0031 is this sub-module's claimed number in a checkout shared with another session. Recorded under Notes for the next migration window; the model change is one line and the index name `scm_lcv_tnt_party_idx` is reserved for it
