...
- [ ] **C1** — `forms/FulfillmentOrchestration/FulfillmentWaves.py` — POSTing the same sales
...
- [ ] **C2** — `views/FulfillmentOrchestration/FulfillmentWaves.py:136-151` — wave_release /
...
- [ ] **I1** — list page N+1 (~45 queries/page at 15 rows): template renders `w.orders.count` +
...
- [ ] **I2** — seed_inventory --flush deletes 5.10 tables but not FulfillmentWave/FulfillmentWaveOrder;
...
- [ ] **M1** — `FulfillmentWaveOrder.clean()` guards sales_order tenant but not wave tenant (non-form
      writers could pair cross-tenant wave). Fix: id-based wave.tenant check keyed "wave".
- [ ] **M2** — `templates/inventory/fulfillment/wave/detail.html:98` renders `m.added_at`; field is
      `created_at` — Added-At column silently blank. Fix binding (+ date filter like siblings).
- [ ] **M3** — list rows render Edit affordance regardless of status; gate `{% if w.is_editable %}`
      matching detail-page behavior (server already refuses).
- [ ] **M4** — board filter bar lacks the `q` input the view supports; add text input mirroring list.
- [ ] **M5** — detail evaluates `obj.pick_progress_pct` up to 3x (3 identical aggregates); hoist once
      into view context (`pick_pct`) and bind template to it.
- [ ] **M6** — seeder `_seed_fulfillment_waves` uses `is_staff=True` lookup only; a tenant with no
