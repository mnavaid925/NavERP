...
Planned: 2026-09-24. Phase 2 is planning only; this edit is intentionally uncommitted and does not add application code.
App: `sales` (new-app first sub-module). Capture `BASE` at the start of the build; do not use the current dirty checkout as a review range.
Migration: `sales.0001_initial`, generated only after all four models are re-exported and the app is installed.
Package: `apps/sales/{models,forms,views,urls}/LeadManagement/` plus the flat app-level `admin.py`, `services.py`, and `management/commands/seed_sales.py`.
Templates: `templates/sales/leadmanagement/<entity>/` and the standalone `templates/sales/overview.html`.
Test subslug: `leadmanagement`.

## Scope and ownership — the 8.1 contract

- [ ] Build exactly four Sales-owned domain models: `LeadScoreEvent`, `LeadQualification`, `LeadRoutingRule`, and `LeadNurtureEnrollment`. Do not create `sales.Lead`, `sales.Opportunity`, `sales.Account`, `sales.Contact`, `sales.LandingPage`, `sales.FormSubmission`, `sales.Campaign`, `sales.EmailCampaign`, `sales.Territory`, `sales.CrmTask`, or a second conversion writer.
- [ ] Treat the existing CRM spine as canonical: `crm.Lead` (`apps/crm/models/CoreData/Leads.py`), `crm.Opportunity` and `source_lead` (`SalesForceAutomation/Opportunities.py`), CRM `lead_convert` (`views/CoreData/Leads.py`), `crm.LandingPage`/`crm.FormSubmission`, `crm.Campaign`/`crm.CampaignMember`/`crm.EmailCampaign`, and `crm.Territory` (`SalesForceAutomation/Territories.py`). Sales reads and links them by string FK; it does not re-declare them.
- [ ] Treat `crm.WorkflowRule` as the generic bounded workflow, `core.NotificationRule`/`NotificationPreference` as a future delivery hook, and `core.OrgUnit`/`core.Activity` as reusable platform context; 8.1 neither duplicates their engines nor claims notification dispatch.
- [ ] Reuse `accounts.User` for owners, `crm.CrmTask` for follow-up, `core.AuditLog` for explainability, `core.ConsentPurpose` for the nurture purpose reference, and global `accounting.Currency` for `budget_currency`; verify each with the model registry/grep before coding (L28/L36).
- [ ] Sales owns the four operational records and only controlled projections onto `crm.Lead.score`, `crm.Lead.rating`, `crm.Lead.status`, and `crm.Lead.owner`. There is no hidden `post_save` signal, second score writer, or second assignment engine.
- [ ] Remove `score` and `rating` from the ordinary `crm.LeadForm` and make them read-only in the CRM lead admin; manual changes must be reasoned `LeadScoreEvent` actions. Keep CRM as the owner of basic lead lifecycle/conversion; document every cross-app projection in `core.AuditLog`.
- [ ] Preferred conversion integration is a CRM-owned `apps/crm/services.py` service (for example `convert_lead`) that preserves the current atomic `crm:lead_convert` behavior; both the existing CRM view and a thin Sales handoff call it. If the service extraction is not done, expose a readiness link to `crm:lead_convert` and do not implement a Sales copy of the Party/Contact/Opportunity writes.
- [ ] The five NavERP bullets remain visible, but capture and conversion are links/operations over CRM-owned records rather than additional Sales tables.

## Models — exactly four

### 1. `LeadScoreEvent` — append-only score ledger

- [ ] File `apps/sales/models/LeadManagement/LeadScoreEvents.py`; inherit a local `TenantEventOwned` base (tenant FK plus immutable `created_at`; no mutable `updated_at`); never expose an edit path. No `number`, ordinary `ModelForm`, edit route, or delete route.
- [ ] Fields: `tenant` (`core.Tenant`, required/indexed); `lead` (`crm.Lead`, `CASCADE`, required); `signal_category`; `event_type`; `score_delta` (`SmallIntegerField`, signed and bounded `-100..100`, never directly form-editable); `source_kind`; `source_ref` (opaque `CharField(255)`, never a GFK or dereferenced relation); `reason` (`TextField`); `effective_until` (nullable `DateTimeField`); `idempotency_key` (nullable `CharField(120)`); `corrects_event` (nullable self-FK, `SET_NULL`); `occurred_at` (`DateTimeField`, default `timezone.now`); `recorded_by` (`accounts.User`, nullable `SET_NULL`, null only for system ingestion); `created_at` (immutable receipt time).
- [ ] Pin `SIGNAL_CATEGORY_CHOICES` exactly: `behavioral`, `demographic`, `qualification`, `manual`, `decay`, `correction`.
- [ ] Pin `EVENT_TYPE_CHOICES` exactly: `form_submitted`, `email_open`, `email_click`, `web_visit`, `content_download`, `event_attendance`, `meeting_booked`, `demo_request`, `call_connected`, `reply_received`, `fit_match`, `fit_mismatch`, `unsubscribe`, `manual_adjustment`, `decay`, `correction`.
- [ ] Pin `SOURCE_KIND_CHOICES` exactly: `form_submission`, `campaign_member`, `communication_log`, `qualification`, `web_tracking`, `api`, `manual`.
- [ ] Add a tenant-scoped unique constraint on `(tenant, idempotency_key)` while allowing multiple null keys; indexes on `(tenant, lead, -occurred_at)`, `(tenant, event_type, -occurred_at)`, and `(tenant, source_kind, source_ref)`. Reject a correction that points to itself, another tenant, or another lead.
- [ ] A normal event may have a system-calculated delta; a manual or correction event requires a non-empty reason. A correction is a new inverse-delta row, never an edit/delete of the original.
- [ ] Implement one scoring service path: lock the tenant lead, reject a duplicate idempotency key, append the event, sum unexpired deltas, clamp only the projection to `0..100`, and update `crm.Lead.score` plus `rating` atomically with audit. Fixed bands are cold `0..39`, warm `40..69`, hot `70..100`; no AI claim.

### 2. `LeadQualification` — current BANT/MEDDIC assessment

- [ ] File `apps/sales/models/LeadManagement/LeadQualifications.py`; inherit `TenantOwned` and use a tenant-scoped `OneToOneField` to `crm.Lead` (one current assessment per lead).
- [ ] Fields: `tenant`; `lead`; `framework`; `status`; `country_code` (normalized two-letter free field); `region`; `city`; `industry`; `employee_count` (nullable positive integer); `seniority`; `budget_status`; `budget_amount` (`DecimalField(18,2)`, nullable); `budget_currency` (nullable FK `accounting.Currency`, global reference); `authority_level`; `need_summary`; `expected_purchase_on` (nullable date); `economic_buyer`; `decision_criteria`; `decision_process`; `technical_requirements`; `pain_points`; `success_metrics`; `disqualification_reason`; `assessed_by` (nullable `accounts.User`); `assessed_at` (system-set nullable timestamp); `next_review_on` (nullable date); `notes`; `created_at`; `updated_at`.
- [ ] Pin `FRAMEWORK_CHOICES` exactly: `bant`, `meddic`, `both`. Pin `STATUS_CHOICES` exactly: `unassessed`, `partially_qualified`, `qualified`, `disqualified`, `archived`.
- [ ] Pin `SENIORITY_CHOICES` exactly: `unknown`, `individual_contributor`, `manager`, `director`, `executive`, `owner`.
- [ ] Pin `BUDGET_STATUS_CHOICES` exactly: `unknown`, `not_confirmed`, `confirmed`, `adequate`, `insufficient`. Pin `AUTHORITY_LEVEL_CHOICES` exactly: `unknown`, `influencer`, `user`, `manager`, `director`, `executive`, `owner`.
- [ ] `qualified` requires evidence for the selected framework: need and expected purchase date for BANT/MEDDIC; MEDDIC (and `both`) additionally requires economic buyer, decision criteria, and decision process. BANT also requires a known budget status and authority level. `disqualified` requires a reason. A terminal decision stamps the acting user/time; archived rows cannot be selected for routing.
- [ ] Qualification changes append compensating score events for changed fit signals and may project `qualified`/`unqualified` to `crm.Lead.status`; they never rewrite score history. A meaningful assessment is archived rather than destructively deleted; delete is allowed only for an unassessed, unconverted lead.
- [ ] Keep geography/firmographics here only as plain assessment facts; do not create a duplicate Party, Address, Account, or Contact master.

### 3. `LeadRoutingRule` — deterministic owner resolver configuration

- [ ] File `apps/sales/models/LeadManagement/LeadRoutingRules.py`; inherit `TenantOwned`, not a numbered document. `name` is unique per tenant. This is configuration, so no `LR-` number (that prefix is already used by HRM leave requests).
- [ ] Fields: `tenant`; `name`; `description`; `is_active` (default true); `priority` (positive integer, lower first); `match_mode`; `conditions` (bounded structured JSON list); `is_catch_all` (explicit boolean for the empty-condition catch-all case, default false); `assignment_mode`; `default_owner`; `territory`; `eligible_owners` (tenant-scoped M2M to `accounts.User`); `fallback_owner`; `max_open_leads` (nullable positive integer); `cursor` (non-negative `editable=False`); `last_assigned_owner` (`editable=False`); `last_assigned_at` (`editable=False`); `created_at`; `updated_at`.
- [ ] Pin `MATCH_MODE_CHOICES` exactly: `all`, `any`. Pin `ASSIGNMENT_MODE_CHOICES` exactly: `fixed_owner`, `territory_manager`, `round_robin`.
- [ ] Conditions are objects with only `field`, `operator`, and bounded scalar `value`. The field allowlist is exactly: CRM lead `source`, `status`, `rating`, `score`, `est_value`, `owner_id`, `company`, `title`, `email_present`, `phone_present`; qualification `qualification_status`, `framework`, `country_code`, `region`, `city`, `industry`, `employee_count`, `seniority`, `budget_status`, `authority_level`, `expected_purchase_on`.
- [ ] The operator allowlist is exactly `eq`, `ne`, `in`, `not_in`, `contains`, `icontains`, `gt`, `gte`, `lt`, `lte`, `is_set`, `is_empty`. Validate field/operator/value shape and maximum condition count/size; unknown input fails closed. Never use `eval`, dynamic imports, model traversal, or relation dereference. `conditions=[]` is legal only with `is_catch_all=True` and a visible warning.
- [ ] Require `default_owner` for `fixed_owner`, `territory` for `territory_manager`, and a non-empty same-tenant `eligible_owners` set for `round_robin`; optional `fallback_owner` and `max_open_leads` do not become a capacity scheduler.
- [ ] Resolution loads active rules by `(priority, id)`, takes the first match, and reports `unrouted` rather than inventing an owner. Fixed mode uses the default owner, territory mode uses `crm.Territory.manager`, and round robin locks the rule/lead, orders owners by PK, rotates from the cursor, skips an owner over `max_open_leads`, and updates cursor/last owner in the same transaction. If everyone is over the cap, use a valid explicit fallback or leave the lead unassigned.
- [ ] A run writes only `crm.Lead.owner`, one reusable `crm.CrmTask`, and `core.AuditLog` in one atomic service. Because `crm.CrmTask` has no lead FK, put the lead number in a bounded subject/description and retain the exact relation in audit evidence rather than adding a Sales task table or GFK. Preview is read-only; real run is POST-only and idempotent by `(lead, rule, current owner)`. No network call is made while locks are held.

### 4. `LeadNurtureEnrollment` — lifecycle in an existing CRM drip campaign

- [ ] File `apps/sales/models/LeadManagement/LeadNurtureEnrollments.py`; inherit `TenantNumbered`, set `NUMBER_PREFIX = "LNE"`, and keep `number` per tenant as `LNE-#####`.
- [ ] Fields: `tenant`; `lead` (`crm.Lead`, `CASCADE`); `email_campaign` (`crm.EmailCampaign`, `PROTECT`); `number`; `status`; `trigger_kind`; `score_at_enrollment` (immutable 0..100 snapshot set on activation); `consent_purpose` (`core.ConsentPurpose`, nullable until activation); `consent_evidence` (`CharField(255)`); `owner` (`accounts.User`, nullable); `started_at`; `last_touch_at`; `next_touch_at`; `touch_count` (non-negative, system-managed); `completed_at`; `exit_reason`; `notes`; `created_at`; `updated_at`.
- [ ] Pin `STATUS_CHOICES` exactly: `pending`, `active`, `paused`, `completed`, `cancelled`, `replied`, `converted`. Pin `TRIGGER_KIND_CHOICES` exactly: `manual`, `score_threshold`, `form_source`, `qualification`, `recycled`.
- [ ] Pin `EXIT_REASON_CHOICES` exactly: `qualified`, `disqualified`, `replied`, `converted`, `unsubscribed`, `bounced`, `cancelled`, `completed`, `manual`. Require a reason for cancelled, replied, converted, and disqualified exits.
- [ ] Enforce unique `(tenant, lead, email_campaign)`; add indexes on `(tenant, status, next_touch_at)` and `(tenant, email_campaign, status)`. Activation requires a same-tenant `crm.EmailCampaign` with `send_type='drip'`, consent-purpose/evidence for an optional purpose, and a non-converted lead. A converted lead cannot have an active enrollment.
- [ ] Surface the current Party-only `core.ConsentRecord` gap in the UI, seed output, and Sales skill: the purpose/evidence reference is recorded honestly, but it is not a substitute for a verified legal grant for a pre-conversion lead and does not authorize sending.
- [ ] Use explicit lifecycle verbs (`activate`, `pause`, `resume`, `complete`, `cancel`, `reply`, `convert_exit`); after activation do not permit arbitrary status/edit/delete overwrites. `next_touch_at` is a recorded target only, never a promise that a worker will send. No Sales campaign, template, body, sender, ESP, or per-recipient engagement table is created.

## Package and backend contract

- [ ] Before implementation, freeze this plan's field/choice/form/URL/context contract in `.claude/tasks/contract-sales-8.1.md`; the contract must include every view context key and FK filter queryset, not only the list variable (L7/L48).
- [ ] Create the new app scaffold: `apps/sales/{__init__.py,apps.py}`, `models/{__init__.py,_base.py,LeadManagement/{__init__.py,LeadScoreEvents.py,LeadQualifications.py,LeadRoutingRules.py,LeadNurtureEnrollments.py}}`, and matching `forms/`, `views/`, and `urls/` packages with the same entity files plus `Overview.py` for the non-model operations board. Follow the CRM-style absolute imports, local tenant base, and re-export blocks; every model/form/view must be re-exported from its package `__init__.py`.
- [ ] Add flat app-level `admin.py`, `services.py`, and `management/{__init__.py,commands/{__init__.py,seed_sales.py}}`; do not add an `analytics.py` or a second generic workflow engine in 8.1.
- [ ] Build the four entities serially in this order: `LeadScoreEvent`, `LeadQualification`, `LeadRoutingRule`, `LeadNurtureEnrollment`; finish each model’s four backend files and templates before the next, and defer package re-exports, admin, seeder, navigation, settings, and URLs to the single-writer integration step (L12/L50).
- [ ] Forms are `TenantModelForm` subclasses from `apps.core.forms`, with an app-local `_reject_foreign`/tenant-validation helper equivalent to the CRM/projects pattern. `accounting.Currency` is global and must not be compared to a tenant. All tenant-owned FK/M2M querysets are scoped to `request.tenant`, including crafted POST values.
- [ ] `LeadScoreAdjustmentForm(forms.Form)`: `lead`, signed `score_delta` bounded `-100..100`, required `reason`. `LeadScoreCorrectionForm(forms.Form)`: `lead`, tenant/lead-scoped `corrects_event`, signed `score_delta`, required `reason`. These are action forms, not a `LeadScoreEventForm`.
- [ ] `LeadQualificationForm`: `lead`, `framework`, `country_code`, `region`, `city`, `industry`, `employee_count`, `seniority`, `budget_status`, `budget_amount`, `budget_currency`, `authority_level`, `need_summary`, `expected_purchase_on`, `economic_buyer`, `decision_criteria`, `decision_process`, `technical_requirements`, `pain_points`, `success_metrics`, `next_review_on`, and `notes`; `lead` is required on create and disabled on edit. Exclude `tenant`, `status`, `disqualification_reason`, `assessed_by`, `assessed_at`, `created_at`, and `updated_at`. `LeadQualificationDecisionForm` accepts only `partially_qualified`, `qualified`, `disqualified`, or `archived`, plus a conditional disqualification reason and optional notes; the view stamps assessor/time.
- [ ] `LeadRoutingRuleForm`: `name`, `description`, `is_active`, `priority`, `match_mode`, `conditions`, `is_catch_all`, `assignment_mode`, `default_owner`, `territory`, `eligible_owners`, `fallback_owner`, and `max_open_leads`; exclude `tenant`, `cursor`, `last_assigned_owner`, `last_assigned_at`, and timestamps. `LeadRoutingPreviewForm` and `LeadRoutingRunForm` contain a tenant-scoped `lead`; preview never writes.
- [ ] `LeadNurtureEnrollmentForm`: `lead`, drip-only `email_campaign`, `trigger_kind`, `consent_purpose`, `consent_evidence`, `owner`, and `notes`; exclude `tenant`, `number`, `status`, `score_at_enrollment`, `started_at`, `last_touch_at`, `touch_count`, `completed_at`, `exit_reason`, and timestamps. `LeadNurtureActivationForm` may accept the staff-recorded `next_touch_at`; lifecycle/exit forms accept only the relevant transition and exit reason. No ordinary form exposes a system timestamp (L22).

### View functions and pinned context keys

- [ ] `LeadManagement/Overview.py`: `lead_overview` (tenant-scoped `stats`, `leads`, `qualifications`, `routing_rules`, `enrollments`, `latest_score_events`, `duplicate_warnings`, `recent_activity`) and `lead_handoff` (POST-only; requires a current same-tenant qualified `LeadQualification`, calls the CRM conversion service, then exits active nurture rows, creates `crm.CrmTask`, and audits; never copies CRM conversion writes).
- [ ] Every view filters its base queryset and related reads by `tenant=request.tenant`; ordinary list/detail pages are member-readable, while routing-rule configuration writes, manual score adjustment/correction, qualification archive/delete after use, and nurture activation/reactivation are `@tenant_admin_required`. Approved rule runs and routine qualification decisions use the narrower module policy and write audit evidence.
- [ ] Keep list output PII-minimal (lead number/name/company only where needed), truncate long reasons/notes consistently, never put full note bodies or consent evidence into logs, and encode action verbs in `AuditLog.changes` while using only the existing `create`/`update`/`delete` action values.
- [ ] `LeadScoreEvents.py`: `lead_score_event_list`, `lead_score_event_detail`, `lead_score_event_adjust`, `lead_score_event_correct`, `lead_score_event_recompute`. List context is exactly `object_list`, `page_obj`, `q`, `date_from`, `date_to`, `signal_category_choices`, `event_type_choices`, `source_kind_choices`, `leads`; detail context is `obj`, `lead`; adjustment context is `form`, `leads`; correction context is `obj`, `form`, `correctable_events`. There is no create/edit/delete view.
- [ ] `LeadQualifications.py`: `lead_qualification_list`, `lead_qualification_create`, `lead_qualification_detail`, `lead_qualification_edit`, `lead_qualification_delete`, `lead_qualification_partial`, `lead_qualification_qualify`, `lead_qualification_disqualify`, `lead_qualification_recalculate`, `lead_qualification_route_preview`. List context is `object_list`, `page_obj`, `q`, `status_choices`, `framework_choices`, `seniority_choices`, `budget_status_choices`, `authority_level_choices`, `assessors`, `review_due`; create/edit context is `form`, `is_edit`, `obj` where applicable, and `leads`; detail context is `obj`, `lead`, `score_events`, `enrollments`, `routing_preview`; action views pass `obj`, `form`, and the same relevant FK querysets.
- [ ] `LeadRoutingRules.py`: `lead_routing_rule_list`, `lead_routing_rule_create`, `lead_routing_rule_detail`, `lead_routing_rule_edit`, `lead_routing_rule_delete`, `lead_routing_rule_toggle`, `lead_routing_rule_preview`, `lead_routing_rule_run`. List context is `object_list`, `page_obj`, `q`, `assignment_mode_choices`, `match_mode_choices`, `territories`, `active`; form context is `form`, `obj`, `is_edit`, `territories`, `users`; detail/preview context is `obj`, `eligible_owners`, `leads`, `preview_form`, and `preview_result` where applicable.
- [ ] `LeadNurtureEnrollments.py`: `lead_nurture_enrollment_list`, `lead_nurture_enrollment_create`, `lead_nurture_enrollment_detail`, `lead_nurture_enrollment_edit`, `lead_nurture_enrollment_delete`, `lead_nurture_enrollment_activate`, `lead_nurture_enrollment_pause`, `lead_nurture_enrollment_resume`, `lead_nurture_enrollment_complete`, `lead_nurture_enrollment_cancel`, `lead_nurture_enrollment_reply`, `lead_nurture_enrollment_convert_exit`. List context is `object_list`, `page_obj`, `q`, `status_choices`, `trigger_kind_choices`, `drip_campaigns`, `owners`, `next_touch`; form context is `form`, `obj`, `is_edit`, `leads`, `drip_campaigns`, `consent_purposes`, `owners`; detail/action context is `obj`, `lead`, `email_campaign`, `consent_purpose`, and `form` where applicable.
- [ ] Pin the complete list filter contracts: score events — `signal_category`, `event_type`, `source_kind`, `lead`, `date_from`, `date_to`; qualifications — `status`, `framework`, `country`, `region`, `seniority`, `budget_status`, `assessor`, `review_due`; routing rules — `active`, `assignment_mode`, `territory`, `priority`; nurture enrollments — `status`, `trigger_kind`, `email_campaign`, `owner`, `next_touch`. Every choice list and FK queryset consumed by these controls is passed by its view.
- [ ] Every list searches with `request.GET.get('q', '').strip()` before pagination, passes every choice list and FK queryset consumed by its filter bar, preserves valid filter values, and applies filters before `Paginator`. Reuse `crud_list` where its contract fits; custom date/due filters must validate malformed values and use `timezone.localdate()` for due comparisons. Guard integer FK parameters with the project `as_db_int`/`crud_list` rule (L11/L44/L54).

### URL names and literal-first order

- [ ] `apps/sales/urls/LeadManagement/Overview.py` exposes `overview/` (`lead_overview`) and `leads/<int:pk>/handoff/` (`lead_handoff`).
- [ ] `LeadScoreEvents.py` exposes `score-events/` (`lead_score_event_list`), `score-events/adjust/` (`lead_score_event_adjust`), `score-events/recompute/` (`lead_score_event_recompute`), then `score-events/<int:pk>/` (`lead_score_event_detail`) and `score-events/<int:pk>/correct/` (`lead_score_event_correct`).
- [ ] `LeadQualifications.py` exposes `qualifications/` (`lead_qualification_list`), `qualifications/add/` (`lead_qualification_create`), `qualifications/<int:pk>/` (`lead_qualification_detail`), `qualifications/<int:pk>/edit/` (`lead_qualification_edit`), `qualifications/<int:pk>/delete/` (`lead_qualification_delete`), and the literal action suffixes `/partial/`, `/qualify/`, `/disqualify/`, `/recalculate/`, `/route-preview/`.
- [ ] `LeadRoutingRules.py` exposes `routing-rules/` (`lead_routing_rule_list`), `routing-rules/add/`, `routing-rules/<int:pk>/`, `/edit/`, `/delete/`, `/toggle/`, `/preview/`, and `/run/` with the corresponding `lead_routing_rule_*` names.
- [ ] `LeadNurtureEnrollments.py` exposes `nurture-enrollments/` (`lead_nurture_enrollment_list`), `nurture-enrollments/add/`, `nurture-enrollments/<int:pk>/`, `/edit/`, `/delete/`, and the literal action suffixes `/activate/`, `/pause/`, `/resume/`, `/complete/`, `/cancel/`, `/reply/`, `/convert-exit/`.
- [ ] `apps/sales/urls/__init__.py` sets `app_name = "sales"` and concatenates all five modules; literal routes precede every `<int:pk>` route. `config/urls.py` mounts `path("sales/", include("apps.sales.urls"))` only after the package exists. Reverse every declared name, not just the imported URL module.

## Templates

- [ ] Create exactly `templates/sales/leadmanagement/leadscoreevent/{list,detail}.html`; do not create a score-event form template. The list has search, category/type/source/lead/date filters, pagination, content assertions, and a manual-adjust action; detail shows the lead, signed delta, reason, source provenance, correction link, and the next recorded score projection without an edit/delete affordance.
- [ ] Create `templates/sales/leadmanagement/leadqualification/{list,detail,form}.html`, `templates/sales/leadmanagement/leadroutingrule/{list,detail,form}.html`, and `templates/sales/leadmanagement/leadnurtureenrollment/{list,detail,form}.html` using the exact bare filenames and context keys above. The three mutable model templates include full Actions columns/sidebars with POST+CSRF delete, condition-gated edit/delete, and status-specific lifecycle buttons.
- [ ] Create `templates/sales/overview.html` as a standalone computed operations board. It reads CRM leads and the four Sales records, shows score/rating, qualification, latest event, routing result, nurture state, duplicate/existing-party warning, and links to `crm:lead_detail`, CRM form submission, CRM campaign, and the CRM-owned conversion route; it never presents a second CRM lead editor.
- [ ] Use existing theme components only: `badge-green`, `badge-red`, `badge-amber`, `badge-info`, `badge-muted`, `badge-slate`; use the real `detail-grid`/`detail-item` layout and never invent `badge-success`, `badge-warning`, `badge-danger`, or `detail-label` classes (L33/L40). Guard nullable user/FK display, pagination links, and all delete confirmations.
- [ ] Templates must show the honest boundary: nurture is enrollment/state only and sends nothing; `next_touch_at` is a recorded target, not a scheduler; routing preview is non-mutating; score/campaign engagement is not a real ESP delivery claim.

## Shared integration, seed, and migration

- [ ] Add `apps.sales` to `config.settings.INSTALLED_APPS`, add the app config/root `__init__.py`, and add the `config.urls` include only in the single-writer integration step. Re-export the four models/forms/views and verify every import path before migration.
- [ ] Add `apps/sales/admin.py` registrations with tenant/list filters/search and system fields read-only; `LeadScoreEvent` is admin append-only, and routing cursor/last-assignment fields cannot be edited in admin.
- [ ] Add one `LIVE_LINKS["8.1"]` block in `apps/core/navigation.py` using the exact NavERP.md labels: `Lead Capture & Ingestion` -> `crm:formsubmission_list`; `Lead Scoring & Grading` -> `sales:lead_score_event_list`; `Lead Qualification & Routing` -> `sales:lead_qualification_list`; `Lead Nurturing & Drip Campaigns` -> `sales:lead_nurture_enrollment_list`; `Lead Conversion & Handoff` -> `sales:lead_overview#handoff`. Add staff-reachable extras for `Lead Operations Board`, `Score Events`, `Qualification Assessments`, `Routing Rules`, and `Nurture Enrollments`; verify every route reverses and the fragment exists.
- [ ] Reconcile the stale `NavERP-ERD.md` Module 1/8 ownership rows in the same close-out pass: CRM remains the owner of Lead/Opportunity/campaign/territory/conversion, while Sales 8.1 adds only the four records above and extends CRM by FK. Do not leave the ERD claiming a second Sales Opportunity/Territory implementation.
- [ ] Add `seed_sales` with per-model tenant guards, existing-row checks, and no bare `.create()` for unique constraints. Reuse seeded CRM leads, users, territory, drip EmailCampaign, ConsentPurpose, and Currency; create representative score events, one valid and one partially qualified assessment, fixed/territory/round-robin rules, and pending/active/paused/exited nurture rows where prerequisites exist. Backfill a score event for any pre-existing CRM lead whose score has no event rather than silently leaving projection history absent. The second run must be a no-op; `--flush`, if offered, must be explicitly scoped and never run against a shared database.
- [ ] The command prints `admin_acme` / `password` and warns that superuser `admin` has `tenant=None` and will see no Sales data. It must state in output that CRM email sends are simulated and that no worker/ESP is being seeded.
- [ ] Generate `sales.0001_initial` only after all model re-exports and the app registry are complete; run `makemigrations sales --dry-run` and inspect every listed model, then `makemigrations sales`, `migrate sales`, `showmigrations sales`, and `manage.py check`. Confirm `makemigrations --check` is clean and that no CRM model schema change is needed for the form/service integration.

## Verification, review, and tests

- [ ] Smoke as `admin_acme` / `password` after migrate + seed: every 8.1 list/detail/form/action page renders 200/302 as expected; detail HTML contains the object identifier and expected row values, not only a status code; no `{#`/`{% comment` marker leaks; page 1 and page 2 pagination links are valid; valid filters return the expected rows and junk filters do not 500 (L3/L8/L9/L11/L44).
- [ ] Run `venv\Scripts\python.exe temp\audit_integrity.py` and record its result; a new app must not disturb the existing spine/integrity checks.
- [ ] Sweep cross-tenant detail/edit/delete/action IDs and crafted foreign FK/M2M POSTs: every unauthorized object is 404 or a field validation error, never a data leak. GET on every POST-only adjustment/correction/route/lifecycle/handoff action is 405; non-admin writes are denied; tenantless `admin` sees empty scoped pages by design.
- [ ] Test the real invariants, not only happy paths: idempotent score replay, expired-event exclusion, inverse corrections, 0/100 clamping and cold/warm/hot bands, CRM form exclusion of score/rating, framework evidence failures, disqualified-reason guard, same-tenant FK/M2M checks, closed routing language/no `eval`, fixed/territory/round-robin resolution, cursor stability under repeated/concurrent-safe calls, explicit fallback/unrouted behavior, drip-only activation, optional-purpose consent evidence, lifecycle transition guards, converted-lead exit, and CRM conversion remaining the sole Party/Contact/Opportunity writer.
- [ ] Reverse every route, run `showmigrations sales`, and use the real `manage.py check`; do not treat `makemigrations --check`, an imported URL module, or an HTTP 200 as proof of registration/migration correctness (L54).
- [ ] Phase 4 review is six serial read-only passes in order: `code-reviewer`, `explorer`, `frontend-reviewer`, `performance-reviewer`, `qa-smoke-tester`, `security-reviewer`, each against the recorded `BASE...HEAD`. Append every result to `.claude/tasks/review-sales-8.1.md`; a missing result is missing coverage. Run the dedicated `code-fixer` phase and verify no finding remains open (L18/L50).
- [ ] Phase 6 first writes the test contract and shared `apps/sales/tests/__init__.py` + `apps/sales/tests/conftest.py`, then one test module at a time: `test_leadmanagement_models.py`, `test_leadmanagement_forms.py`, `test_leadmanagement_views.py`, `test_leadmanagement_security.py`. Every test/helper uses the `leadmanagement`/`test_leadmanagement_` naming prefix; no later module may shadow shared helpers.
- [ ] Run the full unfiltered `apps/sales/tests` suite with migrations enabled on the final gate; use `--nomigrations` only for fast iteration. Run the relevant CRM/accounts/core regression lanes and, where MariaDB-specific locks/JSON/constraint behavior is involved, a real configured-MariaDB check as well as SQLite tests (L19/L47/L49).

## Close-out

- [ ] Create `.claude/skills/sales/SKILL.md` for the new app with frontmatter `name: sales`, trigger wording, overview, all four as-built models and canonical CRM/core/accounting.Currency dependencies, exact routes/context names, templates, `seed_sales` behavior, tenancy/append-only/consent/routing gotchas, common tasks, and the `LIVE_LINKS["8.1"]` sidebar map.
- [ ] Update `README.md` Module 8 from `Roadmap` to an honest `8.1 built` status (not a claim of 8.1’s deferred capture/delivery/analytics features), and add a `### Module 8 8.1 — Lead Management (close-out YYYY-MM-DD)` section here with the real verification results.
- [ ] Update the ERD ownership reconciliation and the Sales skill together with the code; verify documentation paths, field names, choices, URL names, context keys, and seed output against the as-built implementation.
- [ ] Close only after a human sidebar pass confirms all five NavERP bullets land on meaningful staff-reachable destinations, with no “Soon” bullet accidentally left for a shipped 8.1 feature (L30).

### 8.1 close-out — 2026-09-25

**Status: 8.1 Lead Management is built and wired as the first Sales sub-module.** It adds exactly `LeadScoreEvent`, `LeadQualification`, `LeadRoutingRule`, and `LeadNurtureEnrollment`; CRM remains the sole owner of leads, opportunities, capture, territories, campaigns, and conversion identity writes.

- Migrations `sales.0001`–`sales.0003` are applied; `makemigrations sales --check --dry-run` reports no changes.
- `manage.py check` passes; `temp/audit_integrity.py` passes all six integrity gates; 12 Sales template compilations and 39 Sales route reversals pass.
- The full unfiltered Sales suite passes 284 tests with `--nomigrations`; the migration-enabled pytest run remains blocked by unrelated concurrent test processes in the shared checkout, not by a Sales assertion failure.
- `temp/smoke_sales_81.py`, `temp/smoke_sales_actions.py`, and `temp/smoke_sales_forms.py` complete with no failures; POST-only actions return 405 on GET and the cross-tenant check returns 404.
- Existing CRM form/conversion/security regressions pass under the isolated test configuration; the current combined `seed_sales` rerun is blocked only by concurrent 8.3 CRM schema drift (`crm_opportunity.currency_id`). The 8.1-only seeder completed twice before that unrelated migration drift.
- Six serial review passes and the code-fixer pass are complete; all canonical findings are fixed or explicitly documented as scope/performance skips in `.claude/tasks/review-sales-8.1.md`.
- `README.md`, `NavERP.md`, `NavERP-ERD.md`, and `.claude/skills/sales/SKILL.md` now describe 8.1's as-built boundary honestly. The checkout remains intentionally uncommitted because concurrent 8.2/8.3 and Projects work is present.

## Honest deferred scope

- [ ] Capture/ingestion: CRM landing pages, public forms, manual CRM leads, campaign members, and existing campaign lineage are reused; CSV/API/ad-platform/chatbot/email-parser/OCR ingestion and web tracking remain 8.18/CRM hardening.
- [ ] Scoring: deterministic fit/engagement history and fixed bands only; tenant-authored scoring profiles, arbitrary event-rule builders, decay workers, ML/predictive AI, and funnel analytics remain 8.12/23.
- [ ] Qualification/routing: BANT/MEDDIC evidence, fixed owner, CRM territory manager, and bounded round robin are in scope; seller calendars, weighted/skill routing, load balancing, bulk routing, SLA timers, and MQL-to-SQL acceptance remain 8.7/8.17/8.13.
...
- [x] URLs (flat `apps/core/urls.py`, literals before `<int:pk>`):
...
- [x] `language/list.html`, `timezone/list.html` — reference lists, `.table-wrap` + `.empty-state`, no Actions column (read-only).
- [x] `localeprofile/form.html`, `userlocale/form.html` — singleton edit forms.
- [x] `statutoryrule/list.html` (+ search/filters/pagination/Actions), `statutoryrule/form.html`, `statutoryrule/detail.html`.
- [x] `localizationboard.html`, `localizationoverview.html` — computed pages.
  — **review I1:** the three member-reachable pages (`language/list.html`, `timezone/list.html`, `userlocale/form.html`) originally offered only admin-gated exits, including a Cancel button that 403'd; the admin-only affordances are now wrapped in `{% if request.user.is_superuser or request.user.is_tenant_admin %}`, the same condition `core/decorators.py` gates on.

## Shared files & Integration

- [x] Re-export blocks in `apps/core/{models,forms,views}/__init__.py` (models before views; views import from `apps.core.models`).
- [x] Admin registration in `apps/core/admin.py` for all 5 models.
- [x] Extend `apps/core/management/commands/seed_core.py`: `_seed_localization_globals()` (Language + TimeZone — global, guarded by `.exists()`, **not** per tenant) called ONCE outside the tenant loop, and `_seed_localization(tenant)` with a **per-entity guard** (never a tenant-wide one).
- [x] `LIVE_LINKS["0.15"]` in `apps/core/navigation.py` mapping the EXACT NavERP.md bullet names:
  ...
  — verified byte-identical to the NavERP.md bullets by parsing the catalog with the real `parse_catalog()`.
- [x] `makemigrations core` → `0012_...`; `migrate`; `seed_core` ×2 (idempotency); `manage.py check`.

## Verification gate
- [x] `venv\Scripts\python.exe temp\audit_integrity.py` — all 6 checks (a new sub-module must not disturb check 3/4/5).
- [x] `apps/core/tests` full unfiltered suite green, plus 67 new `test_localization_*` tests across four lanes.
- [x] Six-lane review + one `code-fixer` pass — 1 Critical, 4 Important, 7 Minor, all fixed. See `review-core-0.15.md`.
...
- [x] `RateCards.py` — `ProjectRateCard` [RTC-]: Governs role/activity billing rate cards and direct expense markups with multi-tier overrides (Global Standard -> Client Custom -> Project Override).
...
- [x] `BillingRuns.py` — `ProjectBillingRun` [PBR-]: Batch billing session aggregating unbilled approved timesheets (`ResourceTimeEntry`) and expenses (`ProjectExpense`), calculating taxes/markups, generating canonical `accounting.Invoice` rows, and handling PDF dispatch.
...
- [x] `RevenueSchedules.py` — `ProjectRevenueSchedule` [PRS-]: Governs ASC 606 / IFRS 15 revenue recognition plans, cost center allocations, and unbilled vs deferred revenue balances.
...
- [x] `PaymentRecords.py` — `ProjectPaymentRecord` [PPR-]: Manages project-level accounts receivable collections workflow, dunning stages, promise-to-pay commitments, and dispute tracking.
...
- [x] Models: `RateCards.py`, `BillingRuns.py`, `RevenueSchedules.py`, `PaymentRecords.py`.
- [x] Forms: `RateCards.py` (`ProjectRateCardForm`), `BillingRuns.py` (`ProjectBillingRunForm`, `BillingRunDispatchForm`), `RevenueSchedules.py` (`ProjectRevenueScheduleForm`, `RevenueScheduleRecognizeForm`), `PaymentRecords.py` (`ProjectPaymentRecordForm`, `PaymentPromiseForm`, `ContactLogForm`).
- [x] Views: Function-based, `@login_required`, tenant-scoped, audit-logged via `write_audit_log`:
...
- [x] URLs: Literal routes ordered before `<int:pk>/` routes in `urls/FinancialBillingManagement/`:
...
- [x] Concatenate `urlpatterns` in `apps/projects/urls/__init__.py` with disjoint literals (`_fbm_ratecards`, `_fbm_billingruns`, `_fbm_revenueschedules`, `_fbm_paymentrecords`, `_fbm_boards`).
- [x] Admin registration in `apps/projects/admin.py` with `list_select_related`, `list_filter`, `search_fields`, `readonly_fields`.
- [x] Extend `apps/projects/management/commands/seed_projects.py` with idempotent `_seed_715_financial_billing_management(self, tenant)`.
- [x] Navigation wiring: `LIVE_LINKS["7.15"]` in `apps/core/navigation.py`:
...
- [x] Migration: `makemigrations projects` -> `0022_...`, run `migrate`, verify `seed_projects` twice (idempotent).
- [x] `manage.py check` passes with 0 errors.

## Templates (`templates/projects/financialbilling/`)
- [x] `ratecard/`: `list.html` (filter bar by project/is_active, rate cards table, action buttons), `detail.html`, `form.html`.
- [x] `billingrun/`: `list.html` (filter bar by status/billing_type, summary KPI cards, billing runs table), `detail.html` (line breakdowns, canonical invoice link, approval/invoice/dispatch action buttons), `form.html`.
- [x] `revenueschedule/`: `list.html` (filter bar by method/status, period selector, recognition table), `detail.html` (recognition calculation, cost center allocation, journal entry link), `form.html`.
- [x] `paymentrecord/`: `list.html` (filter bar by stage/dunning_level, collections table), `detail.html` (invoice balance, contact log, promise-to-pay commitment card, action buttons), `form.html`.
- [x] Computed boards:
...
- [x] Theme badges: strictly palette classes `.badge-green`, `.badge-amber`, `.badge-red`, `.badge-info`, `.badge-slate`.
...
BASE = `c3ccc923bf123c6db9ce35de9717794f83a4d01e`. Next migration: `0021`.
...
> Phase 1 — **2 new models** (`TaskChecklistItem` [TCL-], `TaskBlock` [TBK-]) + an **in-place
> execution-field extension of `ProjectTask`** + **3 computed pages** (`task_board`,
> `gantt_timeline`, `task_priority`). No new dependency table, no new assignment register, no
> stored aggregate — every board, quadrant, rollup and blocked state is computed on read.
> Migration leaf is `0010_alter_scopeitem_status` today → **expect `0011_…`, never reserve**.

### Scope, conventions, rulings, build order

- [ ] **Concurrency (read first):** 7.1–7.6 are built and live; **the peer session owns 7.7's TEST wave —
      `.claude/tasks/test-contract-projects-7.7.md`, `apps/projects/tests/conftest.py` and
      `apps/projects/tests/test_zz_scope_smoke_throwaway.py` are ITS territory: 7.8 does NOT touch them**.
      `todo.md`, the migration leaf and every shared file (`models/forms/views/urls __init__.py`, `admin.py`,
      `seed_projects.py`, `navigation.py`, `overview.html`) are contended: every shared-file touch is an
      **Integrate-step item**, done once, as a **surgical `Edit` with a re-read anchor** immediately before
      the edit (**L43: never full-rewrite a shared 7,300-line file; NEVER use `Write` on `todo.md`**).
      Commits are path-limited: `git add '<f>'; git commit -m '<msg>' -- '<f>'`.
- [ ] **NavERP.md 7.8 bullets (verbatim — the `LIVE_LINKS["7.8"]` keys must match character-for-character,
      the sidebar maps bullet text → route):**
      1. **Task Creation & Assignment** — Individual and team tasks, sub-tasks, checklists, and bulk operations.
      2. **Priority & Urgency Scoring** — MoSCoW, Eisenhower matrix, and custom priority frameworks.
      3. **Kanban & Scrum Boards** — Visual workflow states, WIP limits, and drag-and-drop progression.
      4. **Gantt Charts & Timeline Views** — Bar charts, timeline dependencies, and progress shading.
      5. **Task Dependencies & Blocking** — Predecessor/successor links, blocked status, and unblock criteria.
- [ ] **2 new models + the in-place extension, no third table:** `TaskChecklistItem` [TCL-], `TaskBlock`
      [TBK-]. Prefixes `TCL`/`TBK` verified free repo-wide (`TSK` is 7.2's, `TASK` is crm 1.8's pre-spine
      stand-in, `SCR` is 7.7's; `DEP`/`RAL`/`RTE` taken — **do not shorten TCL to TASK**). **Do not add a
      `WorkTask`, `TaskExecution`, `TaskAssignment`, `KanbanBoard`, `ColumnConfig` or `WipLimit` table** —
      Rulings 1–4.
- [ ] **Build order: ProjectTask extension → TaskChecklistItem → TaskBlock** (both new tables FK
      `ProjectTask`; the extension shares the same migration, so it lands first).
- [ ] **Rulings 1–5 (research §Rulings), each restated as a one-line build constraint:**
      1. **7.8 EXTENDS `ProjectTask` IN PLACE; no second task model** — the `ProjectTask` docstring
         (`ProjectTasks.py:8-10`) IS the contract; sub-tasks already work (`parent` self-FK). A separate
         `WorkTask`/`TaskExecution` table is the L29/L36 bug.
      2. **Assignment: ONE `assignee` FK on the task; RAL/RTE stay the registers** — nullable FK →
         `AUTH_USER_MODEL` (the doer; `owner` stays the accountable manager; RSP has no User FK so RAL
         cannot serve as the login-grain assignee). Multi-assignee M2M is **deferred (P2)**.
      3. **"Blocked" is DERIVED from `TaskDependency`; a manual block is a `TaskBlock` evidence row** —
         `is_dependency_blocked` is a computed property over `predecessor_links`, **never a stored column**
         (a stored boolean goes stale and has no evidence trail); real-world blockers get a TBK row with
         `unblock_criteria`, verb-stamped and frozen.
      4. **Board columns ARE `ProjectTask.STATUS_CHOICES`; no workflow engine, no column-config table** —
         WIP-limit *values* are 7.19 configuration (counts render now); "drag-and-drop" ships as POST-only
         status verbs per card (no JS framework in the repo).
      5. **`CostControlAccount.percent_complete` stays attested; 7.8 feeds a READ-ONLY lens, no write-back**
         — the effort-weighted rollup is computed on read and rendered advisory; **nothing writes the CCA
         column** (L36: extend by lens, never re-declare; derived figures are never stored columns).
- [ ] **Spine verified (L28), do not re-derive:** `projects.ProjectTask`
      (`ProjectPlanningScheduling/ProjectTasks.py:22` — extend in place), `projects.TaskDependency`
      (`TaskDependencies.py:17`), `critical_path_ids(project)` (`views/_helpers.py:107`),
      `projects.ResourceAllocation`/`projects.ResourceTimeEntry` (7.3, lens only),
      `projects.CostControlAccount` (7.4, advisory lens only), `core.AuditLog`, `AUTH_USER_MODEL`.
      **No `TaskChecklistItem`/`TaskBlock` class exists anywhere**; `related_name`s
      `assigned_project_tasks`/`checklist_items`/`blocks` verified free **on `ProjectTask`** (inventory's
      `checklist_items` sits on other target models — no same-target clash). 7.8 FKs only
      `ProjectTask`/`AUTH_USER_MODEL` and re-declares none of 7.1–7.7 (**L36**).
- [ ] Both new tables: `TenantNumbered` subclasses (`NUMBER_PREFIX` as above), **no money column anywhere
      in 7.8** (L29), **every progress, quadrant, WIP and blocked figure is a derived property or a
      computed view, NEVER a stored column** (the 7.1 ROI / 7.4 EVM / 7.5 simulation ruling), audit
      actions ≤ 10 chars (`AuditLog.action` is varchar(10)), `unique_together ("tenant","number")`.
- [ ] Layers: `apps/projects/{models,forms,views,urls}/TaskWorkManagement/<Entity>.py` — the research's
      package name (full sub-module title in PascalCase, matching `QualityManagement`/`ScopeRequirements`;
      the SHORTENED form is reserved for templates). Same file name in all four layers; sub-package
      `__init__.py` files stay EMPTY; re-exports only in the four top-level `__init__.py`. Files:
      `TaskChecklistItems.py`, `TaskBlocks.py`, plus `ProjectTasks.py` (the task verbs/execution edit),
      `TaskBoard.py`, `GanttTimeline.py`, `TaskPriority.py`.
- [ ] Templates: **`templates/projects/taskwork/`** is the NEW sub-module folder (7.2 owns
      `templates/projects/planning/task/`; the `quality/`←QualityManagement shortening precedent).
      **Shape chosen — entity SUB-FOLDERS for the two entities** (CLAUDE.md rule 3's collapse applies only
      to single-entity sub-modules; 7.8 has two): `taskwork/checklistitem/{list,detail,form}.html`,
      `taskwork/block/{list,detail}.html`; the three computed pages, the execution form and the
      task-detail partials are **standalone at the sub-module root** (rule 6):
      `taskwork/task_board.html`, `taskwork/gantt_timeline.html`, `taskwork/task_priority.html`,
      `taskwork/task_execution.html`, `taskwork/_task_checklist_panel.html`,
      `taskwork/_task_blocks_panel.html`, `taskwork/_task_dependencies_panel.html`. `templates/projects/`
      verified: `cost/ initiation/ planning/ quality/ resource/ risk/ scope/ overview.html` — **no
      `taskwork/` yet**.
- [ ] Migration — **the number is assigned at generation by the disk leaf** (`0010_alter_scopeitem_status`
      today → **expect `0011_…` but never reserve**); **announce-then-generate**: `makemigrations projects
      --dry-run` first, read every model it names — anything beyond `TaskChecklistItem`/`TaskBlock`/
      `ProjectTask` (a peer's model or a missing re-export) means **STOP and report**; the migration
      commit body names exactly what was swept in. Tests `test_taskwork_*` / `taskwork_*` / `_taskwork_*`
      (no collision with `test_initiation_*`, `test_planning_*`, `test_resource_*`, `test_cost_*`,
      `test_risk_*`, `test_quality_*`, or the peer's `test_scope_*`).
- [ ] **L31 — one sub-module per run:** 7.8 builds its own two tables + the `ProjectTask` field extension,
      nothing else; no reach into 7.4 (money), 7.3 (`ResourceAllocation`/`ResourceTimeEntry` registers),
      7.5 (issues), 7.2 (the WBS/dependency/baseline CRUD), 7.13 (sprints), 7.16 (charts), 7.17
      (notifications/rules) or 7.19 (WIP-limit values, priority frameworks).
- [ ] **L29 — no-second-ledger analogues, enforced in review:** **no second time log** (7.3's RTE; 7.8
      renders `time_entries` as a task-detail lens at most), **no second assignment register** (7.3's RAL
      books role/capacity; 7.8's `assignee` is the login-grain doer ON the task), **no second dependency
      graph** (7.2's `TaskDependency`; blocking DERIVES from it — Ruling 3), **no second task table**
      (Ruling 1), **no stored progress/blocked column** (Rulings 3/5), no money columns, no
      `accounting.Currency`/`core.Party` FK.
- [ ] FKs declared **by string** (`"projects.ProjectTask"`, `settings.AUTH_USER_MODEL`) — no cross-app
      model import at module level.
- [ ] **L16 — every date comparison uses `timezone.localdate()`**, never `datetime.date.today()`
      (`is_overdue`, the Gantt today-marker, the window defaults, the verb date stamps).
- [ ] **L27 — all 7.8 verbs are `@login_required` member-level; NO tenant-admin gate this pass** (none is
      a privileged/workspace-config write; the state-machine refusals + frozen rows are the guards).
      `tsk_bulk_update` POSTs per selected id with a per-row audit entry.

---

### ProjectTask — the in-place execution extension (Ruling 1; edit `models/ProjectPlanningScheduling/ProjectTasks.py`, same migration)

#### Choices (exact machine values)
- [ ] `PRIORITY_CHOICES` (`max_length=8`) — `low`/Low, `medium`/Medium, `high`/High, `critical`/Critical
- [ ] `MOSCOW_CHOICES` (`max_length=12`) — `must_have`/Must Have, `should_have`/Should Have,
      `could_have`/Could Have, `wont_have`/Won't Have

#### Fields (appended after `confidence`; NOTHING existing is altered)
- [ ] `assignee` `FK(settings.AUTH_USER_MODEL, SET_NULL, null=True, blank=True,
      related_name="assigned_project_tasks")` — **the doer**; help-text points staffing at 7.3 ("team
      bookings live on ResourceAllocation"). FORM-WRITABLE (both forms).
- [ ] `priority` `CharField(max_length=8, choices=PRIORITY_CHOICES, default="medium")` — FORM-WRITABLE.
- [ ] `moscow` `CharField(max_length=12, choices=MOSCOW_CHOICES, null=True, blank=True)` — **no default:
      "unclassified" is a state, not a value**. FORM-WRITABLE.
- [ ] `is_urgent` / `is_important` `BooleanField(default=False)` — the Eisenhower inputs; the quadrant is
      computed on read. FORM-WRITABLE.
- [ ] `percent_complete` `DecimalField(max_digits=5, decimal_places=2, default=Decimal("0"),
      validators=[MinValueValidator(0), MaxValueValidator(100)])` — task-grain attestation;
      **FORM-WRITABLE** and also stamped to 100 by `tsk_complete`; the deliverable-node rollup is computed
      on read (effort-weighted over descendants), never stored (Ruling 5).
- [ ] `actual_start` / `actual_end` `DateField(null=True, blank=True, editable=False)` — **VERB-WRITTEN
      ONLY** (`tsk_start` / `tsk_complete`), form-excluded via `editable=False` (the 7.4/7.6
      verb-written-stamp idiom). No other execution stamps exist on the task.
- [ ] `Meta.indexes` += `("tenant","assignee")` `tsk_tnt_assignee_idx`, `("tenant","priority")`
      `tsk_tnt_priority_idx` (appended; the existing four indexes untouched).

#### Derived properties (never columns)
- [ ] `is_overdue` — `planned_end` passed and `status in ("planned","in_progress")` (L16 clock).
- [ ] `is_dependency_blocked` — an FS predecessor with `status not in ("done","cancelled")`, or an SS
      predecessor with `status == "planned"` (computed over `predecessor_links`; FF/SF do not block).
      **Derived, never stored — Ruling 3.**
- [ ] `is_manually_blocked` — any active `TaskBlock` (`unblocked_at is null`).
- [ ] `is_blocked` = `is_dependency_blocked or is_manually_blocked`.
- [ ] `checklist_progress` — done/total over `checklist_items` (None when the task has no items); a
      computed input to the progress lens, never a column.
- [ ] `eisenhower_quadrant` — `do_first` (urgent+important) / `schedule` (important only) / `delegate`
      (urgent only) / `eliminate` (neither).
- [ ] Form implications: 7.2's `TaskForm` (`forms/ProjectPlanningScheduling/ProjectTasks.py`)
      `Meta.fields` gains `assignee, priority, moscow, is_urgent, is_important, percent_complete`
      (surgical edit; **`status` STAYS on that form** — it is 7.2's planning write and removing it breaks
      the 7.2 contract; `actual_start`/`actual_end` are unreachable because `editable=False`); `assignee`
      joins `owner` in the NOT-`_reject_foreign` set (users can be tenant-less — the `TaskForm` docstring
      precedent).

---

### Model 1 — `TaskChecklistItem` [TCL-] (`models/TaskWorkManagement/TaskChecklistItems.py`)

Base `TenantNumbered`, `NUMBER_PREFIX = "TCL"`. Realizes bullet **1's checklists** and feeds the progress
rollup — rows give one-time done-stamps; a TextField never could.

#### Fields
- [ ] `task` `FK("projects.ProjectTask", CASCADE, related_name="checklist_items")`
- [ ] `label` `CharField(max_length=255)`
- [ ] `sequence` `PositiveSmallIntegerField(default=0)` — order within the task's checklist
- [ ] `is_done` `BooleanField(default=False)` — **VERB-WRITTEN ONLY via `tcl_check`, NOT on the form**
- [ ] `done_by` `FK(settings.AUTH_USER_MODEL, SET_NULL, null=True, blank=True, editable=False,
      related_name="checked_task_checklist_items")`; `done_at` `DateTimeField(null=True, blank=True,
      editable=False)` — stamped exactly once per direction by `tcl_check`
- [ ] `created_by` `FK(settings.AUTH_USER_MODEL, SET_NULL, null=True, blank=True, editable=False,
      related_name="+")`

#### Meta / behaviour
- [ ] `ordering = ["task_id", "sequence", "id"]` (a checklist reads in sequence order);
      `unique_together = ("tenant","number")`; indexes `("tenant","task")` `tcl_tnt_task_idx`,
      `("tenant","is_done")` `tcl_tnt_done_idx`
- [ ] `__str__` = `f"{self.number} — {self.label}"`
- [ ] Verb **`tcl_check`** (POST-only, login): open → done stamps `done_by=request.user` /
      `done_at=timezone.now()`; done → open clears both (the un-tick goes through the SAME verb + audit —
      one writer per direction); audit `update`, `changes={"verb":"tcl_check","from":…,"to":…}`; capture
      `previous = obj.is_done` BEFORE mutating.
- [ ] Form `TaskChecklistItemForm` excludes `tenant`, `number`, `is_done`, `done_by`, `done_at`,
      `created_by`; `task` tenant-scoped (`_reject_foreign` re-checks); `label`/`sequence` stay editable
      on done items (a tick item is a working row, not frozen evidence — only the STAMPS are verb-only).

---

### Model 2 — `TaskBlock` [TBK-] (`models/TaskWorkManagement/TaskBlocks.py`)

Base `TenantNumbered`, `NUMBER_PREFIX = "TBK"`. Realizes bullet **5's blocked status + unblock criteria**
— the manual half of Ruling 3 (the auto half is the derived property).

#### Fields
- [ ] `task` `FK("projects.ProjectTask", CASCADE, related_name="blocks")`
- [ ] `reason` `TextField()` — required (why it is blocked)
- [ ] `unblock_criteria` `TextField()` — required (the bullet's exact ask: what must be true to unblock)
- [ ] `resolution_note` `TextField(blank=True)` — how the criteria were met, written by `tsk_unblock`
- [ ] `blocked_by` `FK(settings.AUTH_USER_MODEL, SET_NULL, null=True, blank=True, editable=False,
      related_name="task_blocks_raised")`; `blocked_at` `DateTimeField(null=True, blank=True,
      editable=False)` — stamped by `tsk_block`
- [ ] `unblocked_by` `FK(settings.AUTH_USER_MODEL, SET_NULL, null=True, blank=True, editable=False,
      related_name="task_blocks_cleared")`; `unblocked_at` `DateTimeField(null=True, blank=True,
      editable=False)` — **written exactly once** by `tsk_unblock`
- [ ] `created_by` `FK(settings.AUTH_USER_MODEL, SET_NULL, null=True, blank=True, editable=False,
      related_name="+")`

#### Meta / behaviour
- [ ] `ordering = ["-created_at", "-id"]`; `unique_together = ("tenant","number")`; indexes
      `("tenant","task")` `tbk_tnt_task_idx`, `("tenant","unblocked_at")` `tbk_tnt_unblocked_idx`
      (serves the active-block lookup `unblocked_at__isnull=True`)
- [ ] `is_active` property = `unblocked_at is None`; `__str__` = `f"{self.number} — {self.task}"`
- [ ] **Evidence-row ruling: rows are minted ONLY by `tsk_block` and closed ONLY by `tsk_unblock`** —
      **no `tbk_create`/`tbk_edit`/`tbk_delete` routes ship at all**: a form-created row would carry no
      blocker stamp; active blocks refuse edit/delete (frozen while open — the exit is `tsk_unblock` with
      a `resolution_note`); once unblocked the row is frozen evidence entirely (the 7.4/7.6 frozen-row
      idiom). `tbk_list`/`tbk_detail` are the only routes; **no TaskBlock ModelForm exists**.

---

### Computed pages (no model — the 7.3 `capacity_demand` / 7.5 `risk_analysis` / 7.6 `quality_improvement` precedent)

All three GET-only; `?project=`/`?assignee=` parsed through `as_db_int` (`apps.core.crud`), never raw
`request.GET` (L11/L35); every enum lens allow-listed against CHOICES; derived properties are
PRE-SCOPED in the view where a DB-side lens is impossible (L11).

- [ ] **`task_board`** — `views/TaskWorkManagement/TaskBoard.py`, route `task-board/`, name `task_board`,
      template `projects/taskwork/task_board.html`. Context contract:
      - `projects` (tenant `Project` dropdown), `project` (from `?project=`, else None → all-tenant
        board), `assignees` (`owners(tenant)`), `assignee` (from `?assignee=`, else None — the "my tasks"
        lens)
      - `columns` — **one dict per `ProjectTask.STATUS_CHOICES` value in declared order** (`planned`,
        `in_progress`, `done`, `cancelled`): `{"value","label","tasks","count","over_limit"}`;
        within-column order = priority rank (`critical→high→medium→low`), then `planned_start` (nulls
        last), then `-id` (Wrike's priority-default sort); deliverable nodes render as summary cards
        (rollup figures, NO verb buttons)
      - `WIP_LIMITS` — a documented **empty module constant** (`{}`) keyed by status: the 7.19 hook.
        Counts render now; `over_limit` is computed only when an entry exists (always `False` today) —
        **Ruling 4, the WIP-limit source**
      - `ready_queue` — the "Unblocked & ready" queue: `status="planned"`, ≥1 predecessor link, no
        unfinished predecessor, no active `TaskBlock`; ordered `planned_start` (nulls last), `id`
      - `total_count`, `blocked_count`, `overdue_count`
- [ ] **`gantt_timeline`** — `views/TaskWorkManagement/GanttTimeline.py`, route `gantt-timeline/`, name
      `gantt_timeline`, template `projects/taskwork/gantt_timeline.html`. Context contract:
      - `projects`, `project` (from `?project=`; None → the picker empty state)
      - `window` `{"start","end","days"}` — `?start=`/`?end=` via a plain `forms.Form` of `DateField`s;
        defaults = the task-date union padded 7 days each side (undated → `today..today+30`); **clamps:
        `end ≤ start` → `end = start + 30d`; span > 366d → `end = start + 366d`; junk → default** (never
        a 500)
      - `bars` — one row per WBS node in tree order: `{"task","depth","left_pct","width_pct",
        "progress_pct","is_critical","is_deliverable"}` — CSS bars (left/width = offset/duration over the
        window; progress shading from `percent_complete`; deliverable rows use the min/max descendant
        window + the **effort-weighted progress rollup computed on read — Ruling 5**)
      - `critical_ids` — the `critical_path_ids(project)` pk set, for the template's `in` highlight
      - `dep_rows` — `{"dependency","predecessor","successor","link_type_label","lag_days"}` (the links
        table beside the chart; arrows are JS → deferred)
      - `conflicts` — successors whose `planned_start` precedes their FS predecessor's `planned_end`
      - `today` — the today-marker line (`timezone.localdate()`)
- [ ] **`task_priority`** — `views/TaskWorkManagement/TaskPriority.py`, route `task-priority/`, name
      `task_priority`, template `projects/taskwork/task_priority.html`. Context contract:
      - `projects`, `project`, `assignees`, `assignee` (same guards as the board)
      - `moscow_groups` — declared order (`must_have, should_have, could_have, wont_have`) **plus a
        trailing `unclassified` group** (`moscow` null — a state, not a default):
        `{"value","label","tasks","count"}`; tasks ordered priority-rank → `planned_start`
      - `quadrants` — the Eisenhower 2×2 in fixed order `do_first`/`schedule`/`delegate`/`eliminate`,
        bucketed on read from `is_urgent`/`is_important`: `{"key","label","tasks","count","badge"}`
        (colour-named badge, **L33**)
      - `work_queue` — the overdue/urgent queue: live tasks (`planned`/`in_progress`) that are
        `is_overdue` OR `is_blocked` OR `priority ∈ {high, critical}`, ordered overdue-first →
        priority-rank → `planned_start`; cap 25
      - `counts` — `{"overdue","blocked","unassigned","in_progress"}` over the scoped queryset

---

## Backend (`apps/projects/{models,forms,views,urls}/TaskWorkManagement/`) — one file per entity per layer

- [ ] `models/TaskWorkManagement/{TaskChecklistItems,TaskBlocks}.py` as specified above; **the
      `ProjectTask` extension is an in-place edit of the 7.2 model file**
      (`models/ProjectPlanningScheduling/ProjectTasks.py` — append fields/properties/indexes, alter
      nothing existing); sub-package `__init__.py` files EMPTY; nothing added to the top-level
      `__init__.py` until Integrate
- [ ] `forms/TaskWorkManagement/{ProjectTasks,TaskChecklistItems}.py` — `TaskExecutionForm` (ModelForm:
      `assignee`, `priority`, `moscow`, `is_urgent`, `is_important`, `percent_complete` ONLY — plan fields
      stay 7.2's; `assignee` dropdown from `owners(tenant)`, NOT `_reject_foreign`-checked), `TaskBlockForm`
      (plain `forms.Form`: `reason` + `unblock_criteria`, both required `Textarea`s — bound by
      `tsk_block`), `TaskUnblockForm` (plain `forms.Form`: `resolution_note` required — bound by
      `tsk_unblock`), `TaskChecklistItemForm` (exclusions pinned above)
- [ ] `views/TaskWorkManagement/ProjectTasks.py` — the execution edit + five task verbs:
      `tsk_execute` (GET renders `TaskExecutionForm`, POST saves + audit `update`); `tsk_start` (POST;
      `planned` → `in_progress`, stamps `actual_start=timezone.localdate()`, audit `update`; **refused
      unless `status == "planned"` and not `is_blocked`**); `tsk_complete` (POST; `in_progress` → `done`,
      stamps `actual_end` + `percent_complete=100`, audit `update`; **hard-refuses while `is_blocked`** —
      the ClickUp default, the message naming the active dependency/manual blocker); `tsk_block` (POST;
      binds `TaskBlockForm`, mints the TBK row stamped `blocked_by/blocked_at`, audit `create`; **refused
      while an active block already exists** — one open blocker at a time, so `is_manually_blocked` always
      names THE row); `tsk_unblock` (POST; binds `TaskUnblockForm`, stamps `unblocked_by/unblocked_at` +
      `resolution_note` exactly once, audit `update`; refused when nothing is active); every verb captures
      `previous` BEFORE mutating, writes `changes={"verb","from","to"}`, refuses a disallowed transition
      with `messages.*` + redirect
- [ ] `views/TaskWorkManagement/TaskChecklistItems.py` — `tcl_list/create/detail/edit/delete` via the
      `crud_*` helpers + `tcl_check`; every queryset `filter(tenant=request.tenant)`, never `.all()`;
      `?task=`/`?is_done=` filters `as_db_int`-guarded / allow-listed (L11)
- [ ] `views/TaskWorkManagement/TaskBlocks.py` — `tbk_list`/`tbk_detail` ONLY (the evidence-row ruling)
- [ ] `views/TaskWorkManagement/{TaskBoard,GanttTimeline,TaskPriority}.py` — the three computed pages
      (absolute imports only); **reuse `owners(tenant)` and `critical_path_ids(project)` from
      `views/_helpers.py` — add nothing to the helpers**
- [ ] `urls/TaskWorkManagement/` — six modules: `ProjectTasks.py` (the six task routes, under **7.2's
      existing `tasks/` first segment** — disjoint leaf literals, no new first segment),
      `TaskChecklistItems.py` (`checklist-items/`), `TaskBlocks.py` (`blocks/`), `TaskBoard.py`
      (`task-board/`), `GanttTimeline.py` (`gantt-timeline/`), `TaskPriority.py` (`task-priority/`);
      literal routes before `<int:pk>/` (first-match-wins)

## Views, routes & CONTEXT KEYS — the contract (L7/L8: an unpinned name renders blank at 200)

- [ ] **Task verbs + execution edit** (all POST-only except `tsk_execute`; all login/member-level — L27):
      `tasks/<int:pk>/execute/` `tsk_execute`; `tasks/<int:pk>/start/` `tsk_start`;
      `tasks/<int:pk>/complete/` `tsk_complete`; `tasks/<int:pk>/block/` `tsk_block`;
      `tasks/<int:pk>/unblock/` `tsk_unblock`; `tasks/bulk-update/` `tsk_bulk_update` (POST-only;
      status/assignee/priority over multi-select ids, per-id loop with per-row transition gating + a
      per-row audit entry — the `rte_approve_week` bulk idiom; the research's P1, ships after the core
      verbs). Buttons render on board cards, the task-detail panels and the checklist/block registers.
- [ ] **TaskChecklistItem** — `tcl_list`, `tcl_create`, `tcl_detail`, `tcl_edit`, `tcl_delete`; verb
      `checklist-items/<int:pk>/check/` `tcl_check` (POST-only). Routes (prefix `checklist-items/`):
      `checklist-items/`, `checklist-items/add/` (`?task=<pk>` preselect, `as_db_int`-guarded),
      `checklist-items/<int:pk>/`, `checklist-items/<int:pk>/edit/`, `checklist-items/<int:pk>/delete/`.
      `tcl_list`: qs `.select_related("task","task__project","done_by")`;
      `search_fields=["number","label"]`; `filters=[("task","task_id",True), ("is_done","is_done",False)]`.
      `extra_context`: `tasks` (tenant `ProjectTask` queryset). Detail: `obj` + the parent-task link.
- [ ] **TaskBlock** — `tbk_list`, `tbk_detail` ONLY. Routes (prefix `blocks/`): `blocks/`,
      `blocks/<int:pk>/`. `tbk_list`: qs `.select_related("task","task__project","blocked_by",
      "unblocked_by")`; `search_fields=["number","reason","unblock_criteria","resolution_note"]`;
      `filters=[("task","task_id",True)]`; derived `?active=1` → `Q(unblocked_at__isnull=True)`
      (pre-scoped). Detail: `obj` + the block/unblock action panels (the verbs POST to the TASK routes).
- [ ] **Five url first segments pinned — `checklist-items/`, `blocks/`, `task-board/`, `gantt-timeline/`,
      `task-priority/`** — all **disjoint literals** from 7.1–7.7's existing first segments (`""`,
      `project-requests/`, `projects/`, `stakeholders/`, `kickoffs/`, `tasks/`, `dependencies/`,
      `milestones/`, `baselines/`, `resource-profiles/`, `allocations/`, `time-entries/`,
      `capacity-demand/`, `budgetlines/`, `revisions/`, `controlaccounts/`, `expenses/`, `risks/`,
      `responses/`, `issues/`, `escalations/`, `risk-analysis/`, `risk-monitoring/`, `quality-plans/`,
      `quality-reviews/`, `inspections/`, `defects/`, `quality-improvement/`, `quality-acceptance/`,
      `requirements/`, `scope-items/`, `scope-changes/`, `scope-verifications/`, `scope-matrix/`); the
      task-verb routes deliberately REUSE 7.2's `tasks/` segment with disjoint leaf literals
      (`execute|start|complete|block|unblock|bulk-update` tails cannot collide with `<int:pk>/`). **Re-check
      the concatenated `urls/__init__.py` at Integrate** in case the peer added a segment. No route uses a
      converter in its first component.

## Templates (`templates/projects/taskwork/`)

- [ ] `checklistitem/{list,detail,form}.html`; `block/{list,detail}.html` — entity sub-folders because
      7.8 has TWO entities (CLAUDE.md rule 3's collapse is single-entity-only); lists: filter bar
      reflecting `request.GET`, Actions column (delete POST + `confirm()` + `{% csrf_token %}` on
      `tcl_delete`; the block register has NO delete — evidence), pagination with
      `has_previous`/`has_next` guards (**L9**), empty states
- [ ] The three computed pages, the execution form and the partials stand at the sub-module root
      (rule 6): `task_board.html` (status columns with counts + WIP badges + the ready queue),
      `gantt_timeline.html` (CSS bars, progress shading, critical-path highlight, today line, links
      table, conflict flags — **no chart library**, 7.16's), `task_priority.html` (MoSCoW groups, the 2×2
      grid, the work queue), `task_execution.html` (the slim execution form), and
      `_task_checklist_panel.html` / `_task_blocks_panel.html` / `_task_dependencies_panel.html`
      (partials **included by a surgical edit of 7.2's `templates/projects/planning/task/detail.html`** —
      the lenses live on task detail)
- [ ] Colour-named badge classes only (`badge-green/-amber/-red/-info/-muted/-slate`, **L33**) — run
      `grep -n '\.badge-' static/css/theme.css` before writing the priority/blocked/overdue/WIP ternaries;
      every badge block ends `{% else %}{{ obj.get_<field>_display }}{% endif %}`
- [ ] **No nullable FK inside a `|default:` filter argument** (the 7.1 four-500 idiom, **L10**) — use
      `{% if fk %}…{% else %}—{% endif %}` for `assignee`/`done_by`/`blocked_by`/`unblocked_by`/`owner`
- [ ] **Multi-line notes use `{% comment %} … {% endcomment %}`** — never a multi-line `{# … #}` (**L2**);
      the Gantt page carries the computed-lens caveat (planning-grade, no auto-rescheduling)
- [ ] FK `<select>` comparisons use `|stringformat:"d"`; `{% extends "base.html" %}` unchanged

## Integrate (single writer — the ONLY shared-file step; surgical `Edit`, re-read anchors)

- [ ] Re-export blocks `# --- 7.8 Task & Work Management` **appended AFTER the existing `# --- 7.7`
      blocks** in all four top-level `__init__.py` (models: 2 models; forms: 4 forms; views: 2 CRUD sets +
      `tbk_detail` + 5 task verbs + `tsk_execute` + `tcl_check` + 3 computed views; urls: 6 urlpatterns
      imports + concat appended after the `_sr_matrix` line). A missing re-export is a runtime
      `ImportError`
- [ ] `admin.py` — append `TaskChecklistItemAdmin` + `TaskBlockAdmin` after the 7.7 block (`list_display`
      led by `number`, `list_select_related` for every rendered FK, `done_by`/`done_at`/`blocked_by`/
      `blocked_at`/`unblocked_by`/`unblocked_at` readonly — verb-state columns frozen); **plus a surgical
      edit of the existing `ProjectTaskAdmin`**: `list_display` += `assignee`, `priority`,
      `percent_complete`; `list_select_related` += `assignee`
- [ ] `seed_projects.py` — `_taskwork` block with its OWN guard
      (`TaskChecklistItem.objects.filter(tenant=tenant).exists()`), called after `self._scope(...)` in
      `_seed_tenant`: stamps the existing seeded WBS work packages with execution state (assignees, every
      `priority`, every `moscow` value + unclassified rows, all four Eisenhower quadrants,
      `percent_complete` 0/50/100, `in_progress`/`done` rows carrying `actual_start`/`actual_end`, one
      `cancelled`) and adds: a dependency-blocked task (unfinished FS predecessor off the existing
      `_planning` network), a manually blocked task with an OPEN `TaskBlock` (reason + `unblock_criteria`
      + stamps), an unblocked `TaskBlock` trail (both stamp pairs + `resolution_note`), checklists with
      mixed ticks (stamped `done_by`/`done_at` + open items) on several tasks + one empty checklist —
      enough rows for page 2. `--flush`: add `TaskChecklistItem.objects.all().delete()` +
      `TaskBlock.objects.all().delete()` at the TOP of the children-first list (both FK tasks) and update
      the `add_arguments` help text; imports gain both models
- [ ] `apps/core/navigation.py` — one new `LIVE_LINKS["7.8"]` immediately after the `"7.7"` block
      (~`:1808`), verbatim: `Task Creation & Assignment` → `projects:tsk_list`; `Priority & Urgency
      Scoring` → `projects:task_priority`; `Kanban & Scrum Boards` → `projects:task_board`; `Gantt Charts
      & Timeline Views` → `projects:gantt_timeline`; `Task Dependencies & Blocking` →
      `projects:dependencies`; extra live leaf `Task Checklist Register` → `projects:tcl_list`; the
      justification comments record the two deliberate register mappings (bullet 1 → 7.2's task register
      because 7.8 extends that row in place — Ruling 1; bullet 5's links half → 7.2's dependency register
      because the edges are 7.2's — Ruling 3; 7.8 owns the blocking STATE on top, reached from the
      task-detail panels). `_safe_reverse` supports `?query=` (confirmed)
- [ ] `templates/projects/overview.html` — 7.8 quick links (board, Gantt, priority lens, checklist
      register) + counts (in-progress, blocked, overdue tasks), mirroring the 7.6 block; extend the intro
      layer sentence
- [ ] **DB LAST (announce-then-generate):** `python manage.py makemigrations projects --dry-run` — read
      every model named; anything beyond the two new tables + the `ProjectTask` alterations means **STOP
      and report**. Then generate (**number = disk leaf read now — expect `0011_…`, never reserve**),
      `migrate`, `seed_projects` ×2 (second run a no-op), `manage.py check`; the migration commit body
      names exactly what was swept in

## Verify

- [ ] `manage.py check` clean; `makemigrations --check` clean after generate + migrate
- [ ] `seed_projects` ×2 idempotent (second run a no-op); the `--flush` path exercised children-first
- [ ] `temp/` smoke script (`temp/smoke_78.py`, the `smoke_73.py` sibling) as `admin_acme` / `password`
      (NOT the tenant-less `admin`):
      - [ ] every new `projects:*` url renders 200 with CONTENT assertions — the board's four columns +
            counts, a seeded card carrying priority/MoSCoW/assignee badges, the ready queue, the Gantt
            bars + critical highlight + today line + a conflict flag, the MoSCoW groups + the 2×2 quadrant
            labels + the work queue, a seeded `TCL-`/`TBK-` number, the checklist/block/dependency panels
            on task detail — and no `{#` / `{% comment` leaks (**L8**)
      - [ ] junk params `?project=0`, `?project=abc`, `?assignee=²`, `?start=nope`, `?end=nope`,
            `?page=9999`, `?active=nope` → default page, never a 500, never a silently emptied board
            (**L11** allow-list + `as_db_int` — **L35**)
      - [ ] page 2 of the checklist register; cross-tenant IDOR → 404 on every `<int:pk>` route
            (admin_globex pks)
      - [ ] POST-only verbs 405 on GET (`tsk_start`, `tsk_complete`, `tsk_block`, `tsk_unblock`,
            `tsk_bulk_update`, `tcl_check`); state machine holds: complete-while-blocked refused (both the
            dependency and the manual source), start-while-blocked refused, block-twice refused,
            unblock-twice refused, `tsk_execute` never touches plan fields
      - [ ] **gating: 7.8 verbs are ALL member-level (research L27)** — assert a plain member is NOT 403'd
            on the verbs (there is no tenant-admin gate this pass); a 403 check applies only if a gate is
            ever added
- [ ] Sidebar shows **7.8 Live** with all five bullets + the checklist-register leaf

## Close-out (Module Creation Sequence phases 4–7)

- [ ] Review agents, one after another, each appending to `.claude/tasks/review-projects-7.8.md`:
      `code-reviewer` → `explorer` → `frontend-reviewer` → `performance-reviewer` → `qa-smoke-tester` →
      `security-reviewer`
- [ ] `code-fixer` burns the deduped, ID'd findings (Critical → Important → Minor), one commit per file
- [ ] Tests: **`apps/projects/tests/conftest.py` is owned by the live 7.7 test session — do NOT touch it
      this pass**; the `taskwork_*` fixture block + `test_taskwork_{models,forms,views,security}.py` land
      only once the 7.7 test wave has landed (one file per commit, helpers `_taskwork_*`, no namespace
      shadowing), then **one full unfiltered app run** (never `-k`, **L47**; iterate with `--nomigrations`)
- [ ] `.claude/skills/projects/SKILL.md` — append the 7.8 section (the two models + prefixes, the
      in-place `ProjectTask` extension field list, the derived-property rulings incl.
      `is_dependency_blocked`'s FS/SS nuance, the verb table with gates/audit strings, the six url modules
      + the reused `tasks/` segment, the three computed pages + their context contracts, the seeder
      `_taskwork` block, gotchas incl. the empty `WIP_LIMITS` 7.19 hook and the TBK evidence-row no-form
      ruling) and update the frontmatter "As-built" line
- [ ] Mark 7.8 complete in `README.md` — the row reads **7 of 19**, with a 7.8 paragraph (what shipped:
      the in-place execution extension, the checklist/block registers, the three computed lenses; what it
      deliberately does NOT do: no second task table, no stored blocking, no write-back to CCA)

## Later passes / deferred (carried verbatim from the research so nothing is lost)

- [ ] **Multi-assignee M2M on tasks** — forks form/save for a differentiator; single assignee is the
      market norm (Jira; ClickUp guidance).
- [ ] **WIP-limit value storage + over-limit hard stop** — board configuration is master data → **7.19**;
      counts and badges ship now.
- [ ] **Sprint cadence / sprint boards / velocity** — **7.13** (Agile ceremonies) owns agile/scrum; the
      board is continuous flow (Ruling 4).
- [ ] **Auto-rescheduling (dependency date push) / resource levelling** — a scheduling engine →
      **7.17/7.16**; 7.2 shipped no scheduler. The Gantt page flags conflicts instead.
- [ ] **Computed priority scores (RICE/WSJF/custom weighted)** — framework definitions are master data →
      **7.19**; a stored score goes stale (the 7.4 EVM ruling).
- [ ] **Recurring tasks / bulk task duplication** — recurrence needs the scheduler → **7.17**; a copy verb
      is the fallback.
...
- [ ] **Drag-and-drop board JS / timeline arrow drawing** — no JS framework in the repo; POST verbs +
...
## 7.18 Integration & API Hub — build plan (Module 7, apps/projects)

**Mission.** 7.18 is the **connector + sync-run layer** of Module 7: a project-scoped register of connections to the five outside-system families NavERP.md §7.18 names (ERP/finance, CRM, HR/talent, DevOps, file storage), the **field mappings** that teach a connection which local project field answers which remote field, the **sync jobs** that scope a transfer, and the **append-only run log** recording what each sync did — **four models, one `domain` discriminator, zero outbound HTTP**. Nothing in this pass leaves the process: no transport, no worker, no HTTP client, and `ProjectSyncRun.status = "simulated"` is the honest status, so the model docstrings must say so out loud (the 7.17 / scm 4.19 / inventory 5.19 house posture). 7.17 owns event *push* (webhooks, `ProjectWebhookEndpoint` [`PWH-`]); 7.18 owns scheduled/triggered data *sync*, points at the 7.17 endpoint by FK for "notify on sync failure", and re-declares none of it.

**Driven by** `.claude/tasks/research-projects-7.18.md` (981 lines, Phase 1 of this sub-module; §4.4 = the RULING that 7.18 ships its own register, §4.5 = the 7.17 push-vs-sync boundary, §4.6 = the Fernet credential ruling, §5.0 = files/layout/migration, §5.1–§5.4 = the frozen field tables, §5.5 = the exact form `Meta.fields`, §5.6 = every url name + every view context key, §5.7 = the template list, §5.8 = `LIVE_LINKS["7.18"]`, §5.9 = seeder/admin/audit verbs, §6 = risks + `UNVERIFIED` items). Section numbers below are that file's.

**BASE:** re-run `git rev-parse HEAD` at the start of Phase 3 and save it (Phase 4 reviews `BASE...HEAD`). The working tree carries a sibling session's mid-flight 7.16 files (`apps/projects/views/_helpers.py`, `.claude/tasks/contract-projects-7.16.md`, the untracked 7.16 `DashboardWidgets.py` pair, `apps/core/models/Workflow.py` — research §7.2). They are **not ours**: never commit them, never "tidy" them, never full-rewrite a shared file — surgical `Edit` only (L43 / L45).


**As-built facts re-checked on disk 2026-09-20 — where the research disagrees with the code, the code wins (the contract phase needs these):**
- `apps/projects/forms/__init__.py` **DOES** carry a 7.17 re-export block (`from .WorkflowAutomation.…`, lines 183–199 of a 199-line file). Research §7.4 flagged this `UNVERIFIED`; it is now VERIFIED — 7.18's forms block appends after line 199.
- `apps/projects/views/__init__.py` (689 lines) likewise ENDS with the 7.17 block (`WorkflowAutomation.AutomationBoards`, lines 684–689) — 7.18's block appends after line 689. `apps/projects/models/__init__.py` is 174 lines and the 7.18 block goes after line 174, exactly as research §5.0 says.
- 7.17's templates on disk are **lowercase**: `templates/projects/workflowautomation/{workflow,approvalgate,recurringtask,webhook,boards}/…` (18 files, listed). Research §1.2/§6.1's claim that they sit under a PascalCase `WorkflowAutomation/` folder is **wrong** — so 7.18's lowercase disk folders COPY 7.17 instead of "deviating" from it, and the render strings must stay lowercase to match.
- **There is no `TENANT_SCOPED_FKS` attribute anywhere in `apps/projects`** (grep over `apps/projects/models/**` = 0 hits). Research §5.2/§5.3/§5.4 hand one to the mapping, the job and the run from the scm/inventory convention. The app's real tenant guard is a `clean()` re-check (`apps/projects/models/DocumentKnowledgeManagement/ProjectFolders.py:126` — `parent.tenant_id != tenant_id` -> `ValidationError`) plus `_reject_foreign(...)` at the form boundary. Do not invent the attribute and do not add one "for symmetry".
- `core.crud.crud_list/crud_detail/crud_edit` inject `object_list` / `obj` / `form` + `is_edit` only (verified `apps/core/crud.py:8–11`, `:180–183`, `:224–236`) — NOT the research's `connectors` / `connector` / `mapping` / `job` / `run`. 7.17's `views/WorkflowAutomation/Webhooks.py` hand-rolls its own context dicts for exactly this reason (manual `Paginator` at `:54–55`, own `render(...)` dict at `:57–75`). See §3.
- Seeder (verified): `_seed_tenant` calls `self._workflow_automation(tenant, now)` **last**, at `seed_projects.py:499`; that method (`:4138`) is the file's last method; the `--flush` block puts 7.17's six deletes at `:357–362`. The 7.17 seeder helper is named `_workflow_automation` — the brief's `_seed_717_*` does not exist (research §1.5 is right).
- `NavERP.md:1281` = `### 7.18 Integration & API Hub`; `:1282–1286` = the five bullet titles verbatim: `ERP & Financial System Sync`, `CRM Integration`, `HR & Talent Systems`, `Development & DevOps Tools`, `File Storage & Collaboration`.
- `integration/…`, `ixc_*`, `ixm_*`, `syj_*`, `syr_*` are free in the `projects` namespace: a `path("(integration|connectors|mappings|sync-jobs|runs…` grep over `apps/projects/urls/**` -> 0 hits, and the same for every proposed `name="…"` (re-verified 2026-09-20; research §1.4).

### 0. Contract first (freeze it before a single code file)

- [ ] `.claude/tasks/contract-projects-7.18.md` — frozen by the spec step and committed on its own BEFORE any code, pinning: every model field + every choice value (`DOMAIN_CHOICES` = `erp·crm·hris·devops·storage·custom`; `PROVIDER_CHOICES` = the 20 values `sap·oracle·netsuite·dynamics·workday·salesforce·hubspot·dynamics_sales·bamboohr·adp·jira·github·gitlab·azure_devops·ci_cd·sharepoint·google_drive·dropbox·box·custom`; `inbound·outbound·bidirectional`; `none·api_key·basic·oauth2·pat`; `manual·scheduled·event`; `production·sandbox`; `unverified·connected·error·disabled·disconnected`; `to_remote·from_remote·both`; `none·upper·lower·trim·date_iso·number·bool`; `SYNC_ENTITY_CHOICES` = `tasks·issues·risks·milestones·time_entries·resources·documents·folders·budgets·cost_lines·journals·custom`; `local_wins·remote_wins·newest_wins·manual`; `RUN_STATUS_CHOICES` = `pending·running·success·partial·failed·skipped·simulated`); every form `Meta.fields` + the declared extra `credential` field + the excluded lists; every url name and path (**31 route names, 26 view functions** — the five `ixc_*_list` category routes share `ixc_list` through Django's extra-options dict); **every view context key** (research §5.6 is the list — a key the view forgets renders 200 and blank, L8); every template path; the badge classes; the audit verbs; the seeder row counts; the `--flush` order; the migration number.
...
- [ ] Contract committed as its own file (`git add '.claude/tasks/contract-projects-7.18.md'; git commit -m '…'`) — one file per commit, PowerShell-safe `;` never `&&`, never `git push`.



### 1. Models — `apps/projects/models/IntegrationApiHub/`

- [ ] `apps/projects/models/IntegrationApiHub/__init__.py` — docstring only, NO re-exports (the 7.17 sub-package convention verbatim: *"Intentionally EMPTY of re-exports by convention: the package's public surface is the top-level `apps/projects/models/__init__.py` re-export block."*).
- [ ] `apps/projects/models/IntegrationApiHub/Connectors.py` — `class ProjectIntegrationConnector(TenantNumbered)`, `NUMBER_PREFIX = "IXC"`; `from apps.projects.models._base import *` + `from apps.core.crypto import decrypt, encrypt` (the `WorkflowAutomation/Webhooks.py:11` shape). Fields (research §5.1): `project` FK `"projects.Project"` CASCADE `null=True, blank=True, related_name="integration_connectors"` (null => workspace-wide, mirroring the verified `ProjectWebhookEndpoint.project` at `Webhooks.py:20–27`); `name` CharField(120); `domain` CharField(12) default `custom` — the 5-bullet discriminator; `provider` CharField(20) blank default `custom`; `direction` CharField(14) default `bidirectional`; `auth_method` CharField(10) default `api_key` (`oauth2` = recorded intent only — NO refresh-token columns, no token-refresh code); `base_url` CharField(500, blank) carrying the `# WARNING: SSRF` comment and deliberately NOT a `URLField` (git/ssh/on-prem hosts are not http); `remote_scope_ref` CharField(200, blank) (Jira project key, `org/repo`, SharePoint site/library id, SAP company code, NetSuite subsidiary, Salesforce org id); `trigger_mode` CharField(10) default `manual` with a help_text saying intent-only — nothing polls (copy 4.19's `trigger_mode` help_text); `schedule_note` CharField(200, blank); `environment` CharField(10) default `sandbox`; `status` CharField(12) default `unverified`; `is_active` BooleanField(True); `credential` CharField(512, blank) = the Fernet cipher; `last_sync_at` / `last_success_at` DateTimeField(null, blank, editable=False); `consecutive_failures` PositiveIntegerField(0, editable=False); `notify_webhook` FK `"projects.ProjectWebhookEndpoint"` SET_NULL null/blank `related_name="notify_connectors"` (7.17 owns delivery — 7.18 declares no `target_url`, no `secret`, no `event_types`, no signing helper, §4.5); `owner` FK `settings.AUTH_USER_MODEL` SET_NULL null/blank `related_name="owned_project_connectors"`; `notes` TextField(blank).
- [ ] `…/Connectors.py` — the credential choke point + derived properties + `Meta`: `set_credential(raw)` / `get_credential()` / `credential_masked` (degrades, never raises) / `credential_set` / `status_badge` / `domain_badge` / `health_badge` (colour-named classes only); `save()` performs `self.credential = encrypt(self.credential or "")` as the SINGLE choke point (idempotent on a marked value — `crm.Webhook.save()`'s exact pattern); `clean()` re-checks `project` and `notify_webhook` tenancy (`chosen.tenant_id != self.tenant_id` -> `ValidationError` — the `ProjectFolders.py:126` pattern — plus the `project IS NULL` name-duplicate check the DB constraint cannot express, research §6.3); `Meta`: `ordering = ["-created_at", "-id"]`, `unique_together = [("tenant", "number"), ("tenant", "project", "name")]`, indexes `ixc_tnt_domain_idx` (tenant, domain), `ixc_tnt_status_idx` (tenant, status), `ixc_tnt_prj_idx` (tenant, project), `ixc_tnt_active_idx` (tenant, is_active) — every name <=30 chars; `__str__` = `f"{self.number} — {self.name} ({self.get_provider_display()})"`.
- [ ] `…/Connectors.py` docstring must say, in words: (1) 7.18 **records and configures** and runs NO transport — `simulated` is the honest status, and no outbound HTTP exists in this build; (2) deleting a `Project` CASCADEs connectors -> jobs -> runs **deliberately** (a project's connector is project configuration — do not "fix" it to SET_NULL and silently promote a deleted project's Jira connector to workspace-wide, research §6.3); (3) the L36 ruling: this register coexists with scm 4.19 `IntegrationEndpoint` [`CNX-`], inventory 5.19 `IntegrationChannel` [`INT-`] and accounting 2.15 `IntegrationConfig` **on purpose** — four integration registers, four owners.

- [ ] `apps/projects/models/IntegrationApiHub/FieldMappings.py` — `class ConnectorFieldMapping(TenantOwned)` (unnumbered — the `ChannelListingMap` precedent): `connector` FK `ProjectIntegrationConnector` CASCADE `related_name="mappings"`; `local_field` CharField(100) (e.g. `task.status`, `client.name`, `document.title`, `time_entry.hours`); `remote_field` CharField(100) (e.g. `fields.status.name`, `Account.Name`, `path`, `timeSpentSeconds`); `direction` CharField(14) `to_remote·from_remote·both` default `both`; `transform` CharField(12) `none·upper·lower·trim·date_iso·number·bool` default `none`; `value_map` JSONField(default=dict, blank); `default_value` CharField(255, blank); `is_key` BooleanField(False) (the match/identity key — de-dupe + idempotency); `is_required` BooleanField(False); `notes` CharField(255, blank). `Meta`: `ordering = ["connector__name", "local_field", "id"]`, `unique_together = [("tenant", "connector", "local_field", "direction")]`, indexes `ixm_tnt_conn_idx` (tenant, connector), `ixm_tnt_conn_key_idx` (tenant, connector, is_key); `clean()` repeats the connector tenant guard; `__str__` = `f"{self.local_field} -> {self.remote_field}"`.
- [ ] `apps/projects/models/IntegrationApiHub/SyncJobs.py` — `class ProjectSyncJob(TenantNumbered)`, `NUMBER_PREFIX = "SYJ"`: `connector` FK `ProjectIntegrationConnector` CASCADE `related_name="jobs"` with **no second `project` column** (the job's project scope is read THROUGH its connector — one source of truth); `name` CharField(255); `entity_scope` CharField(16) `SYNC_ENTITY_CHOICES` default `custom`; `direction` CharField(14) (may differ from the connector's); `trigger_mode` CharField(10) (intent only); `interval_minutes` PositiveIntegerField(null, blank) (recorded cadence — nothing schedules on it); `schedule_note` CharField(200, blank); `filter_expression` TextField(blank) (recorded, NEVER evaluated — 4.19's precedent); `conflict_policy` CharField(12) `local_wins·remote_wins·newest_wins·manual` default `manual`; `batch_size` PositiveIntegerField(100); `is_active` BooleanField(True); `last_run_at` / `next_run_at` DateTimeField(null, blank, editable=False) (`next_run_at` is a stamp, not a trigger); `run_count` PositiveIntegerField(0, editable=False); `last_status` CharField(10, blank, editable=False). `Meta`: `ordering = ["-created_at", "-id"]`, `unique_together = [("tenant", "number"), ("tenant", "connector", "name")]`, indexes `syj_tnt_conn_idx` (tenant, connector), `syj_tnt_conn_act_idx` (tenant, connector, is_active), `syj_tnt_status_idx` (tenant, last_status); `__str__` = `f"{self.number} — {self.name}"`.
- [ ] `apps/projects/models/IntegrationApiHub/SyncRuns.py` — `class ProjectSyncRun(TenantNumbered)`, `NUMBER_PREFIX = "SYR"`, **append-only** (list + detail + `syr_retry` only; no form, no create/edit/delete url): `job` FK `ProjectSyncJob` CASCADE `related_name="runs"`; `direction` CharField(14) (a snapshot of the job's at run time); `status` CharField(10) `RUN_STATUS_CHOICES` default `pending` (**`simulated` is mandatory honesty** — nothing in this build leaves the process); `trigger_source` CharField(10) `manual·schedule·event` default `manual`; `triggered_by` FK `settings.AUTH_USER_MODEL` SET_NULL null/blank `editable=False` `related_name="+"`; the counters `records_read` / `records_created` / `records_updated` / `records_skipped` / `records_failed`, each PositiveIntegerField(0) (**counts, never one row per record**); `error_code` CharField(50, blank); `error_message` TextField(blank); `payload_excerpt` TextField(blank, **truncated** — may contain partner PII); `attempt_no` PositiveSmallIntegerField(1); `next_retry_at` DateTimeField(null, blank) (a stamp nothing wakes up and reads); `started_at` DateTimeField(default=timezone.now, editable=False); `finished_at` DateTimeField(null, blank, editable=False); `duration_ms` PositiveIntegerField(0, editable=False).
- [ ] `…/SyncRuns.py` — `record()` **classmethod as the ONLY writer** (the `StockSyncRun.record()` precedent: the run verb, the retry verb and the seeder all go through it — the seeder must NEVER bare-`.create()` a run, research §5.9); `SYNC_BACKOFF_SECONDS = (0, 5, 300, 1800, 7200, 18000, 36000, 36000)` (Svix's 8 slots, adopted verbatim as `scm`'s `DELIVERY_BACKOFF_SECONDS` / `inventory`'s `SYNC_BACKOFF_SECONDS` — carry the §7.4 `UNVERIFIED` note that it was not re-fetched in this run); `Meta`: `ordering = ["-started_at", "-id"]`, `unique_together = [("tenant", "number")]`, indexes `syr_tnt_job_stat_idx` (tenant, job, status), `syr_tnt_stat_strt_idx` (tenant, status, started_at), `syr_tnt_job_strt_idx` (tenant, job, started_at); `__str__` = `f"{self.number} — {self.get_status_display()} @ {self.started_at:%Y-%m-%d %H:%M}"`; `status_badge`: success -> `badge-green`, partial -> `badge-amber`, failed -> `badge-red`, simulated -> `badge-info`, pending/running -> `badge-slate`, skipped -> `badge-muted`; the retry docstring lives here (status -> `pending`, `attempt_no += 1`, `next_retry_at` from the backoff tuple, **performs no HTTP request**).


### 2. Forms — `apps/projects/forms/IntegrationApiHub/`

- [ ] `apps/projects/forms/IntegrationApiHub/__init__.py` — docstring only, no re-exports (7.17's `forms/WorkflowAutomation/__init__.py` is a one-line docstring), **and it carries the deliberate-absence note** for the run form: `ProjectSyncRun` is append-only and its only writer is the model's `record()` classmethod, so no `ProjectSyncRunForm` exists — the `IntegrationMessages.py:26–30` / `StockSyncRuns.py` posture (research §5.4/§5.5).
- [ ] `apps/projects/forms/IntegrationApiHub/Connectors.py` — `ProjectIntegrationConnectorForm(TenantUniqueMixin, TenantModelForm)` with `Meta.fields` **exactly** `["project", "name", "domain", "provider", "direction", "auth_method", "base_url", "remote_scope_ref", "trigger_mode", "schedule_note", "environment", "status", "is_active", "notify_webhook", "owner", "notes"]` (`number`, `credential`, `last_sync_at`, `last_success_at`, `consecutive_failures` excluded by construction); a declared **extra** field `credential = forms.CharField(required=False, widget=forms.PasswordInput(render_value=False, attrs={"class": "form-input"}), help_text="Paste the provider token/API key. Stored encrypted; leave blank on edit to keep the current one.")`; `clean()` -> `_reject_foreign(self, cleaned, ["project", "notify_webhook"])` — **NOT `owner`** (a superuser row may carry `tenant=None`; the view scopes the owner queryset instead — 7.17's restraint); a blank `credential` on edit must leave the stored cipher alone (`encrypt()` is idempotent, `apps/core/crypto.py:71–82`, so re-assigning the stored value is safe); a `clean_name` duplicate check covering the `project IS NULL` rows the DB `unique_together` cannot bind (research §6.3).
- [ ] `…/Connectors.py` — `ConnectorTestForm(forms.Form)` (a plain form, NOT a `ModelForm`): the `test_form` the `ixc_detail` context key names (the `WebhookTestPingForm` precedent) — an optional note/payload field the `ixc_test` verb reads when composing the `simulated` run's `payload_excerpt`. No `ModelChoiceField` over a sibling model here, so nothing to tenant-scope.
- [ ] `apps/projects/forms/IntegrationApiHub/FieldMappings.py` — `ConnectorFieldMappingForm(TenantUniqueMixin, TenantModelForm)` with `Meta.fields` **exactly** `["connector", "local_field", "remote_field", "direction", "transform", "value_map", "default_value", "is_key", "is_required", "notes"]`; `clean()` -> `_reject_foreign(self, cleaned, ["connector"])`; `clean_value_map` parses the JSON textarea into a dict (7.17's `clean_custom_headers` pattern) and raises a field error on invalid JSON instead of 500ing.
- [ ] `apps/projects/forms/IntegrationApiHub/SyncJobs.py` — `ProjectSyncJobForm(TenantUniqueMixin, TenantModelForm)` with `Meta.fields` **exactly** `["connector", "name", "entity_scope", "direction", "trigger_mode", "interval_minutes", "schedule_note", "filter_expression", "conflict_policy", "batch_size", "is_active"]` (`number`, `last_run_at`, `next_run_at`, `run_count`, `last_status` excluded); `clean()` -> `_reject_foreign(self, cleaned, ["connector"])`; help_texts on `interval_minutes` and `filter_expression` repeating the "recorded intent — nothing runs this" rule.
- [ ] **No `ProjectSyncRunForm`** — do not create one "for completeness", do not give `ProjectSyncRun` a create/edit/delete route, and do not let the seeder or any view bypass `record()`; the absence is a documented exemption (CRUD Completeness rule's exemption clause), not an oversight.


### 3. Views — `apps/projects/views/IntegrationApiHub/`

- [ ] `apps/projects/views/IntegrationApiHub/__init__.py` — docstring only, no re-exports (7.17's sub-package convention).
- [ ] **The context-source ruling — apply it in every entity module (verified necessity):** `core.crud.crud_list` / `crud_detail` / `crud_edit` inject `object_list` / `obj` / `form` + `is_edit` only (`apps/core/crud.py`), so every 7.18 page whose contract key is `connectors` / `mappings` / `jobs` / `runs` / `connector` / `mapping` / `job` / `run` must be **hand-rolled** exactly like `views/WorkflowAutomation/Webhooks.py` — `get_object_or_404(<Model>, pk=pk, tenant=request.tenant)`, filters parsed BEFORE the manual `Paginator(qs, 25)`, and a view-owned `render(...)` context dict. A view that injects `object_list` while its template reads `connectors` returns **200 and renders blank** (L8). `crud_create` may be used ONLY where the contract's keys are exactly `form` (+`is_edit`); no page may be left speaking both vocabularies.
- [ ] `apps/projects/views/IntegrationApiHub/Connectors.py` — the six **list** entry points: `ixc_list(request, domain=None)` (`@login_required`) plus the five category routes `ixc_erp_list` / `ixc_crm_list` / `ixc_hris_list` / `ixc_devops_list` / `ixc_storage_list` resolving to that ONE function through Django's extra-options dict `{"domain": "erp"|"crm"|"hris"|"devops"|"storage"}` (research §5.0/§5.6). Filters parsed before pagination: `q` over `number`/`name`/`provider`/`remote_scope_ref`, `status`, `provider`, `project` (pk — the template compares with `|stringformat:"d"`), `is_active`, plus the view-kwarg `domain` (the template echoes `domain` — **never** `request.GET.domain`). Context **exactly**: `connectors` (= `page_obj.object_list`), `page_obj`, `projects`, `domain_choices` (**all** `DOMAIN_CHOICES`), `provider_choices` (**all** `PROVIDER_CHOICES`), `status_choices`, `domain`, `q`, `status`, `provider`, `project_id`, `is_active`, and `stats` = `{total, active, connected, error, unverified, due_rotation}` (pin the rotation window for `due_rotation` in the contract).
- [ ] `…/Connectors.py` — `ixc_detail(request, pk)` (`@login_required`): context **exactly** `connector`, `mappings` (first 20), `jobs`, `recent_runs` (15), `test_form` (`ConnectorTestForm`), `revealed_credential`; the reveal is **session-once**: `reveal_data = request.session.pop("_ixc_cred_reveal", None)` and it is surfaced only when `reveal_data.get("pk") == connector.pk` — the verified `_whk_secret_reveal` pattern (`views/WorkflowAutomation/Webhooks.py:87–90`, re-set at `:114` and `:276`).
- [ ] `…/Connectors.py` — `ixc_create` / `ixc_edit` (`@login_required`): context `form` (+ `connector` on edit, so both pages share `connector/form.html`); a non-blank posted `credential` routes through `connector.set_credential(raw)` before `save()` (`save()` stays the second, idempotent choke point); `write_audit_log(user=request.user, obj=…, action="create"|"update", changes={…}, tenant=request.tenant)` and **never** a credential value inside `changes`.
- [ ] `…/Connectors.py` — the four POST-only verbs, each `@login_required` + `@require_POST`: `ixc_delete` (also `@tenant_admin_required`; `action="delete"`), `ixc_rotate_credential` (also `@tenant_admin_required`; writes a fresh raw credential, sets the one-shot `_ixc_cred_reveal` session key, `action="rotate"`, redirects to `ixc_detail`), `ixc_test` (`action="test"`; writes **no** HTTP and no bytes — mints a `ProjectSyncRun` with `status="simulated"` through the model's `record()` classmethod, then `messages.success(...)` + redirect to `ixc_detail`), `ixc_toggle_active` (`action="toggle"`, flips `is_active`).
- [ ] `…/Connectors.py` — `connector_health(request, pk)` (`@login_required`, GET lens at `integration/connectors/<int:pk>/health/`): context **exactly** `connector`, `runs` (last 20), `mappings`, `jobs`, `stats` = `{success_rate, avg_duration_ms, failures_7d, last_success_at, mappings_count, credential_set}` — with deep links out to `syr_list?job=<pk>` and `ixm_list?connector=<pk>`.

- [ ] `apps/projects/views/IntegrationApiHub/FieldMappings.py` — `ixm_list` (filters `q` over `local_field`/`remote_field`, `connector` pk, `direction`; context **exactly** `mappings`, `page_obj`, `connectors`, `connector_id`, `direction_choices`, `transform_choices`, `q`), `ixm_detail` (context `mapping`), `ixm_create` / `ixm_edit` (context `form`, + `mapping` on edit), `ixm_delete` (`@login_required` + `@require_POST`, audit `action="delete"`).
- [ ] `apps/projects/views/IntegrationApiHub/SyncJobs.py` — `syj_list` (filters `q` over `number`/`name`/`schedule_note`, `connector` pk, `entity_scope`, `is_active`; context **exactly** `jobs`, `page_obj`, `connectors`, `entity_choices`, `trigger_choices`, `conflict_choices`, `status_choices`, `q`, `connector_id`, `entity_scope`, `is_active`, `stats` = `{total, active, due, failing}`); `syj_detail` (context **exactly** `job`, `runs` (15), `stats` = `{run_count, success_rate, last_status, mapping_count}`); `syj_create` / `syj_edit` (context `form`, + `job` on edit); `syj_delete` (`@require_POST`); `syj_toggle_active` (`@require_POST`, `action="toggle"`).
- [ ] `…/SyncJobs.py` — `syj_run(request, pk)` (`@login_required` + `@require_POST`): stamps `last_run_at`, increments `run_count`, sets `last_status`, and records the run with `status="simulated"` through `ProjectSyncRun.record()` — **no HTTP, no worker, no subprocess** (the docstring says so); audit `action="run"`; then redirects to `syj_detail`.
- [ ] `apps/projects/views/IntegrationApiHub/SyncRuns.py` — `syr_list` (filters `job` pk, `connector` pk, `status`, `trigger_source`, and `date_from`/`date_to` **widened into a datetime range** so `(tenant, started_at)` is usable — the `scm.IntegrationMessage` list precedent; context **exactly** `runs`, `page_obj`, `jobs`, `connectors`, `status_choices`, `trigger_choices`, `job_id`, `connector_id`, `status`, `trigger_source`, `date_from`, `date_to`, `q`, `stats` = `{pending, running, failed, partial, simulated, failed_today}`), `syr_detail` (context `run`), `syr_retry` (`@login_required` + `@require_POST` + `@tenant_admin_required`: status -> `"pending"`, `attempt_no += 1`, `next_retry_at` stamped from `SYNC_BACKOFF_SECONDS`, audit `action="retry"`, **performs no HTTP request**). No create/edit/delete view exists for runs.
- [ ] `apps/projects/views/IntegrationApiHub/HubBoards.py` — `integration_hub` (`/integration/hub/`: context **exactly** `stats` = `{connectors_total, by_domain, by_status, runs_today, failed_today, jobs_active, mappings_total, credentials_due}`, `domains` (list of dicts `value`/`label`/`total`/`connected`/`failing`), `connectors` (health rows), `recent_runs`, `projects`) and `sync_monitor` (`/integration/monitor/`: filters `q`, `status`, `job` pk, `date_from`/`date_to`; context **exactly** `runs`, `page_obj`, `jobs`, `connectors`, `status_choices`, `stats`, `q`, `status`, `job_id`, `date_from`, `date_to`).
- [ ] Every list view passes **both** the `*_choices` constants and the parent querysets its filters need (Filter Implementation Rules — the app's recurring bug; `syj_list` needs `entity_choices`, `trigger_choices` and `conflict_choices`, `syr_list` needs `status_choices` and `trigger_choices`), and every template's post-pagination block guards with `page_obj.has_previous` / `page_obj.has_next` (L9).

### 4. URLs — `apps/projects/urls/IntegrationApiHub/`

- [ ] `apps/projects/urls/IntegrationApiHub/Connectors.py` — the 14 routes with **literal-first ordering**: `integration/connectors/` -> `ixc_list`, `integration/connectors/add/` -> `ixc_create` (the literal `add/` BEFORE `<int:pk>/`), then the five category literals `erp/`, `crm/`, `hris/`, `devops/`, `storage/` (each `views.ixc_list` + its extra-options dict) BEFORE `integration/connectors/<int:pk>/` -> `ixc_detail`, then `<int:pk>/edit/` -> `ixc_edit`, `<int:pk>/delete/` -> `ixc_delete`, `<int:pk>/rotate-credential/` -> `ixc_rotate_credential`, `<int:pk>/test/` -> `ixc_test`, `<int:pk>/toggle/` -> `ixc_toggle_active`, and `<int:pk>/health/` -> `connector_health` (id + slash exactly as research §5.6 prints them).
- [ ] `apps/projects/urls/IntegrationApiHub/FieldMappings.py` — `integration/mappings/` -> `ixm_list`, `integration/mappings/add/` -> `ixm_create`, `<int:pk>/` -> `ixm_detail`, `<int:pk>/edit/` -> `ixm_edit`, `<int:pk>/delete/` -> `ixm_delete`.
- [ ] `apps/projects/urls/IntegrationApiHub/SyncJobs.py` — `integration/sync-jobs/` -> `syj_list`, `add/` -> `syj_create`, `<int:pk>/` -> `syj_detail`, `<int:pk>/edit/` -> `syj_edit`, `<int:pk>/delete/` -> `syj_delete`, `<int:pk>/run/` -> `syj_run`, `<int:pk>/toggle/` -> `syj_toggle_active`.
- [ ] `apps/projects/urls/IntegrationApiHub/SyncRuns.py` — `integration/runs/` -> `syr_list`, `integration/runs/<int:pk>/` -> `syr_detail`, `integration/runs/<int:pk>/retry/` -> `syr_retry`; **no create/edit/delete path exists** for runs (append-only by construction).
- [ ] `apps/projects/urls/IntegrationApiHub/HubBoards.py` — `integration/hub/` -> `integration_hub`, `integration/monitor/` -> `sync_monitor`.
- [ ] **No greedy `<str:…>` converter anywhere in the new modules** (the whole-app invariant, `urls/__init__.py:12–15`), every first segment is a whole literal component, and the `integration/` first segment is free — re-run the grep at build time (`path\("(integration|connectors|mappings|sync-jobs|runs` over `apps/projects/urls/**` -> 0 hits on 2026-09-20, and 0 hits for the new `name="…"` values).



### 5. Templates — `templates/projects/integrationapihub/<entity>/<page>.html` (lowercase disk folders on purpose, research §6.1 — they COPY 7.17's lowercase `templates/projects/workflowautomation/…`, not a PascalCase deviation; render strings stay lowercase to match)

- [ ] Every page extends `base.html` and uses the design-system classes from `static/css/theme.css` (`.page-header`/`.page-title`/`.breadcrumb`/`.page-actions`; `.card`/`.card-header`/`.card-body`; `.btn`/`.btn-primary`/`.btn-outline`/`.btn-danger`/`.btn-icon`; `.badge`/colour-named only `badge-green`/`badge-red`/`badge-amber`/`badge-info`/`badge-muted`/`badge-slate` — L33; `.table-wrap`/`.table`/`.table-actions`; `.form-group`/`.form-label`/`.form-input`/`.form-select`/`.form-textarea`/`.form-error`; `.stat-card`; `.empty-state`; `.pagination`). Icons: `<i data-lucide="NAME"></i>` (list actions eye / pencil / trash-2). Never invent a modifier class — confirm it exists in `theme.css` or copy a 7.17 sibling's line verbatim.
- [ ] `templates/projects/integrationapihub/connector/{list,detail,form}.html` — list: GET filter card (search `q`, `status` select, `provider` select, `project` FK select with `|stringformat:"d"`, `is_active`), the `stats` strip, a table with `number`/`name`/`domain`/`provider`/`direction`/`status` badges (`status_badge`/`domain_badge`), the `is_active` marker, `credential_set` as a "credential set"/"none" chip (**never** the value), and an Actions column (eye -> `ixc_detail`, pencil -> `ixc_edit`, trash-2 POST+confirm+csrf -> `ixc_delete`). The five category routes share this one template and echo the **view kwarg `domain`** (never `request.GET.domain`) in the header + active tab. Detail: header badges, the Actions sidebar (Edit, Delete POST+confirm+csrf conditional, Back to list), panels for the 15 `recent_runs`, the `jobs` and `mappings` lists, the `test_form` (a plain `forms.Form` -> `ixc_test`), and the session-once `revealed_credential` banner ("Reveal once — stored encrypted", cleared from session after one read — 7.17's `_whk_secret_reveal` pattern). Form: shared create/edit, `form` + (`is_edit` + `connector` on edit), `csrf_token`, per-field `.form-error`, the write-only `credential` `PasswordInput(render_value=False)` with its "leave blank on edit to keep the current one" help text.
- [ ] `templates/projects/integrationapihub/mapping/{list,detail,form}.html` — list: filters (`q`, `connector` FK select `|stringformat:"d"`, `direction`, `transform`), a table with `connector` (link to `ixc_detail`), `local_field -> remote_field`, `direction`/`transform` badges, `is_key`/`is_required` flags, Actions column; pagination; `.empty-state`. Detail: header + Actions sidebar + the `value_map`/`default_value`/`notes` panel. Form: `connector` FK select, `value_map` as a JSON textarea with the parse-error surface, `direction`/`transform` selects, `is_key`/`is_required` checkboxes.
- [ ] `templates/projects/integrationapihub/syncjob/{list,detail,form}.html` — list: filters (`q`, `connector` pk `|stringformat:"d"`, `entity_scope`, `trigger_mode`, `conflict_policy`, `is_active`), `stats` strip, table with `number`/`name`/`connector`/`entity_scope`/`conflict_policy`/`last_status` badges, `run_count`/`last_run_at`, Actions column + a per-row "Run now" POST+csrf -> `syj_run` and an active toggle POST -> `syj_toggle_active`. Detail: header + Actions sidebar + the job's recent `runs` (linked to `syr_detail`). Form: the full `Meta.fields` set with `interval_minutes`/`batch_size` numeric inputs and `filter_expression` as a read-only-intent textarea whose help text says "recorded, never evaluated".
- [ ] `templates/projects/integrationapihub/syncrun/{list,detail}.html` — **no form.html** (append-only). List: filters (`q`, `status`, `trigger_source`, `job` pk, `connector` pk, `date_from`/`date_to` -> widened datetime range), `stats` strip, table with `number`/`job`/`direction`/`status` badge (`status_badge`)/`records_*` counters/`duration_ms`/`attempt_no`/`started_at`, Actions column (eye -> `syr_detail`; a "Retry" POST+csrf+admin -> `syr_retry` shown only on `failed`/`partial`). Detail: header badges, the counters grid, `error_code`/`error_message`/`payload_excerpt` (truncated, labelled "may contain partner data"), the `next_retry_at` stamp labelled "recorded schedule — nothing runs automatically", `triggered_by`/`trigger_source`, and the Actions sidebar (Retry on failed/partial; Back to list).


### 6. Integrate (single writer, the only DB writer)

- [ ] Verify every expected file landed (the backend `.py` set + the 7.18 sub-package `__init__.py`s + the 12 templates) BEFORE wiring anything.
- [ ] `apps/projects/{models,forms,views,urls}/IntegrationApiHub/__init__.py` — docstring only, deliberately EMPTY of re-exports (7.17's convention); the forms one carries the deliberate-absence note for `ProjectSyncRunForm`.
- [ ] `apps/projects/models/__init__.py` — append the `# --- 7.18 Integration & API Hub ---` block after line 174: `from .IntegrationApiHub.Connectors import ProjectIntegrationConnector`, `.FieldMappings import ConnectorFieldMapping`, `.SyncJobs import ProjectSyncJob`, `.SyncRuns import ProjectSyncRun`.
- [ ] `apps/projects/forms/__init__.py` — append the 7.18 block after line 199: the three forms (NOT a run form).
- [ ] `apps/projects/views/__init__.py` — append the 7.18 block after line 689: all 26 view functions.
- [ ] `apps/projects/urls/__init__.py` — append the five `from .IntegrationApiHub.X import urlpatterns as _ih_x` imports and add them to the concatenation list AFTER the 7.17 entries (literal-first within each module preserved).
- [ ] `apps/projects/admin.py` — register the four models mirroring the 7.17 block (`admin.py:850–905`): `list_display`/`list_filter`/`search_fields`/`readonly_fields`; **never** put `credential` in any rendering list.
- [ ] `apps/projects/management/commands/seed_projects.py` — new `_integration_hub(self, tenant, now)` (guard `if ProjectIntegrationConnector.objects.filter(tenant=tenant).exists()`), called in `_seed_tenant` AFTER `self._workflow_automation(tenant, now)` (`:499`); creates the rows in research §5.9 (7 connectors / 30 mappings / 10 jobs / 45 runs, including a Jira `milestones` job — runs through `record()`, never bare `.create()`); extends the `--flush` block with the children-first deletes (`ProjectSyncRun` -> `ProjectSyncJob` -> `ConnectorFieldMapping` -> `ProjectIntegrationConnector`) inserted ABOVE 7.17's webhook deletes (`:357–362`); extends the imports; prints a per-tenant summary line.
- [ ] `apps/core/navigation.py` — ONE `LIVE_LINKS["7.18"]` block immediately after the `"7.17"` block (`:2040`), keys character-for-character from NavERP.md §7.18: `ERP & Financial System Sync` -> `projects:ixc_erp_list`; `CRM Integration` -> `projects:ixc_crm_list`; `HR & Talent Systems` -> `projects:ixc_hris_list`; `Development & DevOps Tools` -> `projects:ixc_devops_list`; `File Storage & Collaboration` -> `projects:ixc_storage_list`; extra live leaves `Integration Hub` -> `projects:integration_hub`, `Sync Monitor` -> `projects:sync_monitor`, `Connector Register` -> `projects:ixc_list`, `Field Mappings` -> `projects:ixm_list`, `Sync Jobs` -> `projects:syj_list`, `Sync Runs` -> `projects:syr_list`. (The L36 ownership comment naming the four coexisting registers is the MAIN SESSION's line, §9.)
- [ ] `apps/projects/views/ProjectInitiation/Overview.py` — add the three 7.18 context keys the overview template reads (`integration_connector_count`, `sync_runs_failed_today`, `sync_runs_today`), tenant-scoped, computed in Python (the `open_defect_count` shape) so a cross-app aggregate never lands in a hot loop.
- [ ] `README.md` — mark 7.18 complete in the Module 7 roadmap (Phase 7's job; listed here so the integrate pass leaves it unchecked).
- [ ] `config/settings.py` / `config/urls.py` — **NOT touched** (the app already exists; adding them is a bug).


### 7. Migrate + seed + verify (venv python, PowerShell-safe `;`)

- [ ] `venv\Scripts\python.exe manage.py makemigrations projects` -> expect `0026_…`; if a sibling session took `0026`, take the next free number and amend the CONTRACT (not the code).
- [ ] `venv\Scripts\python.exe manage.py migrate`
- [ ] `venv\Scripts\python.exe manage.py seed_projects` (run twice — the 2nd run must print the idempotent-skip line and create no new rows)
- [ ] `venv\Scripts\python.exe manage.py check` -> clean
- [ ] Smoke script in `temp/` (gitignored): `force_login(admin_acme)` with `ALLOWED_HOSTS=['testserver',…]`; GET every new url name via `reverse` (sample a pk per model) asserting status 200/302; assert no `{#`/`{% comment` leak markers; assert a seeded `IXC-`/`SYJ-`/`SYR-` record renders on its list page (content, not just status, L8); junk-param list (`?page=abc&status=nope`) -> 200; page 2 (`?page=2`) -> 200; cross-tenant IDOR: as `admin_acme` GET `admin_globex`'s connector/mapping/job/run pk -> **404**. Fix any drift against the contract.
- [ ] POST smoke: `ixc_test` and `syj_run` write a `simulated` `ProjectSyncRun` through `record()` and perform **no** HTTP; `syr_retry` on a failed run flips status to `pending` + bumps `attempt_no` + stamps `next_retry_at`; `ixc_rotate_credential` re-encrypts and the old `decrypt` output no longer matches; the session-once reveal appears exactly once.


### 8. Review -> code-fixer -> tests -> skill/README (Phases 4–7, strictly serial, one agent at a time)

- [ ] Phase 4 review — six reviewers one after another (`code-reviewer` -> `explorer` -> `frontend-reviewer` -> `performance-reviewer` -> `qa-smoke-tester` -> `security-reviewer`), each reading `BASE...HEAD` (the sha saved in §0) and appending to `.claude/tasks/review-projects-7.18.md`; dedupe, sort Critical -> Important -> Minor, assign IDs, commit the file.
- [ ] Phase 5 `code-fixer` — fixes every finding in ID order, verifies each, one commit per file, marks each `[x] fixed` / `[~] skipped — reason`; the main session does not apply findings itself.
- [ ] Phase 6 tests — one agent pins the test contract + writes `tests/__init__.py` + `conftest.py`; then `test_integrationapihub_models.py` -> `test_integrationapihub_forms.py` -> `test_integrationapihub_views.py` -> `test_integrationapihub_security.py` one at a time (every test named `test_integrationapihub_*`, helpers `_integrationapihub_*`); commit each; finish with the **full unfiltered** `apps/projects` suite green (never `-k`, L47).
- [ ] Phase 7 — update `.claude/skills/projects/SKILL.md` with the 7.18 models/routes/templates/seeder rows and `README.md` marking 7.18 complete; one file per commit; never `git push`.


### 9. L36 ownership ruling — the three durable homes (the MAIN SESSION writes the navigation + lessons lines; the build writes the docstring)

- [ ] Model docstring in `apps/projects/models/IntegrationApiHub/Connectors.py` records: four integration registers coexist deliberately — scm 4.19 `IntegrationEndpoint` [`CNX-`] (supply-chain, transport-less), inventory 5.19 `IntegrationChannel` [`INT-`] (commerce-stock), accounting 2.15 `IntegrationConfig` (finance), **this** model `ProjectIntegrationConnector` [`IXC-`] (project-scoped) — plus the sentence "7.18 performs no outbound HTTP and writes no accounting row."
- [ ] `apps/core/navigation.py` comment at the new `"7.18"` `LIVE_LINKS` entry naming the split and the "five domain leaves are five category-scoped routes onto the one project-scoped register" mapping (the `navigation.py:2044–2059` omission-comment precedent). **Main session's line.**
- [ ] `.claude/tasks/lessons.md` new entry recording the ruling + the do-not-reuse list (class names + 4.19 columns that must not be shadowed), so a future pass does not "consolidate" the registers. **Main session's line.**


### Review

- [x] Review findings recorded in `.claude/tasks/review-projects-7.18.md` (six serial lanes, deduped and fixer-marked; the file is Phase 4's deliverable).


- [x] `templates/projects/integrationapihub/boards/{integration_hub,sync_monitor,connector_health}.html` — standalone pages (no entity folder). `integration_hub`: the `stats` strip, a `domains` five-panel grid (value/label/total/connected/failing, each linking to its `ixc_<domain>_list`), the `connectors` health rows, and `recent_runs`. `sync_monitor`: filter card + the heat table + `stats`. `connector_health` (reached from `ixc_detail` and the hub): the one connector's status/last_success/failure-counters/`recent_runs`/jobs/mappings. Each links back to the register it reports on.
- [x] `templates/projects/overview.html` — 7.18 quick-link rows and stat cards are present with the tenant-scoped `integration_connector_count`, `sync_runs_failed_today`, and `sync_runs_today` context keys.
...
- [x] Migration-enabled verification passes: the full migration graph applies successfully through `projects.0027` (including HRM `0049`) using a temporary file-backed SQLite test database; no old migration defect was found, so no migration file was changed.
- [x] The migration-backed 253-test 7.18 lane and the full unfiltered `apps/projects` suite pass using the reusable migrated database. The temporary settings silenced only the concurrent 7.19 admin-check IDs; no 7.19 files were changed.
- [x] README, NavERP status, test contract, and `.claude/skills/projects/SKILL.md` now describe 7.18 as built.
- [x] 7.18 changes are committed one file at a time; no push was performed.
...
Migration: the next free `sales` migration after the current `0003` leaf, assigned only during Integrate after the concurrent tree is re-read; it is not reserved in this plan.
...
- [ ] Reuse `crm.HealthScore` and `crm.HealthScoreHistory` as the only stored account-health truth. The 8.3 workspace may read the current score, trend, and explainable inputs; it must not create a Sales health score, history ledger, or silently recompute CRM health.
- [ ] Reuse `crm.Opportunity` for account-plan links, pipeline context, and revenue potential. Reuse `scm.SalesOrder` and `scm.SalesOrderLine` read-only for commercial history and `accounting.Invoice`/`accounting.InvoiceLine` read-only for realized revenue. Do not add a Sales opportunity, order, product, or financial ledger.
- [ ] Reuse `core.PartyRelationship` for generic corporate and reporting-line edges, `core.Activity`, `crm.CommunicationLog`, `crm.CalendarEvent`, `crm.CrmTask`, `core.Document`, and `core.AuditLog` for activity, follow-up, documents, and evidence. `AccountStakeholder` adds only account-specific buying-center context; it is not a second org chart.
- [ ] Keep `crm.ContactProfile.account` as the one primary account. Additional affiliations and account-specific roles belong in `AccountStakeholder`; do not add `is_primary`, a second primary-account field, or a parallel contact identity.
- [ ] Reuse `crm.AccountProfile.parent_account` as the only parent pointer. Harden that existing CRM field for same-tenant organization, non-self, and transitive-cycle safety before any hierarchy tree or rollup is trusted; do not create `sales.AccountHierarchy`, `sales.GlobalAccount`, or a second parent field.
- [ ] Reuse `core.ContactMethod` for accepted email/phone/mobile values and update the CRM compatibility projection in the same transaction. Do not create a second email, phone, address, social-profile, or contact-method table.
- [ ] Reuse `core.ConsentPurpose`/`core.ConsentRecord` and `current_consent()` for lawful-basis metadata and review. An enrichment proposal without the required basis may be recorded as a failed/pending review, but it is not authorized for an external transfer.
- [ ] Never combine `crm.AccountProfile.annual_revenue`, `crm.Opportunity.amount`, and `accounting.Invoice.total` into one figure. Rollups are grouped by verified `accounting.Currency`; missing currency is shown as unavailable, and CRM annual revenue remains metadata without a currency.
- [ ] Treat the physical 8.2 Sales source classes as unverified runtime entities until their top-level re-exports, migration, admin, URL, and navigation wiring exist. 8.3 must not FK to `OpportunityPipeline`, `OpportunityTeams`, `CompetitiveIntelligence`, or `OpportunityOutcomes`; it uses the live `crm.Opportunity` instead.
- [ ] Build no outbound HTTP, provider connector, LinkedIn sync, email-send worker, scheduled enrichment, bulk refresh, merge service, or inferred relationship network. `PartyEnrichmentEvent` is the local proposal/evidence contract for a later integration pass.
...
- [ ] Pin `KIND_CHOICES` exactly: `firmographic`, `contact`, `email_validation`, `phone_validation`, `social`, `employment_change`, `duplicate_check`.
- [ ] Pin `SOURCE_KIND_CHOICES` exactly: `manual`, `provider`, `email_signature`, `linkedin`, `import`, `api`.
- [ ] Pin `STATUS_CHOICES` exactly: `proposed`, `applied`, `rejected`, `no_match`, `failed`. `requested_by` is stamped by the request service; `reviewed_by`/`applied_at` are stamped by the decision service; ordinary status edits are not allowed.
- [ ] Restrict `changes` to a bounded list/object of allowlisted concrete business fields: `job_title`, `department`, `linkedin`, `work_email`, `phone`, `mobile`, `website`, `industry`, `employee_count`, and `annual_revenue`. Each proposal carries only a field name, a scalar value, and an optional per-field confidence; reject nested instructions, arbitrary model paths, executable expressions, credentials, raw provider payloads, and arbitrary fetch URLs. Cap the serialized proposal size and sanitize error text.
- [ ] Require a same-tenant `legal_basis_purpose` for every external transfer source (`provider`, `email_signature`, `linkedin`, `import`, `api`); manual local proposals may omit it. `source_reference` is opaque text, never rendered as a clickable URL.
- [ ] Enforce a tenant-scoped unique constraint on `(tenant, idempotency_key)` when the key is present, plus indexes on `(tenant, party, -occurred_at)`, `(tenant, status, -occurred_at)`, and `(tenant, kind, -occurred_at)`. A replayed idempotency key returns the existing event rather than creating a second row.
...
- [ ] Pin `ROLE_CHOICES` exactly: `decision_maker`, `economic_buyer`, `champion`, `influencer`, `blocker`, `technical_evaluator`, `procurement`, `end_user`, `advisor`, `other`.
- [ ] Pin `INFLUENCE_CHOICES` exactly: `high`, `medium`, `low`, `unknown`; `ATTITUDE_CHOICES`: `positive`, `neutral`, `negative`, `unknown`; `RELATIONSHIP_STRENGTH_CHOICES`: `strong`, `moderate`, `weak`, `unknown`; `STATUS_CHOICES`: `active`, `former`.
- [ ] `account` must be a same-tenant `core.Party` with `kind='organization'`; `contact` must be a different same-tenant `core.Party` with `kind='person'`; `valid_to` cannot precede `valid_from`. Validate on model, form, and service paths, not only through narrowed select widgets.
...
- [ ] Pin `TIER_CHOICES` exactly: `strategic`, `key`, `growth`, `nurture`.
- [ ] Pin `LIFECYCLE_STAGE_CHOICES` exactly: `prospect`, `active_customer`, `expansion_candidate`, `dormant`, `former_customer`.
- [ ] Pin `STRATEGIC_PRIORITY_CHOICES` exactly: `high`, `medium`, `low`; `REVENUE_POTENTIAL_CHOICES`: `very_high`, `high`, `medium`, `low`, `unknown`; `WALLET_CATEGORY_CHOICES`: `none`, `small`, `medium`, `large`, `full_wallet`.
- [ ] Require a non-empty rationale for a tier/lifecycle change and require a review date for `strategic` and `key` rows. `effective_on` defaults to `timezone.localdate()`; an edit creates an audited current row update, not a historical classification table. A duplicate create redirects to the existing row's edit page rather than raising a 500.
...
- [ ] Pin `STATUS_CHOICES` exactly: `draft`, `active`, `review_due`, `completed`, `archived`. New plans start `draft`; the ordinary form does not edit status. POST-only verbs own transitions: `draft -> active`, `active -> review_due`, `active|review_due -> completed`, and `draft|active|review_due|completed -> archived`. Archived plans refuse ordinary edit/delete; non-draft plans are archived rather than destructively deleted.
- [ ] `period_end` must be on or after `period_start`; owner must be active and in the same tenant; every selected opportunity must be a same-tenant `crm.Opportunity` belonging to the same `account`. Validate M2M selections in the form and again in the save service because model `clean()` cannot validate an unsaved M2M set.
- [ ] Enforce unique `(tenant, number)` and indexes on `(tenant, account, status)`, `(tenant, owner, next_review_on)`, and `(tenant, status, next_review_on)`. Use no child `AccountPlanOpportunity` model; the M2M is the plan-to-existing-deal link.
- [ ] Store the strategy narrative and human whitespace assessment only. Do not copy opportunity amounts, product holdings, invoice revenue, CRM health, task state, QBR state, onboarding state, renewal state, or product-coverage counters. Follow-up actions create/reuse `crm.CrmTask`; health and commercial context are read-only links.
- [ ] Account-plan detail must label exact product white-space as incomplete until a governed `crm.Product`/`scm.Item` mapping exists; an empty mapping is not a zero-coverage result.

### Model-free hierarchy, rollup, and workspace features

- [ ] Harden the existing CRM `AccountProfile.parent_account` write path before computing any tree: parent must be a different same-tenant organization with a valid CRM account profile; reject direct model/admin/form/API writes that create a transitive cycle; make the check iterative and cycle-safe rather than relying on a one-level self check.
- [ ] Keep the CRM account form and account detail as the parent/profile editor. Add a reusable CRM validation/service helper and call it from the model, form, view, and admin write paths; Sales consumes the validated pointer and never creates a parallel parent writer.
- [ ] Implement `account_hierarchy` as a read-only explorer over `AccountProfile.parent_account`: root, parent, child, leaf, and descendant filters; a selected account's ancestors/descendants; and an explicit `cycle_detected`/unavailable state rather than recursion failure. Add a bounded row/recursion cap and deterministic ordering.
- [ ] Implement currency-safe hierarchy rollups by grouping `crm.Opportunity` amounts, `scm.SalesOrder` totals, and `accounting.Invoice` totals by verified currency. Show realized, open, weighted, and potential values separately; never combine unlike currencies or treat `annual_revenue` as ledger revenue.
- [ ] Implement `account_workspace` as a computed Account 360 over the canonical Party/CRM profile, direct and descendant accounts, contacts, stakeholder rows, classification, plans, opportunities, orders, invoices, health/history, activity, tasks, documents, and interaction recency. It is not an `Account360` table and it never creates or edits canonical masters.
- [ ] Implement `account_coverage` as a derived buying-center matrix from `AccountStakeholder` plus `crm.ContactProfile.account`: present/missing decision maker, economic buyer, champion, influencer, blocker, and primary contact; active/former counts; reporting-line links; and explainable last/next interaction indicators. Do not call inferred email connections verified relationships.
- [ ] Implement `account_white_space` as a read-only qualitative/derived board over plans, existing opportunities, classification, and canonical product/order data. It must display the missing `crm.Product`/`scm.Item` mapping as a limitation and must not claim complete cross-sell or upsell coverage.
- [ ] Add a single enrichment apply service with bounded field-level overwrite policy. A protected/manual value is not silently replaced; a proposed value that would overwrite a protected value is rejected or requires the explicit review decision. Update `core.ContactMethod`, the relevant CRM profile projection, and `core.AuditLog` atomically, with no raw provider data in audit changes.
- [ ] Add a read-only activity-recency helper over existing CRM/core activity rows. Do not store last-interaction, response ratio, or relationship-strength counters on `AccountStakeholder`; render the query result with its date window and missing-data state.

### Exact backend package and template layout

- [ ] Create only these new files under `apps/sales/models/ContactAccountManagement/`: `__init__.py`, `PartyEnrichment.py`, `AccountStakeholders.py`, `AccountClassifications.py`, and `AccountPlans.py`. The sub-package `__init__.py` files follow the existing Sales convention and contain no model re-exports until the single-writer integration step.
- [ ] Create the matching form files: `apps/sales/forms/ContactAccountManagement/{__init__.py,PartyEnrichment.py,AccountStakeholders.py,AccountClassifications.py,AccountPlans.py}`.
- [ ] Create the matching view files: `apps/sales/views/ContactAccountManagement/{__init__.py,PartyEnrichment.py,AccountStakeholders.py,AccountClassifications.py,AccountPlans.py,AccountBoards.py}`. `AccountBoards.py` is a computed view module, not a model or analytics table.
- [ ] Create the matching URL files: `apps/sales/urls/ContactAccountManagement/{__init__.py,PartyEnrichment.py,AccountStakeholders.py,AccountClassifications.py,AccountPlans.py,AccountBoards.py}`. `apps/sales/urls/__init__.py` keeps `app_name = 'sales'`, concatenates the 8.3 modules after the live 8.1 modules, and preserves literal-before-`<int:pk>` order across the whole app.
- [ ] Extend the existing `apps/sales/forms/_common.py` with tenant-scoped querysets for organizations, persons, opportunities, users, and consent purposes; extend the existing flat `apps/sales/services.py` with the enrichment apply and account-rollup helpers. Do not create a second service package, a second generic workflow engine, or a new `analytics.py` for this sub-module.
- [ ] Add the top-level model/form/view re-export blocks only at Integrate, after all entity files exist. Add the URL imports and concatenation surgically; do not full-rewrite any shared file while concurrent work is live.
- [ ] Create exactly these templates under `templates/sales/contactaccountmanagement/`: `partyenrichmentevent/list.html`; `partyenrichmentevent/detail.html`; `accountstakeholder/{list,detail,form}.html`; `accountclassification/{list,detail,form}.html`; `accountplan/{list,detail,form}.html`; and standalone `account_hierarchy.html`, `account_workspace.html`, `account_coverage.html`, and `account_white_space.html` at the sub-module root. There is no `PartyEnrichmentEvent` form template because proposal creation is a POST action and the event is append-only.
- [ ] Every mutable entity list has search, filters, pagination, and an Actions column; every mutable detail has an Actions sidebar with Edit, POST+CSRF Delete with confirmation, and Back to List. The enrichment list instead has View/Request/Apply/Reject actions and no edit/delete affordance.
- [ ] Use the existing `base.html`, cards, tables, forms, icons, and verified colour-named badges (`badge-green`, `badge-red`, `badge-amber`, `badge-info`, `badge-muted`, `badge-slate`). Do not invent `badge-success`, `badge-warning`, `badge-danger`, or other unverified theme classes. Every status/category badge has an `{% else %}` display fallback.
- [ ] Use `request.GET.status == value` for string choices and `|stringformat:'d'` for FK pk comparisons, never `slugify`. Guard nullable FK display with `{% if fk %}`, guard pagination with `has_previous`/`has_next`, and use only single-line Django comments or `{% comment %}` blocks.

### Forms and validation contract

- [ ] `PartyEnrichmentProposalForm` is a `TenantActionForm`, not a general `PartyEnrichmentEventForm`: fields are exactly `party`, `kind`, `source_kind`, `source_name`, `source_reference`, `changes`, and `legal_basis_purpose`. The request service stamps tenant, requester, status, and timestamps. Parse and validate bounded JSON before saving; never accept a raw provider response.
- [ ] `PartyEnrichmentApplyForm` is an action form with `selected_fields` (a bounded choice set derived from the event's validated allowlist) and `review_note`; `PartyEnrichmentRejectForm` has a required `review_note`. Neither exposes status, tenant, requester, reviewer, applied time, or confidence as editable fields. Apply is a canonical-master write and is tenant-admin gated; request and reject are tenant-scoped member actions with audit.
- [ ] `AccountStakeholderForm.Meta.fields` is exactly `account`, `contact`, `role`, `influence`, `attitude`, `relationship_strength`, `status`, `valid_from`, `valid_to`, and `notes`. Exclude `tenant`, `created_at`, and `updated_at`; validate same-tenant organization/person, account/contact inequality, and date order in `clean()` and in the service.
- [ ] `AccountClassificationForm.Meta.fields` is exactly `account`, `tier`, `lifecycle_stage`, `strategic_priority`, `revenue_potential`, `wallet_category`, `rationale`, `effective_on`, and `review_due_on`. Exclude `tenant`, `classified_by`, `created_at`, and `updated_at`; classification writes require the tenant-admin policy and an audit row.
- [ ] `AccountPlanForm.Meta.fields` is exactly `account`, `title`, `period_start`, `period_end`, `owner`, `business_drivers`, `objectives`, `strategy`, `strengths`, `weaknesses`, `opportunities`, `threats`, `white_space_assessment`, `growth_initiatives`, `risk_summary`, `next_review_on`, and `related_opportunities`. Exclude `tenant`, auto `number`, workflow `status`, and all system timestamps. Status transitions use POST verbs, not a free-form status field.
- [ ] All tenant-owned FK/M2M querysets are narrowed to `request.tenant` in `__init__`, and `_reject_foreign`-style checks run again on crafted POST values. `accounting.Currency` is global and must not be tenant-filtered. `owner`/`classified_by` querysets are active tenant users; system users are stamped by the service.
- [ ] The CRM `AccountForm` parent queryset and clean path must additionally reject a parent that is not an organization, belongs to another tenant, lacks the canonical CRM account profile, or is in the proposed account's descendant chain. This is an in-place CRM hardening item, not a Sales duplicate form.

### Views, URL names, context keys, and filter contracts

#### `PartyEnrichment.py`

- [ ] `party_enrichment_list` (`@login_required`, manual `Paginator` because the model is append-only) filters before pagination: `q` across `party__name`, `source_name`, `source_reference`, `error_code`, and `error_summary`; `party`; `kind`; `source_kind`; `status`; `date_from`; and `date_to`. Validate enum values against the model choices and integer FKs with `as_db_int`; junk values are ignored or safely ignored, never 500.
- [ ] `party_enrichment_list` context is exactly `object_list`, `page_obj`, `q`, `parties`, `kind_choices`, `source_kind_choices`, `status_choices`, `party_id`, `kind`, `source_kind`, `status`, `date_from`, `date_to`, `stats` (`total`, `proposed`, `applied`, `rejected`, `no_match`, `failed`), `proposal_form`, and `export_url`. `parties` is the same-tenant Party queryset, not all Parties.
- [ ] `party_enrichment_detail` context is exactly `obj`, `party`, `proposal_rows`, `canonical_values`, `can_apply`, `can_reject`, `apply_form`, `reject_form`, `audit_events`, and `export_url`. It renders provenance, per-field confidence, current canonical values, legal-basis state, and the honest no-provider/no-send boundary; it never renders raw secrets or an unsafe source-reference link.
- [ ] `party_enrichment_request` is POST-only, accepts `PartyEnrichmentProposalForm`, creates one `proposed` event through the service, and redirects to its detail. `party_enrichment_apply` and `party_enrichment_reject` are POST-only, tenant-scoped, audited, and reject disallowed terminal-state transitions. There is no ordinary create/edit/delete route for `PartyEnrichmentEvent`.
- [ ] `party_enrichment_export` is a login-required, tenant-scoped CSV response using the same filters, the active export cap, and `csv_safe` for every cell. It exports provenance/status and sanitized summaries, not raw `changes` payloads or credentials, and records an audit event with the verb in `changes` when the platform's audit action vocabulary requires it.

#### `AccountStakeholders.py`

- [ ] `account_stakeholder_list` provides `account_stakeholder_list`, `_create`, `_detail`, `_edit`, and `_delete`; delete is POST-only and confirms before removing. The list base is `AccountStakeholder.objects.filter(tenant=request.tenant).select_related('account','contact')`, never `.all()`.
- [ ] Filters: `q` over account/contact Party names and `notes`; `account`; `contact`; `role`; `influence`; `attitude`; `relationship_strength`; `status`; `valid_from`; and `valid_to`. List context is exactly `object_list`, `page_obj`, `q`, `accounts`, `contacts`, `role_choices`, `influence_choices`, `attitude_choices`, `relationship_strength_choices`, `status_choices`, `account_id`, `contact_id`, `role`, `influence`, `attitude`, `relationship_strength`, `status`, `valid_from`, `valid_to`, `stats`, and `export_url`.
- [ ] Detail context is exactly `obj`, `account`, `contact`, `is_primary_affiliation`, `reports_to`, `related_opportunities`, `interaction_recency`, `coverage_roles`, and `can_edit`. Form context is `form`, `is_edit`, `obj` on edit, `accounts`, and `contacts`. Invalid or foreign FKs produce field errors; no cross-tenant object is ever placed in a dropdown.
- [ ] CRUD views are tenant-scoped and audited. A member may manage account-specific stakeholder facts, but no stakeholder row can create or overwrite a CRM contact/account master.

#### `AccountClassifications.py`
...
- [ ] `account_plan_list`, `_create`, `_detail`, `_edit`, and `_delete` are tenant-scoped CRUD. Create defaults to `draft`; ordinary edit cannot change status or number. Delete is POST-only and allowed only for draft rows; active, review-due, and completed plans are archived rather than silently destroyed, while archived plans refuse delete. All writes are audited.
- [ ] Filters: `q` over `number`, `title`, business drivers, objectives, strategy, whitespace assessment, and growth initiatives; `account`; `status`; `owner`; `next_review` (`overdue`, `due_soon`, `scheduled`); and date window `period_from`/`period_to`. List context is exactly `object_list`, `page_obj`, `q`, `accounts`, `owners`, `status_choices`, `account_id`, `owner_id`, `status`, `next_review`, `period_from`, `period_to`, `stats`, and `export_url`.
- [ ] Detail context is exactly `obj`, `account`, `owner`, `related_opportunities`, `opportunity_rollups`, `health`, `health_history`, `tasks`, `recent_activity`, `documents`, `currency_rollups`, `coverage`, `white_space_note`, `allowed_actions`, and `can_edit`. Form context is `form`, `is_edit`, `obj` on edit, `accounts`, `owners`, and `opportunities`.
...
- [ ] `account_coverage` (`GET accounts/coverage/`) accepts `q`, `account`, `role`, and `status`. Context is exactly `coverage_rows`, `accounts`, `role_choices`, `status_choices`, `q`, `account_id`, `role`, `status`, `stats`, and `caveats`. It derives the matrix from stakeholder and primary-contact facts and labels inferred interaction data separately.
- [ ] `account_white_space` (`GET accounts/white-space/`) accepts `q`, `account`, `tier`, `lifecycle_stage`, and `review_due`. Context is exactly `white_space_rows`, `accounts`, `tier_choices`, `lifecycle_stage_choices`, `q`, `account_id`, `tier`, `lifecycle_stage`, `review_due`, `product_mapping_note`, `stats`, and `caveats`. It reports qualitative whitespace and unmapped-product limits without claiming a complete product gap.
- [ ] `account_workspace_export` is a tenant-scoped CSV endpoint for the selected workspace, with the same validated filters, a row cap, currency-separated columns, `csv_safe`, and no cross-tenant enrichment payloads. It is a read/export operation, not a report snapshot model.

#### URL and context integration contract

- [ ] `PartyEnrichment.py` URL names: `party_enrichment_list`, `party_enrichment_detail`, `party_enrichment_request`, `party_enrichment_apply`, `party_enrichment_reject`, and `party_enrichment_export`; paths are `enrichment-events/`, `enrichment-events/request/`, `enrichment-events/export/`, `enrichment-events/<int:pk>/`, `enrichment-events/<int:pk>/apply/`, and `enrichment-events/<int:pk>/reject/`, with both literal routes before the pk route.
- [ ] `AccountStakeholders.py` URL names: `account_stakeholder_list`, `account_stakeholder_create`, `account_stakeholder_detail`, `account_stakeholder_edit`, `account_stakeholder_delete`, and `account_stakeholder_export`; paths are `account-stakeholders/`, `account-stakeholders/add/`, `account-stakeholders/<int:pk>/`, `/edit/`, `/delete/`, and `/export/`.
- [ ] `AccountClassifications.py` URL names: `account_classification_list`, `account_classification_create`, `account_classification_detail`, `account_classification_edit`, `account_classification_delete`, and `account_classification_export`; paths use `account-classifications/` with the same literal-first five-route shape.
- [ ] `AccountPlans.py` URL names: `account_plan_list`, `account_plan_create`, `account_plan_detail`, `account_plan_edit`, `account_plan_delete`, `account_plan_activate`, `account_plan_review_due`, `account_plan_complete`, `account_plan_archive`, and `account_plan_export`; paths use `account-plans/` and literal `add/`/`export/` before all `<int:pk>` routes.
- [ ] `AccountBoards.py` URL names: `account_hierarchy`, `account_workspace`, `account_coverage`, `account_white_space`, and `account_workspace_export`; paths are `accounts/hierarchy/`, `accounts/workspace/`, `accounts/coverage/`, `accounts/white-space/`, and `accounts/workspace/export/`. The export literal precedes any future account pk route.
- [ ] Reverse every declared name in a smoke script after integration. The URLconf must not rely on an unregistered 8.2 class, and every `<int:pk>` lookup must include the tenant scope through the owning view/service.

### Template content contract

- [ ] `partyenrichmentevent/list.html` shows search, party/kind/source/status/date filters, provenance columns, stats, pagination, View and POST Request/Apply/Reject actions where state allows, and no edit/delete buttons. It shows source reference as text, not an arbitrary link.
- [ ] `partyenrichmentevent/detail.html` shows the Party link, proposed field/value/confidence preview, current canonical value, legal basis, requester/reviewer/timestamps, sanitized error, and the exact apply/reject forms. It states that local validation is not a successful send or provider verification and that no outbound provider is called.
- [ ] `accountstakeholder/{list,detail,form}.html` shows all account-specific role/influence/attitude/strength/status fields, primary affiliation distinction, links to CRM contact/account pages, activity recency, and the full Actions column/sidebar. The form never edits `core.Party` or `crm.ContactProfile` master fields.
- [ ] `accountclassification/{list,detail,form}.html` shows tier/lifecycle/priority/potential/wallet, rationale and review state, the read-only CRM health/rollup context, and the full Actions column/sidebar. It never presents classification as a combined health score.
- [ ] `accountplan/{list,detail,form}.html` shows number, account, period, status, owner, review date, plan narrative panels, linked opportunities, tasks/health context, and the full Actions column/sidebar. Status buttons are POST+CSRF and state-gated; the form has no status/number/system timestamp inputs.
- [ ] `account_hierarchy.html` renders a deterministic tenant-scoped tree/list with root/parent/child/descendant filters, cycle/unavailable messaging, and currency-separated rollup rows. It links to CRM account pages for parent/profile edits.
- [ ] `account_workspace.html` renders the canonical profile links, classification, stakeholders, plans, opportunity/order/invoice context, health/history, activity/tasks/documents, coverage, enrichment review, and honest white-space/mapping notes. It uses `id="enrichment"`, `id="relationships"`, and `id="growth"` anchors for navigation deep links.
- [ ] `account_coverage.html` renders role coverage, primary-contact status, reporting-line links, active/former counts, and interaction indicators with their window; it labels inferred/derived values and never calls an unverified connector a fact.
- [ ] `account_white_space.html` renders qualitative initiatives/assessment, classification, linked opportunities, and the explicit product-catalog mapping limitation. It does not show a fake zero for unmapped products.
- [ ] Export pages/links use the shared download conventions, active-filter parity, `csv_safe`, bounded output, and a notice when rows are truncated. No export, log, or flash message contains raw provider payloads, access tokens, or unnecessary PII.

### Admin, export, navigation, and seed integration

- [ ] Register exactly the four new models in `apps/sales/admin.py`. Use tenant/list filters, search, `list_select_related`, and readonly system fields; make `PartyEnrichmentEvent` append-only in admin with no ordinary add/change/delete permission; keep `AccountPlan.number`, workflow status, and verb stamps out of direct admin edits.
- [ ] Add a safe admin queryset/action guard for account/contact Party kinds and same-tenant plan/stakeholder relations. Do not register a second account/contact/hierarchy/health/opportunity model in Sales admin.
...
- [ ] Print `admin_acme` / `password` and the existing warning that superuser `admin` has `tenant=None` and sees no tenant Sales data. State in seed output that provider enrichment, email/phone verification, LinkedIn sync, and sending are not simulated workers in 8.3.

### Migration, tenant, security, and performance verification

- [ ] At Phase 3, after every expected backend/template file is present and top-level re-exports/URL concatenation are ready, run `makemigrations sales --dry-run` and inspect every named model. The expected new migration contains only the four 8.3 tables; a parent-hierarchy hardening change should be schema-free unless an explicitly reviewed CRM index is added.
- [ ] Assign the next free `sales` migration number at generation time after re-reading the disk leaf. The current checkout suggests `0004`, but concurrent work and registry-wide changes may advance it; do not reserve or overwrite another session's migration.
- [ ] Generate the migration, run `migrate`, run `showmigrations sales`, run `manage.py check`, and confirm `makemigrations --check` reports no pending model changes. The migration must not create tables for 8.2 source classes, Account/Contact masters, health, opportunities, orders, products, or financial records.
- [ ] Verify every model has `tenant`, every list/detail query filters `tenant=request.tenant`, every FK/M2M selection is same-tenant, Party kinds are enforced, and a tenantless `admin` receives empty tenant-scoped pages by design. Cross-tenant list, detail, edit, delete, apply/reject, export, board, and action IDs must return 404 or a field error, never a leak.
- [ ] Test parent invariants directly: no self-parent, no cross-tenant parent, no non-organization parent, no missing CRM account profile, and no transitive cycle through a descendant. Verify direct model/admin/form/API paths all invoke the guard; hierarchy computation must stop safely if legacy corrupt data is encountered.
- [ ] Test enrichment security: arbitrary JSON/paths/instructions fail validation; provider/external source without purpose/consent is refused; source references are text, not links; errors are sanitized; apply updates ContactMethod plus the CRM projection atomically; rejected/terminal events cannot be replayed into a second apply; no network call, credential, raw payload, or PII is logged.
- [ ] Test classification and plan integrity: duplicate account classification redirects/upserts, rationale/review requirements hold, strategic review dates are required, plan numbering is tenant-unique, period order holds, owners are active/same-tenant, M2M opportunities are same-account/same-tenant, status transitions are one-way and idempotent, and non-draft plans cannot be silently deleted.
- [ ] Test tenantless/GET/POST policy: every mutating verb is `@require_POST` and returns 405 on GET; classification governance and enrichment apply are tenant-admin gated; member CRUD and read boards follow the pinned policy; CSRF is present on every state-changing form; delete confirms before POST; no route permits an open redirect from a stored source reference.
- [ ] Smoke as `admin_acme` / `password`: render every new list, detail, form, board, and export route with content assertions, not only status codes; assert seeded numbers/Party names and the expected account/plan/stakeholder/enrichment values appear; assert no `{#` or `{% comment` markers leak; assert no blank context-driven regions.
- [ ] Sweep junk filters on every list/board (`status=nope`, `kind=unknown`, `account=abc`, `party=-1`, `date_from=bad`, `page=9999`) and prove valid filters return the correct rows before pagination, page 1/page 2 links are valid, and malformed dates/integers never 500. Verify the page-2 path with enough seeded rows.
...
- [ ] Freeze the complete contract in `.claude/tasks/contract-sales-8.3.md` before implementation: every model field and choice, form `Meta.fields` and exclusions, URL names/paths, view context keys, filter names, tenant/kind/cycle rules, service/overwrite policy, export columns, seed guards, and migration rule. No key may be left for an implementation agent to guess.
- [ ] Phase 4 is six serial read-only reviews in this order: `code-reviewer`, `explorer`, `frontend-reviewer`, `performance-reviewer`, `qa-smoke-tester`, `security-reviewer`. Append each result to `.claude/tasks/review-sales-8.3.md`; a missing result is missing coverage, not a clean bill of health.
- [ ] Phase 5 uses one `code-fixer` pass to burn findings down in Critical -> Important -> Minor order, verify each fix, mark the review file, and leave no finding open. The main session does not silently expand scope while fixing.
