...
- [x] **C1** — `apps/projects/views/WorkflowAutomation/*.py` fixed — fix(projects): correct write_audit_log arguments in WorkflowRules/ApprovalGates/RecurringTasks/Webhooks views (C1)
...
- [x] **C2** — `apps/projects/models/WorkflowAutomation/RecurringTasks.py:130` fixed — fix(projects): align RecurringTaskSchedule.generate_task with ProjectTask schema (C2)
...
- [x] **C3** — `apps/projects/forms/WorkflowAutomation/ApprovalGates.py:57` & `apps/projects/views/WorkflowAutomation/ApprovalGates.py:278` fixed — fix(projects): scope ApprovalDelegateForm queryset by tenant and verify delegate tenant (C3)
...
- [x] **C4** — `apps/projects/views/WorkflowAutomation/ApprovalGates.py:194-271` fixed — fix(projects): enforce authorization and block self-approval on approval gates (C4)
...
- [x] **C5** — `apps/projects/forms/WorkflowAutomation/ApprovalGates.py:16-32` & `apps/projects/views/WorkflowAutomation/ApprovalGates.py:101-109` fixed — fix(projects): remove auto_approve_threshold from ProjectApprovalGateForm (C5)
...
- [x] **C6** — `templates/projects/workflowautomation/boards/overview.html:85` & `boards/approval_inbox.html:63, 140` fixed — fix(projects): use gate.title instead of gate.name in overview/approval_inbox templates (C6)
...
- [x] **C7** — `templates/projects/workflowautomation/boards/overview.html:204, 221` fixed — fix(projects): use log.rule trigger displays and fired_at in overview template (C7)
...
- [x] **C8** — `apps/projects/views/WorkflowAutomation/RecurringTasks.py:80` fixed — fix(projects): select_related assignee in rts_detail to prevent N+1 queries (C8)
...
- [x] **I1** — `apps/projects/views/WorkflowAutomation/ApprovalGates.py:174-190` fixed — fix(projects): prevent deletion of decided approval gates in par_delete (I1)
...
- [x] **I2** — `apps/projects/views/WorkflowAutomation/Webhooks.py:99, 259` & `templates/projects/workflowautomation/webhook/detail.html` fixed — fix(projects): store/pop and render one-time webhook secret reveal (I2)
...
- [x] **I3** — `apps/projects/views/WorkflowAutomation/*.py` (WorkflowRules:38, ApprovalGates:35, RecurringTasks:33, Webhooks:35, 278) fixed — fix(projects): guard GET integer filter params with isdigit (I3)
...
- [x] **I4** — `templates/projects/workflowautomation/recurringtask/detail.html:151` fixed — fix(projects): use assignee and planned_end in recurringtask detail template (I4)
...
- [x] **I5** — `templates/projects/workflowautomation/workflow/detail.html:253` fixed — fix(projects): provide user input field for target_id in workflow execute now form (I5)
...
- [x] **I6** — `templates/projects/workflowautomation/boards/*.html` (overview, approval_inbox, recurrence_calendar, webhook_diagnostics) fixed — fix(projects): replace table-responsive with table-wrap and btn-xs with btn-sm in board templates (I6)
...
- [x] **I7** — `templates/projects/workflowautomation/webhook/delivery_detail.html:54` fixed — fix(projects): support all 2xx HTTP response codes as success in delivery_detail template (I7)
...
- [x] **I8** — `apps/projects/views/WorkflowAutomation/*.py` (RecurringTasks:208, WorkflowRules:246, Webhooks:227) fixed — fix(projects): wrap multi-step mutations in transaction.atomic (I8)
...
- [x] **I9** — `apps/projects/models/WorkflowAutomation/WorkflowRules.py:103` & `Webhooks.py:123` fixed — perf(projects): add composite indexes to WorkflowExecutionLog and ProjectWebhookDelivery (I9)
...
- [x] **I10** — `apps/projects/views/WorkflowAutomation/AutomationBoards.py:132` & list views fixed — perf(projects): consolidate multi-scan queries into aggregates (I10)
...
- [x] **I11** — `apps/projects/views/WorkflowAutomation/WorkflowRules.py:204, 242` fixed — fix(projects): validate target_id belongs to tenant in pwf_test_run and pwf_execute_now (I11)
...
- [x] **I12** — `apps/projects/management/commands/seed_projects.py:4170` fixed — fix(projects): standardize rule conditions and title_template syntax in seeder (I12)
...
- [x] **M1** — `apps/projects/models/WorkflowAutomation/WorkflowRules.py:101` & `views/WorkflowAutomation/WorkflowRules.py:218` fixed — fix(projects): standardize executed_actions format to list of dicts in pwf_test_run (M1)
...
- [x] **M2** — `templates/projects/workflowautomation/recurringtask/detail.html:151` fixed — fix(projects): guard nullable assignee in recurringtask detail template (M2)
...
- [x] **M3** — `apps/projects/forms/WorkflowAutomation/Webhooks.py:21` fixed — fix(projects): require at least one event type in ProjectWebhookEndpointForm (M3)
...
- [x] **M4** — `apps/projects/forms/WorkflowAutomation/ApprovalGates.py:25` fixed — fix(projects): exclude requested_by from ProjectApprovalGateForm and set in view (M4)
...
- [x] **M5** — `templates/projects/workflowautomation/boards/*.html` fixed — style(projects): refactor board stat cards to design system (M5)
...
- [x] **M6** — `apps/projects/views/WorkflowAutomation/AutomationBoards.py:72, 104` fixed — perf(projects): eliminate redundant count queries before iteration in boards (M6)
...
- [x] **M7** — `apps/projects/models/WorkflowAutomation/WorkflowRules.py:110` & `Webhooks.py:130` fixed — fix(projects): eliminate N+1 query in __str__ methods (M7)
