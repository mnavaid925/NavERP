...
- **Status:** [x] fixed — migration 0030 adds `(tenant, owner_client)` to BOTH `Item.Meta.indexes` (`scm_item_tnt_owner_idx`) and `Location.Meta.indexes` (`scm_loc_tnt_owner_idx`) in one change, per the note below that it must not be applied to one model only. `0030` was claimed by the main session after confirming `0029` was committed and the tree held no concurrent session's work; `makemigrations --check` reports no drift and `manage.py check` is clean.
