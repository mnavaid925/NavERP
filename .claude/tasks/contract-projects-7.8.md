...
3. **`tsk_block` refuses terminal statuses** (extends §5.1). A `done` or `cancelled` task cannot raise a `TaskBlock` — the verb refuses with a named message. The check is re-tested INSIDE the `select_for_update` block alongside the one-open-blocker re-test, so a concurrent complete/cancel cannot slip a block past the gate. The one-open-blocker invariant is unchanged.
...
9. **`TaskExecutionForm` guards attested history** (extends §3/§5.1, review M9). A `cancelled` task refuses execution-field writes outright (`clean()` raises a named error); a `done` task keeps its verb-attested `percent_complete` — `clean()` restores the stored value rather than rejecting the form, so the other five fields stay editable while the completion stamp is frozen. `tsk_complete` wrote that 100 as evidence and a form must not be able to quietly lower it.
10. **The three 7.8 pages flash when a filter id is not in this workspace** (amends §5.5's pinned degrade behaviour, review M11). A well-formed `?project=` / `?assignee=` id that resolves to nothing still degrades to the unfiltered workspace with **no 500** — but it now also emits a `messages.warning` naming the fallback, so the user is not silently shown rows they did not ask for.
11. **The bulk update is capped and terminal-gated** (extends §5.1's bulk verb, reviews I9/M10). The POST accepts at most `_BULK_CAP = 500` ids (the remainder is reported, not silently dropped); every id resolves through ONE tenant-scoped `pk__in` fetch; the old assignee User is loaded only in the audit branch; and a transition to a terminal status is refused for any row with an open `TaskBlock` (named by its TBK number) so a block can never outlive the work it was holding.
