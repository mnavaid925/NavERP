...
description: >-
  ...
  acknowledgement). Use when the user asks to add/change/debug anything under apps/projects or
  templates/projects, extend the seed_projects seeder, touch project sidebar wiring
  (LIVE_LINKS 7.1), work on ProjectRequest/Project/ProjectStakeholder/ProjectKickoff, or invokes
  /projects.
---

# Module 7 — Project Management (`apps/projects`)

**As-built: 7.1 only.** 7.2–7.19 are roadmap. Do not assume a model exists because NavERP.md lists
the feature — check first.
...
## Routes (`app_name = "projects"`, 32 names)

`overview` · `prq_{list,create,detail,edit,delete}` · `prj_…` · `pst_…` · `pko_…` plus the verbs.

**The 15 POST-only verbs are `@require_POST`, so a GET returns 405, not 302** — that is the house
pattern, not a bug.
...
Entity folders `projectrequest/ project/ projectstakeholder/ projectkickoff/`, plus
`templates/projects/overview.html` at the app root. Extend `base.html`; colour-named theme.css
...
Idempotent, per-tenant guarded. Creates **9 ProjectRequests / 3 Projects / 6 ProjectStakeholders /
2 ProjectKickoffs per tenant** (Acme + Globex) plus `core.Activity` rows. Log in as `admin_acme` /
`admin_globex`, password `password`. Run it twice to prove idempotency.
...
## Tests — `apps/projects/tests/` (1243, green unfiltered)

`conftest.py` (45 fixtures — **owned by itself; edit it only with a full unfiltered re-run**) plus
`test_initiation_{models,forms,views,security}.py`. Naming: every test `test_projectinitiation_*`,
every helper `_projectinitiation_*`, so the next sub-module cannot shadow them.
...
- **Build 7.2+** — use `/next-module 7.2`. New sub-module = a new `<SubModule>/` folder in each of
