...
- **System-managed fields are excluded from their ModelForm** (prevents mass-assignment self-approval/forgery): `Expense.status/submitted_by/approved_by`, **`Timesheet.status/approved_by`** (1.8 — status moved off the form), **`ContractDocument.status/current_version/body_snapshot`** (1.9 — body_snapshot is generated, not typed), `ProductStock.on_hand_qty`. Set them only in the dedicated action views.
