...
All four former monoliths were split into packages that share the **same 12 sub-module folders and
the same entity file names**, so the four layers line up one-to-one:

```
apps/crm/
  models/   _base.py (django imports + abstract TenantNumbered)   <- from apps.crm.models._base import *
  forms/    _common.py (django forms + core TenantModelForm ...)  <- from apps.crm.forms._common import *
  views/    _common.py (django + apps.core crud/decorators ...)   <- from apps.crm.views._common import *
  urls/     __init__.py sets app_name="crm" + concatenates each entity module's urlpatterns
     +-- CoreData/ SalesForceAutomation/ MarketingAutomation/ CustomerService/ ActivityManagement/
         AnalyticsReporting/ FinanceBilling/ ProjectDelivery/ DocumentContract/ AutomationWorkflow/
         CustomerSuccess/ InventoryVendor/          (= NavERP sub-modules 1.1 .. 1.12)
```

e.g. the Quote entity is `models/SalesForceAutomation/Quotes.py` (`Quote` + `QuoteLine`),
`forms/SalesForceAutomation/Quotes.py`, `views/SalesForceAutomation/Quotes.py`,
`urls/SalesForceAutomation/Quotes.py`.

**Every package `__init__.py` re-exports everything**, so `from apps.crm.models import Lead`,
`from apps.crm.forms import LeadForm`, `views.<name>` in the URLconf, and
`include("apps.crm.urls")` in `config/urls.py` all work exactly as before the split.

**Rules when working here:**
- **Adding a model / form / view:** put it in the entity file, **then add it to that package's
  `__init__.py` re-export block** — otherwise `from apps.crm.models import X` / `views.<name>` will
  fail. Adding a new entity = a new `<SubModule>/<Entity>.py` in each layer + its re-export block.
- **Adding a URL:** add the `path()` to `urls/<SubModule>/<Entity>.py`'s `urlpatterns`. If you add a
  brand-new entity module, wire it into `urls/__init__.py` too. **Order matters** — Django is
  first-match-wins; put literal routes before `<int:pk>` ones inside the same list.
- **Models:** entity files hold the primary model **plus its children** (`Cases.py` = `Case` +
  `CaseComment`). The abstract `TenantNumbered` lives in `models/_base.py`; the 1.6 choice lists in
  `models/AnalyticsReporting/_choices.py`. Model modules sit deeper than the app root but Django
  still derives `app_label="crm"` from the app config, so **migrations are unaffected**.
- **Imports inside these packages must be ABSOLUTE** (`from apps.crm.models import X`). A relative
  `from .models import X` now resolves to the wrong package — this bit `forms.py` twice during the
  split.
- The 1.10 workflow engine helpers (`_run_rule` etc.) live in `views/AutomationWorkflow/_engine.py`;
  `views/__init__.py` also re-exports 4 private helpers (`_render_doc_body`, `_safe_record_field`,
  `_eval_conditions`, `_run_rule`) that `seed_crm.py` and `tests/test_workflow_110.py` import.
- `apps/crm/forms` intentionally no longer re-exports **model** classes (the old flat `forms.py`
  leaked them). Import models from `apps.crm.models`.

## Models (`apps/crm/models/` — one file per entity; see App layout above)
...
is a module-level constant in `models/CoreData/Accounts.py` (re-exported from `apps.crm.models`). `website`/`linkedin` use `forms.URLField(assume_scheme="https")`.

## URLs / routes (`apps/crm/urls/` package, `app_name="crm"`)
...
- **Adding a view:** put it in the entity's file, then **add it to `views/__init__.py`'s re-export block
  for that file** — otherwise `views.<name>` in the URLconf will `AttributeError`. Add the `path(...)` to
  `urls/<SubModule>/<Entity>.py`'s `urlpatterns`. Each URL module imports views **absolutely**
  (`from apps.crm import views`); a relative `from . import views` would resolve to
  `apps.crm.urls.<SubModule>.views` and raise ImportError.
...
- **Add a field to a model:** edit the model's entity file under `apps/crm/models/<SubModule>/`; add to the matching `Meta.fields` in
  `forms/<SubModule>/<Entity>.py` (unless system-set); surface in the entity's `detail.html`/`form.html`/`list.html`;
...
validators). Models under `apps/crm/models/`:
...
**Routes (`urls/<SubModule>/*.py`):** standard `<entity>_list/_create/_detail/_edit/_delete` (delete POST-only) for
...
**Forms (`forms/SalesForceAutomation/*.py`):** `OpportunityForm` (+ forecast_category/competitor/loss_reason/territory; lost_at/
...
(Case + KnowledgeArticle columns + 4 tables). Models under `apps/crm/models/`:
...
**Forms (`forms/CustomerService/*.py`):** `CaseForm` (+ sla_policy; SLA timers/closed_at/CSAT/public_token excluded),
...
**Forms (`forms/MarketingAutomation/*.py`):** `CampaignForm` (+ objective/parent_campaign/utm_*, self-parent excluded) + per-model forms.
...
`apps/crm/analytics.py`** — the `models/` package owns only the field choice lists
(`models/AnalyticsReporting/_choices.py`) and never imports `analytics.py`
...
Added as an extension pass on the **same `apps/crm` app** (the `models/`, `forms/`, `views/` and
`urls/` packages, `admin.py`, `seed_crm.py`). Migration `0005` created the original 18 tables; later migrations
...
**1.11** (`OnboardingTemplate` + `OnboardingTemplateStep` + `HealthScoreHistory`; `Survey.classification` per-type;
`CrmTask` `(tenant,party)` index). **Spine status:**
...
| `OnboardingStep` | — (plain) | 1.11 | order(auto in view), title, assignee, due_date, completed_at; inline on plan detail; has an edit form | `plan`→OnboardingPlan(CASCADE) |
| `OnboardingTemplate` | `OTPL-` | 1.11 | name, description, is_active; prop `step_count`; **reusable blueprint** — `onboardingtemplate_apply` clones its steps into a new OnboardingPlan; **authoring `@tenant_admin_required`**; idx `(tenant,is_active)` | — |
| `OnboardingTemplateStep` | — (plain) | 1.11 | order(auto), title, description, `offset_days` (cloned step due = plan start + N days); inline on template detail | `template`→OnboardingTemplate(CASCADE) |
...
| `HealthScoreConfig` | — (plain, OneToOne tenant) | 1.11 | weight_tickets/nps/tasks/engagement (form validates sum=100 + red<yellow), red/yellow_threshold (singleton per tenant) | — |
| `HealthScoreHistory` | — (plain) | 1.11 | **append-only** trend: score/tier/breakdown/computed_at; one row per recompute; list/detail-only (admin add/change/delete blocked); idx `(tenant,account,-computed_at)` | account→Party(CASCADE) |
| `Survey` | `NPS-` | 1.11 | survey_type(nps/csat/ces), trigger, score, **`classification` auto-set per type in `save()`** (NPS promoter/passive/detractor · CSAT satisfied/neutral/dissatisfied · CES easy/neutral/hard), `token`=token_urlsafe(32), sent_at(via send action)/responded_at; SurveyForm validates per-type score range | account/contact→Party, related_case→Case |
...
**Service fn `compute_health_score(party, tenant, config=None)`** (`models/CustomerSuccess/HealthScores.py`, `transaction.atomic()`):
derives a 0–100 score from `crm.Case` (open), latest `crm.Survey` NPS, `crm.CrmTask` completion, and open
`crm.Opportunity` engagement, weighted by `HealthScoreConfig`; `update_or_create`s one `HealthScore` per account,
**appends a `HealthScoreHistory` trend row**, and on **Red tier auto-raises one guarded churn-risk `CrmTask`**
(high/follow_up — skipped if an open "Churn risk:" task already exists). Pass `config` to reuse one config across a
bulk loop. Called by `recompute_health_score` (single) + `recompute_all_health_scores` (admin bulk) + the seeder.
(No invoice/payment signal — Accounting not built.)
...
- **1.11 Onboarding:** `onboardingstep_add`/`_complete`(toggle)/`_edit`/`_delete`; **templates** `onboardingtemplate_*` CRUD + `onboardingtemplatestep_add`/`_edit`/`_delete` + `onboardingtemplate_apply` (clone steps → new plan) — **all template authoring is `@tenant_admin_required`** (list/detail/apply are `@login_required`).
- **1.11 Health:** `recompute_health_score` (member, audits) + `recompute_all_health_scores` (**`@tenant_admin_required`**, per-account error-isolated); `health_config_edit` (**`@tenant_admin_required`**).
- **1.11 Surveys:** `survey_results` (NPS analytics — one conditional-aggregate query); `survey_send` (**`@tenant_admin_required`**, stamps sent_at, email deferred); **public** `survey_respond(token)` (no login; parses score via `int()` not `isdigit()` → type-aware clamp NPS 0–10/CSAT 1–5/CES 1–7, caps feedback 4000, won't lock on empty score, single-response guard).
...
1.11 (Onboarding Pipelines/Health Scoring/Surveys & Feedback (NPS) + **Onboarding Templates**→`onboardingtemplate_list` & **Survey Analytics**→`survey_results` extras), 1.12 (Purchase Orders/Stock Tracking/
...
list views are paginated. `PurchaseOrder.recalc_total()` / `compute_health_score` aggregate DB-side. **1.11:**
`survey_results` is one conditional-aggregate query; bulk recompute fetches `HealthScoreConfig` once; `CrmTask
(tenant,party)` index backs the per-account task counts; `HealthScoreHistory (tenant,account,-computed_at)` +
`OnboardingTemplate (tenant,is_active)` indexes; `onboardingtemplate_list` annotates `n_steps` (no per-row count).
