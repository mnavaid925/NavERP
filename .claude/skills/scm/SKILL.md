...
Migrations **0031** (the four models + `Item.weight_kg`/`volume_cbm`) and **0032** (`scm_lcv_tnt_party_idx`, the
`(tenant, party)` index on `LandedCostVoucher` — review finding **M3**). M3 was deferred at review time only because
0031 was the number claimed in a shared checkout; it landed as its own migration once that window cleared. **No
app-wide index sweep is owed** — 4.17's `(tenant, owner_client)` finding was never deferred alongside it, having
already shipped on `Item` and `Location` in **0030**.
