...
3.23 Learning Management (LMS).** Reuses the
...
+ a department `OrgUnit` — creates NO new course/person rows. ASCII-only stdout. Runs **LAST**, after `_seed_training`.
...
upgrade, evergreen auto-reopen), (the Performance-Management cluster — 3.18 Goal Setting, 3.19 Performance Review, 3.20 Continuous Feedback, 3.21 Performance Improvement — is now **built**; 3.22 Training Management + 3.23 Learning Management (LMS) are now **built**, with 3.24 Training Administration the deferred sibling),
