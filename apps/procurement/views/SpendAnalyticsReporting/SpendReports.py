...
* **Every queryset is ``filter(tenant=request.tenant)``** — never ``.all()``. Both models carry
  their own ``tenant`` column, so a snapshot is fetched
  ``get_object_or_404(SpendReportSnapshot, pk=pk, tenant=request.tenant)`` rather than through
  its parent report.
...
def _report_qs(request):
    return SpendReport.objects.filter(tenant=request.tenant).select_related(*_ROW_RELATIONS)
...
def spendreport_list(request):
    ...
    base = SpendReport.objects.filter(tenant=request.tenant)
    stats = {
        "total": base.count(),
...
def spendreportsnapshot_detail(request, pk):
    ...
    snapshot = get_object_or_404(
        SpendReportSnapshot.objects.select_related("report", "generated_by"),
...
def spendreportsnapshot_export(request, pk):
    ...
    snapshot = get_object_or_404(SpendReportSnapshot, pk=pk, tenant=request.tenant)
    data = snapshot.data if isinstance(snapshot.data, dict) else {}
    columns, rows = _result_table(data)
