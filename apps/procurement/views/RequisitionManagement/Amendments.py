...
requisitions. Requesting is self-service (any staff member, on their requisitions); deciding is
tenant-admin gated (the same gate 4.1 puts on approve/reject) because ONLY the approve action
mutates the ``scm.PurchaseRequisition`` spine. Every decision is recorded and audited; the
amendment row itself is the reviewable diff.
"""
...
def req_amendment_create(request, requisition_pk):
    """File an amendment against one requisition (self-service; admin decides later)."""
    if request.tenant is None:
        messages.error(request, "Select a tenant workspace before filing amendments.")
        return redirect("dashboard:home")
    requisition = get_object_or_404(PurchaseRequisition, pk=requisition_pk,
    ...
    if requisition.status not in RequisitionAmendment.AMENDABLE_STATUSES:
        messages.error(request,
                       ...
                       f"amended — drafts can be edited directly; converted/cancelled ones are "
                       f"closed.")
    ...
    if request.method == "POST":
        form = RequisitionAmendmentForm(request.POST, instance=amendment, tenant=request.tenant)
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if form.cleaned_data.get("amendment_type") == "cancel" and filled:
                form.add_error(None,
                               "Line changes apply only to 'Amend details' — switch the type or "
                               "remove the proposed line rows.")
            else:
...
def amendment_approve(request, pk):
    ...
    obj = get_object_or_404(RequisitionAmendment.objects.select_related("requisition"),
                            pk=pk, tenant=request.tenant)
    ...
    if requisition.status not in RequisitionAmendment.AMENDABLE_STATUSES:
        messages.error(request,
                       f"The requisition is now '{requisition.get_status_display()}' — there is "
                       f"nothing left to amend. Reject this amendment instead.")
        return redirect("procurement:amendment_detail", pk=pk)
