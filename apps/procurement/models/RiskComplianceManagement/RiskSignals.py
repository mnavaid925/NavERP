...
#: The ``ProcurementAlert.kind`` this model raises (``Policies.py`` imports it and raises the
#: same kind for an overdue attestation, so this is the kind of EVERY alert 6.17 creates).
#:
#: **HAND-OFF COMPLETE.** ``("risk", "Risk")`` is registered in ``ProcurementAlert.KIND_CHOICES``
#: and ``kind_css`` maps it to ``badge-red``
#: (``apps/procurement/models/DashboardPortal/ProcurementAlerts.py``). Registering it is not
#: cosmetic, which is what the earlier note here got wrong: the 6.1 inbox builds its kind
#: ``<select>`` from ``KIND_CHOICES``, so an unregistered value **cannot be filtered for**
#: (``crud_list._enum_values`` silently drops the unknown param and returns the UNFILTERED
#: inbox); ``get_kind_display()`` returns the bare value, so the chip is unlabelled; and
#: ``ProcurementAlertForm`` carries ``kind`` in ``Meta.fields``, so editing a risk alert through
#: 6.1's form would post the first option and silently re-label it "Deadline".
#:
#: The matching ``AlterField`` on ``procurementalert.kind`` is generated by the session that owns
#: migrations (precedent: ``0012_alter_procurementalert_kind`` did exactly this for 6.8's
#: ``"contract"``).
ALERT_KIND = "risk"
