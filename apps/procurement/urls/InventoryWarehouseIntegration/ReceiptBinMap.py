"""Procurement 6.18 Inventory & Warehouse Integration — Receipt → Bin map URL pattern.

One route, one first segment: ``receipt-bin-map/``, collision-checked as a whole COMPONENT against
the concatenated ``urls/__init__.py`` inventory. Django matches path components, not strings, so
this segment cannot reach ``material-issues/`` or any other in this app however similarly they read.
...
from apps.procurement.views.InventoryWarehouseIntegration import ReceiptBinMap as views


urlpatterns = [
    path("receipt-bin-map/", views.receipt_bin_map, name="receipt_bin_map"),
]
