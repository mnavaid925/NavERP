...
### **Claude Code Folder Structure Mapping (Authoritative)**

This project was built with Claude Code and uses the `.claude/` directory as its authoritative configuration and workflow hub. Antigravity integrates with and strictly preserves this structure:

1. **Rules & Guidelines (`.claude/CLAUDE.md` & `.claude/PROMPT.md`)**:
   - Master reference for all coding standards, Django function-based views, multi-tenancy, backend package structures, color-named badges, and PowerShell single-file git commit rules. Antigravity must read and enforce these rules verbatim.
2. **Tasks & Roadmaps (`.claude/tasks/`)**:
   - Master checklist & roadmap: `.claude/tasks/todo.md`
   - Pre-build contracts: `.claude/tasks/contract-<slug>-<N.M>.md`
   - Research catalogs: `.claude/tasks/research-<slug>-<N.M>.md`
   - Review findings: `.claude/tasks/review-<slug>-<N.M>.md`
   - Codified lessons: `.claude/tasks/lessons.md` (read before building, update on any user correction)
   - Antigravity MUST read, update, and commit task files directly in `.claude/tasks/`.
3. **Skills (`.claude/skills/`)**:
   - All 11 domain and utility skills reside in `.claude/skills/` (`next-module`, `sqa-review`, `manual-test`, `dump-module`, `accounting`, `crm`, `hrm`, `scm`, `projects`, `procurement`, `inventory`).
   - Registered in Antigravity via `.agents/skills.json` using workspace-relative path `.claude/skills`.
   - When a skill or slash command (e.g. `/next-module`) is invoked, Antigravity executes the procedure defined in `.claude/skills/<skill>/SKILL.md`.
4. **Agents (`.claude/agents/`)**:
   - Specialist agent personas are defined in `.claude/agents/*.md` (`research.md`, `todo.md`, `explorer.md`, `code-reviewer.md`, `frontend-reviewer.md`, `performance-reviewer.md`, `qa-smoke-tester.md`, `security-reviewer.md`, `code-fixer.md`, `test-writer.md`).
   - In Antigravity, subagents are executed strictly serially (ONE at a time) using these system prompts.
5. **Preservation Invariant**:
   - Keep `.claude/` folder intact. Never move, rename, delete, or break the `.claude/` hierarchy.
